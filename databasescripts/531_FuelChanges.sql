...
VALUES ('531', '2017-10-08 16:40:18', 'Mrudang Vora', 'Fuel Sensor Changes', '0');

/* START - Not related to fuel but db patch was missing */
ALTER TABLE `geofence` CHANGE `geolat` `geolat` DECIMAL( 9, 6 ) NOT NULL;
ALTER TABLE `geofence` CHANGE `geolong` `geolong` DECIMAL( 9, 6 ) NOT NULL;
ALTER TABLE `geofence_archive` CHANGE `geolat` `geolat` DECIMAL( 9, 6 ) NOT NULL;
ALTER TABLE `geofence_archive` CHANGE `geolong` `geolong` DECIMAL( 9, 6 ) NOT NULL;
/* END - Not related to fuel but db patch was missing */

UPDATE `speed`.`vehicle` SET `fuelMaxVoltCapacity` = '325',
`fuel_min_volt` = '1.09',
`fuel_max_volt` = '3.64' WHERE `vehicle`.`vehicleid` =10020;

ALTER TABLE `vehicle` ADD `old_fuel_balance` DECIMAL(6,2) NULL AFTER `fuel_balance`;
				...
				*/
                ...
                IF  fuelMaxVoltCapacityVar IS NOT NULL AND fuelMinVoltVar IS NOT NULL AND fuelMaxVoltVar IS NOT NULL
					...
					BEGIN
						...
						SET fuelInaccuracyPercentVar = 5;
						...
						IF oldFuelBalanceVar IS NULL THEN
							BEGIN
								UPDATE  vehicle
								SET     old_fuel_balance = fuelBalanceVar
								WHERE   uid = uidParam;
							END;
						END IF;
					END;
...
UPDATE speed.dbpatches SET isapplied=1 WHERE patchid = 531;