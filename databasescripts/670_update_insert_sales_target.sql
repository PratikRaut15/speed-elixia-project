...
VALUES (
'670', '2019-02-20 13:00:00', 'Manasvi Thakur','Update insert sales target SP', '0');
...
CREATE TABLE `sales_target_details` (
  ...
  `target_set_for_month` tinyint(2) NOT NULL,
  `target_set_for_year` tinyint(4) NOT NULL,
...
CREATE TRIGGER `after_invoice_INSERT`
 ...
 BEGIN
        ...
        SET @istDateTime    = CONVERT_TZ(@serverTime, 'SYSTEM', '+05:30');
            ...
            WHERE MONTH(NEW.paymentdate) =   target_set_for_month
            AND YEAR(NEW.paymentdate) = target_set_for_year
