...
BEGIN
 ...
 DECLARE limitCondition VARCHAR(10);
		...
		SET @STMT = CONCAT("SELECT fat.*,f.fencename,u.realname,v.vehicleno FROM fenceman_audit_trail fat LEFT JOIN `fence` f on f.fenceid = fat.fenceid INNER JOIN vehicle v on v.vehicleid = fat.vehicleid LEFT JOIN user u on u.userid = fat.insertedBy WHERE fat.customerno =", customernoParam, " AND fat.vehicleid =", vehicleIdParam," AND date(fat.insertedOn) BETWEEN '",startdateParam,"' AND '",enddateParam,"' AND f.fencename IS NOT NULL ORDER BY fat.fm_auditId desc  ",limitCondition);
        PREPARE S FROM @STMT;
		EXECUTE S;
		DEALLOCATE PREPARE S;
...
CREATE TRIGGER `after_vehicle_update` AFTER UPDATE ON vehicle FOR EACH ROW BEGIN
    BEGIN
        ...
        (NEW.fuelcapacity <> OLD.fuelcapacity) OR (NEW.average <> OLD.average) OR(NEW.isdeleted <> OLD.isdeleted) THEN
...
CREATE TRIGGER `after_checkpointmanage_insert` AFTER INSERT ON `checkpointmanage`
 FOR EACH ROW BEGIN
	BEGIN
		INSERT INTO checkpointmanage_audit_trail
		SET
...
CREATE TRIGGER `after_fenceman_insert` AFTER INSERT ON `fenceman`
 FOR EACH ROW BEGIN
	BEGIN
		INSERT INTO fenceman_audit_trail
		SET
...
UPDATE  dbpatches
SET     patchdate = '2019-05-23 15:01:00'
    ,isapplied =1
WHERE   patchid = 713;
