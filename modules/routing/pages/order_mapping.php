...
<div class='container-fluid'>
    ...
    if($todays_zs_orders['zoneZero']==0){
        ...
        echo "<a href='assign.php?id=3' style='color:#fff;'><button class='btn btn-danger' style='margin-bottom:5px;' >Zone Assigning Pending for {$todays_zs_orders['zoneZero']} order$s</button></a><br style='clear:both'/>";
    }
    ?>
    ...
    <table class="table" id='droptableid' >
        <thead>
            ...
            foreach($allSlots as $sid=>$sText){
                ...
                echo "<th id='slot$sid'>$sid ( ".date(speedConstants::DEFAULT_TIME, strtotime($sText['start_time']))." - ".date(speedConstants::DEFAULT_TIME, strtotime($sText['end_time']))." )</th>";
            }
            echo "</tr>";
            ?>
        </thead>
            ...
            foreach($allSlots as $sid=>$sText){
                ...
                if($this_order!=''){
                    foreach($this_order as $vehid=>$orderDet){
                        ...
                        $teText = implode('<br/>', $t);
                        $delstr="";
                        $delboyname = $dm->get_deliveryboyname($vehid);
                        if(!empty($delboyname)){
                            $delstr = "(".$delboyname.")";
                        }

                        //$text .= "Vehicle No: {$sO['vehno']}{$delstr}===><br/>$teText<br/>";
                        $text .= "{$delstr}===><br/>$teText<br/>";
                    }
                }
