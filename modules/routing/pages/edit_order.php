...
$deliveryboy = $dm->getdeliveryboy();


$deliveryboy = array_filter($deliveryboy);
?>
<style type="text/css">
...
<div class='container' >
    <center>
    <form id="editorders" method="POST"  onsubmit="editOrders();" enctype='application/json'>
    <table class='table table-condensed'>
        ...
        <tbody>
            ...
            <td class='frmlblTd'>Order Date</td><td>
             ...
             <tr><td class='frmlblTd'>Total Amount</td><td>
                    <input type="text" name="total_amount" value="<?php echo $details->total_amount;?>" id="total_amount"></td>
            <td class='frmlblTd'>Redeem Limit</td><td>
                <input type="text" name="redeem_limit" value="<?php echo $details->reedeem_limit;?>" id="redeem_limit" ></td>
            </tr>
            <tr>
                <td class='frmlblTd'>Customer Name</td><td>
                <input type="text" name="customername" value="<?php echo $details->cust_name;?>" id="customername" ></td>
            </tr>
            <tr>
            ...
            <td>
                <input type="hidden" name="orderid" id="orderid" value="<?php echo $details->id;?>"/>
                ...
                <select name="hstatus" id="hstatus" onchange="changelist();">
                   ...
                   if(isset($getstatus))
                   {
                       ...
                       {
                       ?>
                    <option value="<?php echo $status->statusid;?>" <?php if($details->status == $status->statusid) { echo "selected";  } ?>><?php echo $status->status;?></option>
                        <?php
                        }
                   }
                   ?>
        ...
        <tr><td class='frmlblTd'>Delivery Boy</td>
            <td>
            <select name="delboyid" id="delboyid">
                <option value="0">Select Delivery Boy</option>
                <?php
                if(isset($deliveryboy) && !empty($deliveryboy)){
                    foreach($deliveryboy as $del){
                        ?>
                    <option value="<?php echo $del->pid;?>" <?php if(isset($details->deliveryboyid)){ if($del->pid==$details->deliveryboyid){ echo "selected"; } }?> ><?php echo $del->name;?></option>
                    <?php
                    }
                }
                ?>
