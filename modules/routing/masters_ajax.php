...
if($of=='zone'){
    ...
    $columns = array(
        ...
        //array('db' => 'a.entry_date','dt'=> 2),
        //array('db'=>'a.update_date','dt'=>3),
    );
    ...
    $columns = array(
       ...
       array('db' => 'a.entry_date', 'dt'=>5,
        ...
        array('db' => 'area_master_id', 'dt'=>5, 'formatter' => function($d, $row){
            return "<a href='assign.php?id=10&oid=$d'><img src='../../images/edit.png' width='16px;' height='16px;' alt='Edit' title='Edit'  /></a>";
        }),
    );
    if(isset($_SESSION['groupid']) && $_SESSION['groupid']!= 0){
        $grpid = (int)$_SESSION['groupid'];
...
elseif($of=='slot'){
    ...
    $columns = array(
        ...
        array('db' => 'slot_id', 'dt'=>5, 'formatter' => function($d, $row){
            return "<a href='assign.php?id=19&oid=$d'><img src='../../images/edit.png' width='16px;' height='16px;' alt='Edit' title='Edit'  /></a>";
        }),
    );
    if(isset($_SESSION['groupid']) && $_SESSION['groupid']!= 0){
        $grpid = (int)$_SESSION['groupid'];
