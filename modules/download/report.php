...
elseif ($report == 'realtimedata') {
    ...
    $umanager->insert_viewed_report($view_data);
} elseif ($report == 'secsummary') {
    //firstcall last call data
    ob_start();
    ...
    $umanager->insert_viewed_report($view_data);
} elseif ($report == 'inactive') {
    ob_start();
    require_once 'activeInactiveVehicle.php';
    $umanager->insert_viewed_report($view_data);
} elseif ($report == 'grp_temperature') {
    $group_id = isset($_GET['g']) ? (int) $_GET['g'] : exit('Group id not found');
    ob_start();
    require_once 'temperature.php';
    $view_data['vehicleid'] = 0;
    $umanager->insert_viewed_report($view_data);
} else {
