...
class Sales extends DatabaseSalesManager {
    ...
    public function getsupervisors_byasm($asmid, $customerno) {
        $salesdata = array();
        $Query = sprintf("SELECT userid,role,roleid,email,realname,username,phone,heirarchy_id FROM " . SPEEDDB . ".`user` where isdeleted=0 AND customerno=" . $customerno . " AND heirarchy_id=" . $asmid . " order by realname asc");
        $this->executeQuery($Query);
        if ($this->get_rowCount() > 0) {
            while ($row = $this->get_nextRow()) {
                $salesuser = new stdClass();
    ...
    public function get_sr_by_supervisors($supids, $customerno, $all = NULL) {
        if ($all == "ALL") {
            $salesdata = null;
            $Query = sprintf("SELECT userid,role,roleid,email,realname,username,phone,heirarchy_id FROM " . SPEEDDB . ".`user` where role='sales_representative' AND isdeleted=0 AND customerno=" . $customerno . " order by realname asc");
            $this->executeQuery($Query);
            if ($this->get_rowCount() > 0) {
                while ($row = $this->get_nextRow()) {
                    $salesuser = new stdClass();
    ...
    public function get_secondary_product_details($orderid) {
        $salesdata = null;
        $Query = sprintf("SELECT sop.skuid,st.styleno,st.franchiseId,c.categoryid,c.categoryname,sop.quantity,sop.status,sop.soid,sop.sopid, f.franchiseName, sop.packs,sop.macro,sop.micro,sop.skuTotalPrice,sop.isPromo FROM secondary_order_productlist as sop"
            . "  left join style as st on st.styleid = sop.skuid left join category as c on c.categoryid = st.categoryid
                left join franchise as f on f.franchiseId = st.franchiseId
                  where sop.soid = " . $orderid . " AND sop.isdeleted=0 AND sop.customerno=" . $this->customerno);
        $this->executeQuery($Query);
        if ($this->get_rowCount() > 0) {
            while ($row = $this->get_nextRow()) {
                $salesuser = new stdClass();
                ...
                $salesuser->franchiseId = $row['franchiseId'];
                $salesuser->franchiseName = $row['franchiseName'];
                $salesuser->packs = $row['packs'];
                $salesuser->macro = $row['macro'];
                $salesuser->micro = $row['micro'];
                $salesuser->skuTotalPrice = $row['skuTotalPrice'];
                $salesuser->isPromo = $row['isPromo'];
                $salesdata[] = $salesuser;
            }
        }
        return $salesdata;
    ...
    public function insert_styledata1($styleno, $category, $mrp, $distprice, $retprice, $comselprince, $carton) {
        $Query = "Insert into style (customerno,styleno,categoryid,mrp,distprice,retailprice,companysellingprice,carton,entrytime,addedby) VALUES ($this->customerno,'%s','%s','%s','%s','%s','%s','%s','$this->today',$this->userid)";
        $SQL = sprintf($Query, Sanitise::String($styleno), Sanitise::String($category), Sanitise::Float($mrp), Sanitise::Float($distprice), Sanitise::Float($retprice), Sanitise::Float($comselprince), Sanitise::String($carton)
        );
        $this->executeQuery($SQL);
        $lastid = $this->get_insertedId();
        return $lastid;
    }

    public function update_styledata($skuid, $styleno, $category, $mrp, $distprice, $retprice, $carton, $comselprince) {
        $Query = "update style set styleno='" . $styleno . "',categoryid=" . $category . ",carton=" . $carton . ",styleno = '" . $styleno . "',mrp='" . $mrp . "',distprice='" . $distprice . "',retailprice='" . $retprice . "',companysellingprice='" . $comselprince . "',updatedtime='" . $this->today . "', updated_by='" . $this->userid . "' where styleid=" . $skuid;
        $SQL = sprintf($Query);
        $this->executeQuery($SQL);
    }

    public function skuimagefilenameupdate($filename, $skuid) {
        $filename = trim($filename);
        $Query = "update style set productimage='" . $filename . "' where styleid=" . $skuid;
        $SQL = sprintf($Query);
        $this->executeQuery($SQL);
    }

    public function delete_styledata($skuid) {
        $Query = "update `style` set `isdeleted`=1 where `styleid`=" . $skuid;
        $SQL = sprintf($Query);
        $this->executeQuery($SQL);
    }
    ...
    public function get_shop_api($saleid, $areaid) {
        ...
        $Query = "SELECT s.shopid,s.salesid,a.areaname,st.shop_type,s.shoptypeid,s.phone,s.phone2,s.dob,s.owner,s.owner_shop,s.address,s.emailid,s.shopname,s.areaid,s.sequence_no, s.salesid,s.distributorid FROM shop as s left join shoptype as st on st.shid = s.shoptypeid left join area as a on a.areaid = s.areaid WHERE s.areaid = " . $areaid . " AND s.salesid = " . $saleid . " AND s.customerno = " . $this->customerno . " AND s.isdeleted=0 ORDER BY s.sequence_no ASC";
        $SQL = sprintf($Query);
        $this->executeQuery($SQL);
        if ($this->get_rowCount() > 0) {
            while ($row = $this->get_nextRow()) {
                ...
                if (!empty($shopid)) {
                    $shopview = new stdClass();
                    $shopview->shopid = isset($row['shopid']) ? utf8_encode($row['shopid']) : "";
                    $shopview->shopname = isset($row['shopname']) ? utf8_encode($row['shopname']) : "";
                    $shopview->areaid = isset($row['areaid']) ? utf8_encode($row['areaid']) : "";
                    $shopview->salesid = isset($row['salesid']) ? utf8_encode($row['salesid']) : "";
                    $shopview->distributorid = isset($row['distributorid']) ? utf8_encode($row['distributorid']) : "";
                    $shopview->shoptypeid = isset($row['shoptypeid']) ? utf8_encode($row['shoptypeid']) : "";
                    $shopview->sequence_no = isset($row['sequence_no']) ? utf8_encode($row['sequence_no']) : "";
                    $shopview->areaname = isset($row['areaname']) ? utf8_encode($row['areaname']) : "";
                    $shopview->shop_type = isset($row['shop_type']) ? utf8_encode($row['shop_type']) : "";
                    $shopview->phone = isset($row['phone']) ? utf8_encode($row['phone']) : "";
                    $shopview->phone2 = isset($row['phone2']) ? utf8_encode($row['phone2']) : "";
                    $shopview->dob = isset($row['dob']) ? utf8_encode($row['dob']) : "";
                    $shopview->owner = isset($row['owner']) ? utf8_encode($row['owner']) : "";
                    $shopview->owner_shop = isset($row['owner_shop']) ? utf8_encode($row['owner_shop']) : "";
                    $shopview->owner = isset($row['owner']) ? utf8_encode($row['owner']) : "";
                    $shopview->address = isset($row['address']) ? utf8_encode($row['address']) : "";
                    $shopview->emailid = isset($row['emailid']) ? utf8_encode($row['emailid']) : "";
                    $shopview->address = isset($row['address']) ? utf8_encode($row['address']) : "";
                    $shopview->is_entryexists = $this->check_entryexists($row['salesid'], $row['shopid']);
                    $getshopdata[] = $shopview;
                }
            }
            return $getshopdata;
        }
    ...
    public function get_shop_api_count($saleid, $areaid) {
        $getshopdata = array();
        $shopid = "";
        $isentryexists = 0;
        $todaydate = date("Y-m-d", strtotime($this->today));
        $Query = "SELECT s.shopid FROM shop as s WHERE s.areaid = " . $areaid . " AND s.salesid = " . $saleid . " AND s.customerno = " . $this->customerno . " AND s.isdeleted=0 ORDER BY s.sequence_no ASC";
        $SQL = sprintf($Query);
        $this->executeQuery($SQL);
        if ($this->get_rowCount() > 0) {
            while ($row = $this->get_nextRow()) {
    ...
    public function get_shop_api_sync($srid, $distid, $timelastsync) {
        $getshopdata = array();
        $shopid = "";
        $isentryexists = 0;
        $todaydate = date("Y-m-d", strtotime($this->today));
        $Query = " select sh.shopid,sh.salesid,a.areaname,st.shop_type,sh.shoptypeid,sh.phone,sh.phone2,sh.dob,sh.owner,sh.owner_shop,sh.address,sh.emailid,sh.shopname,sh.areaid,sh.sequence_no, sh.salesid,sh.distributorid,sd.operation_type,sd.syncid
            from syncdata as sd
                inner join shop as sh ON sd.rowid = sh.shopid
                left join shoptype as st on st.shid = sh.shoptypeid
                left join area as a on a.areaid = sh.areaid "
            . "  WHERE sd.table_type=0 AND sd.srid= " . $srid . " AND sh.distributorid =" . $distid . " AND sd.timestamp >'" . $timelastsync . "'";
        $SQL = sprintf($Query);
        $this->executeQuery($SQL);
        if ($this->get_rowCount() > 0) {
            while ($row = $this->get_nextRow()) {
                ...
                if (!empty($shopid)) {
                    ...
                    $shopview->address = isset($row['address']) ? utf8_encode($row['address']) : "";
                    $shopview->is_entryexists = $this->check_entryexists($row['salesid'], $row['shopid']);
                    $shopview->operation_type = isset($row['operation_type']) ? utf8_encode($row['operation_type']) : "0";
                    $shopview->syncid = isset($row['syncid']) ? utf8_encode($row['syncid']) : "0";
                    $getshopdata[] = $shopview;
                }
            }
            return $getshopdata;
    ...
    public function get_sku_api() {
        $getskudata = array();
        $Query = "SELECT styleid,carton as cartonperqty,productimage,styleno,categoryid,mrp,distprice,retailprice,carton FROM style WHERE customerno = $this->customerno AND isdeleted=0 ";
        $SQL = sprintf($Query);
        $this->executeQuery($SQL);
        if ($this->get_rowCount() > 0) {
            while ($row = $this->get_nextRow()) {
                $getskudata[] = array(
                    ...
                    'cartonperqty' => utf8_encode($row['cartonperqty']),
                    'productimage' => utf8_encode($row['productimage']),
                    'imagelink' => $this->geturl($row['productimage'])
                );
            }
            return $getskudata;
        }
    ...
    public function geturl($imagename) {
        //$serverPath = "http://192.168.1.108/elixiaspeed_test/";
        $serverPath = "http://speed.elixiatech.com/";
        $sendurl = '';
        $filename = '../../customer/' . $this->customerno . '/skuphotos/' . $imagename;
        $filepath = $serverPath . "customer/$this->customerno/skuphotos/" . $imagename;
        if (file_exists($filename) && $imagename != "") {
            $sendurl = $filepath;
        }
        return $sendurl;
    }
    ...
    public function get_area_api_sr($distid) {
        $distidin = "";
        if (is_array($distid)) {
            $distid = implode(",", $distid);
            $distidin = " a.distributorid IN (" . $distid . ") AND ";
        } elseif ($distid != "") {
            $distidin = "a.distributorid IN (" . $distid . ") AND ";
        }
        $getareadata = array();
        $Query = "SELECT u.heirarchy_id,a.areaid,a.areaname,a.distributorid FROM  area as a  inner join " . SPEEDDB . ".`user` as u  ON a.distributorid = u.`userid`  WHERE  $distidin a.customerno = $this->customerno AND a.isdeleted=0 ";
        $SQL = sprintf($Query);
        $this->executeQuery($SQL);
        if ($this->get_rowCount() > 0) {
            while ($row = $this->get_nextRow()) {
                $getareadata[] = array(
                    ...
                    'areaname' => utf8_encode($row['areaname']),
                    'distributorid' => utf8_encode($row['distributorid']),
                    'srid' => utf8_encode($row['heirarchy_id'])
                );
            }
            return $getareadata;
        }
    ...
    public function get_area_api_sync($distid, $timelastsync, $srid) {
        $distidin = "";
        if (is_array($distid)) {
            $distid = implode(",", $distid);
            $distidin = " AND  a.distributorid IN (" . $distid . ")  ";
        } elseif ($distid != "") {
            $distidin = " AND a.distributorid IN (" . $distid . ")  ";
        }
        $getareadata = array();
        $Queryarea = "SELECT a.areaid,a.areaname, a.distributorid,sd.operation_type,sd.syncid FROM syncdata as sd inner join area as a ON sd.rowid = a.areaid  WHERE sd.table_type=1 AND sd.srid= " . $srid . " AND sd.timestamp >'" . $timelastsync . "' $distidin ";
        $SQLarea = sprintf($Queryarea);
        $this->executeQuery($SQLarea);
        if ($this->get_rowCount() > 0) {
            while ($row = $this->get_nextRow()) {
                $getareadata[] = array(
                    'areaid' => utf8_encode($row['areaid']),
                    'areaname' => utf8_encode($row['areaname']),
                    'distributorid' => utf8_encode($row['distributorid']),
                    'operation_type' => utf8_encode($row['operation_type']),
                    'syncid' => utf8_encode($row['syncid'])
                );
            }
            return $getareadata;
        }
...
//    public function get_distributor() {
    ...
    public function add_areadata($distid, $areaname) {
        ...
        $this->executeQuery($SQL);
        $areaid = $this->get_insertedId();

        $Query1 = "SELECT heirarchy_id FROM user WHERE customerno=$this->customerno AND userid=" . $distid . " AND isdeleted=0";
        $SQL1 = sprintf($Query, Sanitise::String($distid));
        $this->executeQuery($SQL1);
        if ($this->get_rowCount() > 0) {
            while ($row1 = $this->get_nextRow()) {
                $srid = $row1['heirarchy_id'];
            }
        }

        //insert-1, update=2, delete =3
        $Querysync = "Insert into syncdata (operation_type,table_type,rowid,srid,timestamp)VALUES(%d,%d,%d,%d,'%s')";
        $SQL2 = sprintf($Querysync, 1, 1, $areaid, $srid, $this->today);
        $this->executeQuery($SQL2);
    }
    ...
    public function update_areadata($areaid, $distid, $areaname) {
        ...
        $this->executeQuery($SQL);
        $salesid = 0;
        $Query1 = "SELECT heirarchy_id FROM " . SPEEDDB . ".`user` WHERE customerno=$this->customerno AND userid=%d AND isdeleted=0";
        $SQL1 = sprintf($Query1, (int) $distid);
        $this->executeQuery($SQL1);
        if ($this->get_rowCount() > 0) {
            while ($row1 = $this->get_nextRow()) {
                $salesid = $row1['heirarchy_id'];
            }
        }
        //insert-1 update-2 delete-3 //table type - shops-0 area-1
        $Query = "SELECT * FROM syncdata where rowid=" . $areaid . " AND table_type=1";
        $SQL = sprintf($Query);
        $this->executeQuery($SQL);
        if ($this->get_rowCount() > 0) {
            while ($row = $this->get_nextRow()) {
            ...
            //insert-1, update=2, delete =3
            $Queryupdate = "update syncdata set timestamp='" . $this->today . "', operation_type=2, srid=" . $salesid . ", table_type=1 where syncid = " . $syncid . " AND rowid=" . $rowid;
            $SQL1 = sprintf($Queryupdate);
            $this->executeQuery($SQL1);
        } else {
            $Querysync = "Insert into syncdata (operation_type,table_type,rowid,srid,timestamp)VALUES(%d,%d,%d,%d,'%s')";
            $SQL2 = sprintf($Querysync, 2, 1, $areaid, $salesid, $this->today);
            $this->executeQuery($SQL2);
        }
    }
    ...
    public function delete_areadata($id) {
        ...
        $SQL = sprintf($Query);
        $this->executeQuery($SQL);
        $Query1 = "select u.heirarchy_id from area as a inner join speed.`user` as u ON u.userid = a.distributorid where  a.`areaid`=" . $id;
        ...
        //insert-1 update-2 delete-3 //table type - shops-0 area-1
        $Query = "SELECT * FROM syncdata where rowid=" . $id . " AND table_type=1";
        $SQL = sprintf($Query);
        $this->executeQuery($SQL);
        if ($this->get_rowCount() > 0) {
            while ($row = $this->get_nextRow()) {
            ...
            $Queryupdate = "update syncdata set timestamp='" . $this->today . "', operation_type=3, srid=" . $srid . ", table_type=1 where syncid = " . $syncid . " AND rowid=" . $rowid;
            $SQL = sprintf($Queryupdate);
            $this->executeQuery($SQL);
        } else {
            $Querysync = "Insert into syncdata (operation_type,table_type,rowid,srid,timestamp)VALUES(%d,%d,%d,%d,'%s')";
            $SQL2 = sprintf($Querysync, 3, 1, $id, $srid, $this->today);
            $this->executeQuery($SQL2);
        }
    }
     ...
     * @return boolean
     */
    public function is_shopname_exists($shopname, $areaid) {
        $Query = "SELECT shopname FROM shop WHERE customerno=$this->customerno AND shopname='%s' AND areaid='%s' AND isdeleted=0";
        $SQL = sprintf($Query, Sanitise::String($shopname), Sanitise::String($areaid));
        $this->executeQuery($SQL);
        if ($this->get_rowCount() > 0) {
            return true;
        } else {
...
//      Shop sequnecing as per area
        ...
        //also insert in synctable
        //insert=1 update=2 delete=3
        //type shops-0 / area-1
        $Querysync = "Insert into syncdata (operation_type,table_type,rowid,srid,timestamp)VALUES(%d,%d,%d,%d,'%s')";
        $SQL2 = sprintf($Querysync, 1, 0, $shopid, $saleid, $this->today);
        $this->executeQuery($SQL2);
    }

    public function add_shopdata_api($shopObj) {
        $seq = 0;
        $owner_shop = $shopObj->owner . '-' . $shopObj->shopname;
        $dob = date('Y-m-d', strtotime($shopObj->dob));
        // $dob = date('Y-m-d', strtotime($shopObj->dob1));

        if ($shopObj->status == 1) {
            $androidshopid1 = $shopObj->androidshopid;
        } else {
            $androidshopid1 = 0;
        }
        $newseq = 1;
        if ($shopObj->prior_shopid != 0) {
//      Shop sequnecing as per area
            $seqdata = $this->getseqid_byshopid($shopObj->prior_shopid);
            $preshopid = $seqdata[0]['shopid'];
            $preareaid = $seqdata[0]['areaid'];
            $preseq = $seqdata[0]['sequence_no'];
            $presalesid = $seqdata[0]['salesid'];
            $newseq = $preseq + 1;
            $old_shopdata_area = $this->getshopdata_byarea($shopObj->areaid);
        ...
        $SQL = sprintf($Query, (int) $androidshopid1, (int) $shopObj->distid, Sanitise::string($dob), (int) $shopObj->srid, (int) $shopObj->areaid, Sanitise::String($shopObj->shopname), $shopObj->shoptype, Sanitise::String($shopObj->sphoneno), Sanitise::String($shopObj->sphoneno2), Sanitise::String($shopObj->owner), Sanitise::String($owner_shop), Sanitise::String($shopObj->saddress), Sanitise::String($shopObj->semail), (int) $shopObj->goods_avail, (int) $shopObj->status, (int) $newseq);
        $this->executeQuery($SQL);
        $id = $this->get_insertedId();
        $lastid = $id;

        $sql = "SELECT * from " . SPEEDDB . ".user WHERE userid = '$shopObj->srid' AND isdeleted = 0 limit 1";
        $result = $this->executeQuery($sql);
        if ($shopObj->signature != NULL || $shopObj->photo != NULL) {
            if ($this->get_rowCount() > 0) {
                while ($row = $this->get_nextRow()) {
                ...
                if ($shopObj->signature != NULL) {
                    $image = base64_decode($shopObj->signature);
                    file_put_contents("../../customer/" . $customerno . "/secsales/signature/" . $id . ".jpg", $image);
                }

                if ($shopObj->photo != NULL) {
                    $image1 = base64_decode($shopObj->photo);
        ...
        if ($shopObj->status == 1) {
            $Querysync = "Insert into syncdata (operation_type,table_type,rowid,srid,timestamp)VALUES(%d,%d,%d,%d,'%s')";
            $SQL2 = sprintf($Querysync, 1, 0, $lastid, $shopObj->srid, $this->today);
            $this->executeQuery($SQL2);
        } else {
            $Querysync = "Insert into syncdata (operation_type,table_type,rowid,srid,timestamp)VALUES(%d,%d,%d,%d,'%s')";
            $SQL2 = sprintf($Querysync, 1, 0, $lastid, $shopObj->srid, $this->today);
    ...
    public function update_shopdata($shoptypeid, $sid, $distid, $saleid, $areaid, $shopname, $sphoneno, $sphoneno2, $owner, $saddress, $semail, $dob1) {
        ...
        $this->executeQuery($SQL);
        //insert -1 update-2 delete-3 //table type - shops-0 area-1
        $Query = "SELECT * FROM syncdata where rowid=" . $sid . " AND table_type=0";
        $SQL = sprintf($Query);
        $this->executeQuery($SQL);
        if ($this->get_rowCount() > 0) {
            while ($row = $this->get_nextRow()) {
            ...
            $Queryupdate = "update syncdata set timestamp ='" . $this->today . "',srid=" . $saleid . ", operation_type=2, table_type=0 where syncid = " . $syncid . " AND shopid=" . $rowid;
            $SQL = sprintf($Query);
            $this->executeQuery($Queryupdate);
        } else {
            $Querysync = "Insert into syncdata (operation_type,table_type,rowid,srid,timestamp)VALUES(%d,%d,%d,%d,'%s')";
            $SQL2 = sprintf($Querysync, 2, 0, $sid, $saleid, $this->today);
            $this->executeQuery($SQL2);
        }
    }
    ...
    public function delete_shopdata($id) {
        ...
        $Query = "SELECT shopid,salesid FROM shop where shopid=" . $id;
        $SQL = sprintf($Query);
        $this->executeQuery($SQL);
        if ($this->get_rowCount() > 0) {
            while ($row = $this->get_nextRow()) {
        ...
        //insert -1 update-2 delete-3 //table type - shops-0 area-1
        $Query = "SELECT * FROM syncdata where rowid=" . $id . " AND table_type=0";
        $SQL = sprintf($Query);
        $this->executeQuery($SQL);
        if ($this->get_rowCount() > 0) {
            while ($row = $this->get_nextRow()) {
                $syncid = $row['syncid'];
                $rowid = $row['rowid'];
                $Queryupdate = "update syncdata set srid=" . $srid . ", timestamp ='" . $this->today . "', operation_type=3, table_type=0 where rowid=" . $sid;
                $SQL = sprintf($Query);
                $this->executeQuery($Queryupdate);
            }
        } else {
            $Querysync = "Insert into syncdata (operation_type,table_type,rowid,srid,timestamp)VALUES(%d,%d,%d,%d,'%s')";
            $SQL2 = sprintf($Querysync, 3, 0, $id, $srid, $this->today);
            $this->executeQuery($SQL2);
        }
    }
    ...
    public function get_styleedit($skuid) {
        ...
        if ($this->get_rowCount() > 0) {
            while ($row = $this->get_nextRow()) {
                ...
                $styedit->productimage = $row['productimage'];
                $styedit->imagelink = $this->geturl($row['productimage']);
                $styedit->companysellingprice = $row['companysellingprice'];
//$support1->timestamp = date(speedConstants::DEFAULT_DATETIME, strtotime($row['create_on_date']));
...
//$support1->timestamp = date(speedConstants::DEFAULT_DATETIME, strtotime($row['create_on_date']));
    ...
    public function getdistributordata_api($saleid = NULL) {
        ...
        if ($this->get_rowCount() > 0) {
            while ($row = $this->get_nextRow()) {
                ...
                $salesuser->salesid = $row['heirarchy_id'];
                $salesdata[] = $salesuser;
            }
            return $salesdata;
        }
    ...
    public function getdistributor_api($saleid = NULL, $distid) {
        $salesdata = null;
        $Query = sprintf("SELECT userid,role,roleid,email,realname,username,phone,heirarchy_id FROM " . SPEEDDB . ".`user` where isdeleted=0 AND customerno=" . $this->customerno . " AND role = 'Distributor' AND userid=" . $distid . " AND heirarchy_id=" . $saleid);
        $this->executeQuery($Query);
        if ($this->get_rowCount() > 0) {
            while ($row = $this->get_nextRow()) {
                $salesuser = new stdClass();
...
//$support1->timestamp = date(speedConstants::DEFAULT_DATETIME, strtotime($row['create_on_date']));
    ...
    public function getskuid($catid) {
        $catids = array();
        if ($catid == 1) {
            $Query = sprintf("SELECT categoryid from category where isdeleted=0 AND categoryid<>1");
            $this->executeQuery($Query);
            if ($this->get_rowCount() > 0) {
                while ($row = $this->get_nextRow()) {
        ...
        $styleviewdata = array();
        $Query = "SELECT * FROM style WHERE customerno=$this->customerno AND categoryid IN (" . $catid . ") AND isdeleted=0";
        $SQL = sprintf($Query);
        $this->executeQuery($SQL);
        if ($this->get_rowCount() > 0) {
            while ($row = $this->get_nextRow()) {
    ...
    // public function add_secondarysalesdata_api($role, $shopid, $skudata, $orderdate, $is_deadstock, $reason, $discount, $distid, $srid, $ordertypeid, $deliverydatetime) {
    public function add_secondarysalesdata_api($orderObj) {
        //$orderdate = date("Y-m-d h:i:s", strtotime($orderdate));
        //echo $orderdate ; die();
        $is_asm = 0;
        if ($orderObj->role == "sales_representative") {
            $is_asm = 0;
        }
        if ($orderObj->role == "ASM") {
            $is_asm = 2;
        }
        if ($orderObj->role == "Supervisor") {
            $is_asm = 1;
        }
        if ($orderObj->is_deadstock == 1) {
            //deadstock insert
            $Query = "INSERT INTO deadstock (customerno,shopid,ordertypeid,stockdate,is_asm,addedby,entrytime,reason,srid) VALUES($this->customerno,%d,%d,'%s',%d,%d,'%s','%s',%d)";
            $SQL = sprintf($Query, (int) $orderObj->shopid, (int) $orderObj->otId, (string) $orderObj->orderdate, $is_asm, $this->userid, $this->today, $orderObj->reason, $orderObj->srid);

            $this->executeQuery($SQL);
            $id = $this->get_insertedId();

            if (isset($orderObj->skudata)) {
                foreach ($orderObj->skudata as $row) {
        ...
        } elseif ($orderObj->is_deadstock == 0) {
            //add secondary sales
            $Query = "INSERT INTO secondary_order (customerno,shopid, ordertypeid,orderdate,entrydate,srid,is_asm,addedby,is_deadstock, discount, deliveryDateTime,orderAmt,pendingAmt) VALUES($this->customerno,%d,%d,'%s','%s',%d,%d,%d,%d,%d,'%s','%s','%s')";
            $SQL = sprintf($Query, (int) $orderObj->shopid, (int) $orderObj->otId, (string) $orderObj->orderdate, (string) $this->today, $orderObj->srid, $is_asm, $this->userid, $orderObj->is_deadstock, $orderObj->discount, $orderObj->deliverydatetime, $orderObj->orderAmt, $orderObj->pendingAmt);
            $this->executeQuery($SQL);
            $id = $this->get_insertedId();

            //update_inventory_secondarysales_qty
            $invskulist = array();
            $getinventorylist = $this->get_inventory_list($orderObj->distid, $this->userid);
            if (isset($getinventorylist) && !empty($getinventorylist)) {
                for ($i = 0; $i < count($getinventorylist); $i++) {
                    $invskulist[] = $getinventorylist[$i]->styleid;
                }
            }
            ...
            if (isset($orderObj->skudata)) {
                foreach ($orderObj->skudata as $row) {
                    if (in_array($row['skuid'], $invskulist)) {
                        $this->update_inventory_secondarysales_qty($orderObj->distid, $row['skuid'], $row['quantity']);
                    } else {
                        $this->insert_inventory_sec_qty($orderObj->distid, $row['skuid'], $row['quantity'], $is_asm);
                    }

                    $franchiseId = isset($row['franchiseId']) ? $row['franchiseId'] : 0;
                    $macro = isset($row['macro']) ? $row['macro'] : 0;
                    $packs = isset($row['packs']) ? $row['packs'] : 0;
                    $micro = isset($row['micro']) ? $row['micro'] : 0;
                    $skuTotalPrice = isset($row['skuTotalPrice']) ? $row['skuTotalPrice'] : 0;

                    $Query = "INSERT INTO secondary_order_productlist (customerno,soid,skuid,quantity,status,entrydate,franchiseId,macro,packs,micro,skuTotalPrice, categoryid) VALUES($this->customerno,%d,%d,%d,%d,'%s','%s','%s','%s','%s','%s',%d)";
                    $SQL = sprintf($Query, $id, $row['skuid'], $row['quantity'], $row['status'], (string) $this->today, $franchiseId, $macro, $packs, $micro, $skuTotalPrice, $row['categoryid']);
                    $this->executeQuery($SQL);

                    if ($this->customerno == 698) {
                        $this->updateOrderStatus($id, $orderObj->distid, $row['skuid'], $row['quantity'], $this->userid, $this->customerno);
                    }

                    if (isset($row['offers']) && !empty($row['offers'])) {
                        $this->insertOfferedProducts($id, $row['offers'], $orderObj->distid, $franchiseId, $orderObj->shopid);
                    }
                }
            }
        }
    ...
    function insertOfferedProducts($orderId, $Offers, $distributorId, $franchiseId, $shopId) {
        //update_inventory_secondarysales_qty

        $objProduct = new stdClass();
        $objProduct->productId = $Offers['promoProductId'];
        $objProduct->userId = $this->userid;
        $objProduct->customerNo = $this->customerno;
        $objProduct->shopId = $shopId;
        $objProduct->franchiseId = $franchiseId;

        $arrProduct = $this->getProductList($objProduct);
        if (isset($arrProduct) && !empty($arrProduct)) {
            $invskulist = array();
            $getinventorylist = $this->get_inventory_list($distributorId, $this->userid);
            if (isset($getinventorylist) && !empty($getinventorylist)) {
                for ($i = 0; $i < count($getinventorylist); $i++) {
                    $invskulist[] = $getinventorylist[$i]->styleid;
                }
            ...
            if (in_array($Offers['promoProductId'], $invskulist)) {
                $this->update_inventory_secondarysales_qty($distributorId, $Offers['promoProductId'], $Offers['promoQty']);
            } else {
                $this->insert_inventory_sec_qty($distributorId, $Offers['promoProductId'], $Offers['promoQty'], $is_asm);
            }

            $categoryId = $arrProduct[0]['categoryId'];

            $item_macro = $arrProduct[0]['macro'];
            $item_micro = $arrProduct[0]['micro'];
            $item_packs = $arrProduct[0]['packs'];

            $packs = $Offers['promoQty'];

            $micro = $Offers['promoQty'] * $item_micro;
            $macro = (double) $Offers['promoQty'] / ($item_packs * $item_macro);

            $status = 0;

            /* Calculate Sku Price */
            $dec = $this->percentToDecimal($Offers['discount']);
            $fact = $arrProduct[0]['mrp'] * $dec;

            $skuTotalPrice = $micro * $fact;

            $Query = "INSERT INTO secondary_order_productlist (customerno,soid,skuid,quantity,status,entrydate,franchiseId,macro,packs,micro,skuTotalPrice, categoryid, isPromo) VALUES($this->customerno,%d,%d,%d,%d,'%s','%s','%s','%s','%s','%s',%d,%d)";
            $SQL = sprintf($Query, $orderId, $Offers['promoProductId'], $Offers['promoQty'], $status, (string) $this->today, $franchiseId, $macro, $packs, $micro, $skuTotalPrice, $categoryId, 1);
            $this->executeQuery($SQL);
        }
    }
    ...
    public function updateOrderStatus($orderId, $distributorId, $skuId, $quantity, $userId, $customerNo) {
        ...
        $sp_params = "'" . $orderId . "'"
            ...
            . ",'" . $customerNo . "'"
            . ",'" . $todaysdate . "'";
        $sqlCallSP = "CALL " . speedConstants::SP_UPDATE_ORDER_STATUS . "($sp_params)";
        $result = $pdo->query($sqlCallSP);
    }

    public function getOrderStatus($orderId, $customerNo) {
        $orderStatus = 0;
        $pdo = $this->CreatePDOConn();
        $SQL = "SELECT orderStatus from secondary_order WHERE soid = %d AND customerno = %d AND isdeleted = 0";
        $Query = sprintf($SQL, Validator::escapeCharacters($orderId), Validator::escapeCharacters($customerNo));
        $this->executeQuery($Query);
        if ($this->get_rowCount() > 0) {
            while ($row = $this->get_nextRow()) {
    ...
    public function add_orderdata($srcode, $shopid, $orderdate, $entrytime, $skudata, $distid) {
        $newDate = date("Y-m-d", strtotime($orderdate));
        $orderdate1 = $newDate . ' ' . $entrytime . ":00";
        $is_asm = 0;
        if ($_SESSION['role_modal'] == "sales_representative") {
    ...
    public function update_orderdata($orderid, $distid, $areaid, $shopid, $orderdate, $entrytime, $skudata, $orderStatus = null) {
        $newDate = date("Y-m-d", strtotime($orderdate));
        $orderdate1 = $newDate . ' ' . $entrytime . ":00";
        $is_asm = 0;
        if (isset($role) && $role == "sales_representative") {
            $is_asm = 0;
        }
        if (isset($role) && $role == "ASM") {
            $is_asm = 2;
        }
        if (isset($role) && $role == "Supervisor") {
            $is_asm = 1;
        }
        ...
        if (isset($orderStatus) && $orderStatus == 0) {
            $approveOrder = " , orderStatus= '" . $orderStatus . "'";
        }

        $Query = "Update secondary_order set orderdate='" . $orderdate1 . "',updatedtime='" . $this->today . "',updated_by=" . $this->userid . ", is_asm=" . $is_asm . " " . $approveOrder . " where soid = " . $orderid;
        $SQL = sprintf($Query);
        $this->executeQuery($SQL);

        if ($this->customerno == 698 && isset($orderStatus) && $orderStatus == 0) {
            $this->pushOrderToERP($orderid, $distid);
        }
    ...
    function getERPDetails($orderId) {
        $orderDetails = array();
        $Query = "SELECT soid, shopid, srid, deliverydatetime, user.erpUserToken
                FROM secondary_order
                INNER JOIN " . SPEEDDB . ".user on user.userid = secondary_order.addedby AND user.isdeleted = 0
                WHERE soid = %d AND orderStatus = 0 AND secondary_order.isdeleted = 0";
        $SQL = sprintf($Query, $orderId);
        $this->executeQuery($SQL);
        if ($this->get_rowCount() > 0) {
            while ($row = $this->get_nextRow()) {
                $orderDetails['orderId'] = $row['soid'];
                $orderDetails['shopId'] = $row['shopid'];
                $orderDetails['srId'] = $row['srid'];
                $orderDetails['deliveryDateTime'] = $row['deliverydatetime'];
                $orderDetails['erpUserToken'] = $row['erpUserToken'];
            }
        }
        return $orderDetails;
    }

    function getERPSKUDetails($orderId) {
        $orderSKUDetails = array();
        $Query = "SELECT sop.sopid, sop.soid, sop.categoryid, sop.skuid, sop.quantity, sop.franchiseId,sop.status, sop.entrydate
                FROM secondary_order_productlist sop
                INNER JOIN secondary_order so on so.soid = sop.soid
                WHERE sop.soid = %d AND so.orderStatus = 0 AND so.isdeleted = 0 and sop.isdeleted=0";
        $SQL = sprintf($Query, $orderId);
        $this->executeQuery($SQL);
        if ($this->get_rowCount() > 0) {
            while ($row = $this->get_nextRow()) {
                $sku = array();
                $sku['categoryid'] = $row['categoryid'];
                $sku['skuid'] = $row['skuid'];
                $sku['quantity'] = $row['quantity'];
                $sku['status'] = $row['status'];
                $sku['entrydate'] = $row['entrydate'];
                $sku['franchiseId'] = $row['franchiseId'];
                $orderSKUDetails[] = $sku;
            }
        }
    ...
    public function pushOrderToERP($orderId, $distributorid) {
        $orderDetails = $this->getERPDetails($orderId);

        if (isset($orderDetails) && !empty($orderDetails)) {
            $objCN = new stdClass();

            $skuData = $this->getERPSKUDetails($orderId);

            $objCN->manualCnNo = $orderId;
            $objCN->manualBookingNo = $orderId;

            $objCN->userkey = $orderDetails['erpUserToken'];
            $objCN->deliveryDatetime = $orderDetails['deliveryDateTime'];
            $objCN->moreProductDetails = $skuData;
            $objCN->consigneeSalesRefId = $orderDetails['shopId'];
            $objCN->clientSalesRefId = $orderDetails['shopId'];
            $objCN->isDriverApp = $orderDetails['srId'];
            $objCN->isClosed = 0;
            ...
            if (isset($this->customerno) && $this->customerno == 193) {
                $insertConsignorNote = "/insertConsignorNote";
            } elseif (isset($this->customerno) && $this->customerno == 698) {
    ...
    public function edit_secondarysalesdata_api($role, $skudata, $orderid, $userkeyid, $otId = Null, $deliverydatetime = Null) {
        $isUpdated = 0;
        $is_asm = 0;
        $deliverydatetime = (isset($deliverydatetime) AND $deliverydatetime != 0) ? $deliverydatetime : "";
        if ($role == "sales_representative") {
            $is_asm = 0;
        }
        if ($role == "ASM") {
        ...
        $Query = "Update secondary_order set updatedtime=
            '" . $this->today .
            "',updated_by=" . $userkeyid .
            ", is_asm=" . $is_asm .
            ", ordertypeid=" . $otId;
        if ($deliverydatetime) {
            $Query .= ", deliveryDateTime='" . $deliverydatetime . "'";
        }

        $Query .= " where soid = " . $orderid;
        $SQL = sprintf($Query);
        $this->executeQuery($SQL);
    ...
    public function check_entryexists($saleid, $shopid) {
        $pdo = $this->CreatePDOConn();
        $output = 0;
        $todaydate = date("Y-m-d", strtotime($this->today));
        $Query1 = "SELECT salesid FROM entry WHERE salesid = '" . $saleid . "' AND DATE(`entrytime`) ='" . $todaydate . "' AND shopid = '" . $shopid . "' AND customerno =$this->customerno order by entryid desc ";
        $result = $this->query($Query1, __FILE__, __LINE__);
    ...
    public function get_secondarysales_details_api($orderid) {
        $data = array();
        $Query = "select
                    area.areaid,a.addedby,a.soid,a.orderdate,sh.areaid,
                    sh.distributorid,a.shopid,sh.shopname,a.discount,a.is_asm, a.ordertypeid, a.deliveryDateTime, a.orderStatus
        ...
        if ($this->get_rowCount() > 0) {
            while ($row = $this->get_nextRow()) {
                $data[] = array(
                    ...
                    'areaid' => $row['areaid'],
                    'addedby' => $row['addedby'],
                    'ot_id' => $row['ordertypeid'],
                    'orderStatus' => $row['orderStatus'],
                    'deliverydatetime' => (isset($row['deliveryDateTime']) AND $row['deliveryDateTime'] != 0) ? $row['deliveryDateTime'] : "",
                    'totalskus' => $totalskus
                );
            }
            return $data;
    ...
    public function get_primarysales_orders_api($userid, $userkey_id, $distid) {
        $data = array();
        $Query = "select u.realname,u.role,a.prid,a.srid,a.distributorid,a.entrydate,a.deliverydate, a.addedby, a.is_approved, a.is_asm from primary_order as a
            ...
            where MONTH(a.entrydate)= MONTH(CURDATE())AND a.srid=" . $userid . " AND a.distributorid=" . $distid . " AND a.customerno=$this->customerno and a.isdeleted=0 order by a.entrydate desc ";
        $SQL = sprintf($Query);
        $this->executeQuery($SQL);
        if ($this->get_rowCount() > 0) {
            while ($row = $this->get_nextRow()) {
    ...
    public function get_primarysales_orderlist_api($userid, $userkey_id, $distid, $selecteddate) {
        $data = array();
        $selecteddate1 = date('Y-m-d', strtotime($selecteddate));
        $Query = "select u.realname,u.role,a.prid,a.srid,a.distributorid,a.entrydate,a.deliverydate, a.addedby, a.is_approved, a.is_asm from primary_order as a
            INNER JOIN " . SPEEDDB . ".user as u ON a.addedby = u.userid
            where a.srid=" . $userid . " AND distributorid =" . $distid . " AND a.customerno=$this->customerno and a.isdeleted=0 AND DATE(a.entrydate) = '" . $selecteddate1 . "' order by a.entrydate desc ";
        $SQL = sprintf($Query);
        $this->executeQuery($SQL);
        if ($this->get_rowCount() > 0) {
            while ($row = $this->get_nextRow()) {
    ...
    public function get_secondarysales_orders_api($userid, $userkey_id, $shopid) {
        $data = array();
        $Query = "select u.realname,u.role,a.soid,a.addedby,a.shopid, a.entrydate,a.orderdate, a.is_asm, a.ordertypeid, a.deliveryDateTime, a.orderAmt, a.orderStatus
                    from secondary_order as a
                    inner join " . SPEEDDB . ".user as u on a.addedby = u.userid
                    where  MONTH(a.entrydate) = MONTH(CURDATE()) AND a.shopid =" . $shopid . " AND a.customerno=$this->customerno and a.isdeleted=0 AND is_deadstock = 0 AND a.orderStatus = 0 order by a.entrydate desc ";
        $SQL = sprintf($Query);
        $this->executeQuery($SQL);
    ...
    public function get_secondarysales_orderlist_api($userid, $userkey_id, $distid, $selected_date) {
        $todaysdate = date('Y-m-d', strtotime($selected_date));
        $shopids = $this->getshopids_bydist($distid);
        $shopids1 = implode(",", $shopids);
        $data = array();
        $Query = "select u.realname,u.role,sh.shopname,ar.areaname,a.soid,a.addedby,a.shopid, a.entrydate,a.orderdate, a.is_asm, a.ordertypeid,
                    a.deliveryDateTime
                    from    secondary_order as a
                    inner   join " . SPEEDDB . ".user as u on a.addedby = u.userid
                    inner   join shop as sh on sh.shopid = a.shopid
                    inner   join area as ar on ar.areaid = sh.areaid
                    where   DATE(a.entrydate) = '" . $todaysdate . "' AND  a.srid=" . $userid . " AND a.shopid IN (" . $shopids1 . ") AND a.customerno=$this->customerno and a.isdeleted=0 AND is_deadstock = 0 order by a.entrydate desc ";
        $SQL = sprintf($Query);
        $this->executeQuery($SQL);
    ...
    public function get_deadstock_orders_api($userid, $userkey_id, $shopid) {
        ...
        $Query = " select a.addedby,a.is_asm,a.shopid,a.stockid,u.realname,u.role,a.stockdate,a.reason from  deadstock as a
                            ...
                            where  a.entrytime >= DATE(NOW()) - INTERVAL 7 DAY AND  a.srid=" . $userid . " AND a.shopid = " . $shopid . " AND a.customerno= $this->customerno and a.isdeleted=0 ";
    ...
    public function get_deadstock_orderlist_api($userid, $userkey_id, $distid, $selected_date) {
        $todaysdate = date('Y-m-d', strtotime($selected_date));
        $shopids = $this->getshopids_bydist($distid);
        $shopids1 = implode(",", $shopids);
        $data = array();
        $Query = " select a.addedby,sh.shopname,a.is_asm,ar.areaname,a.shopid,a.stockid,u.realname,u.role,a.stockdate,a.reason from  deadstock as a
                            inner join " . SPEEDDB . ".user as u on u.userid = a.addedby
                            inner join shop as sh on sh.shopid = a.shopid
                            inner join area as ar on ar.areaid = sh.areaid
                            where  DATE(a.stockdate) = '" . $todaysdate . "' AND a.shopid IN (" . $shopids1 . ") AND a.srid = " . $userid . " AND a.customerno= $this->customerno and a.isdeleted=0  order by stockid desc";
        $SQL = sprintf($Query);
        $this->executeQuery($SQL);
        if ($this->get_rowCount() > 0) {
            while ($row = $this->get_nextRow()) {
    ...
    public function shopcount($userid, $role, $customerno) {
        ...
        } elseif ($role == "sales_representative") {
            $salesuser = new stdClass();
            $salesuser->userid = $userid;
            $srdata = array($salesuser);
        }
        $srids = array();
        if (isset($srdata) && !empty($srdata)) {
            foreach ($srdata as $row) {
                $srids[] = $row->userid;
            }
            $distdata = $this->getDistributordata_bysr($srids);
            $shopcountadd = array();
            $distids = array();
            if (isset($distdata) && !empty($distdata)) {
                foreach ($distdata as $row1) {
                    $distids[] = $row1->userid;
                }

                $areadata = $this->get_area_api_sr($distids);
                if (isset($areadata) && !empty($areadata)) {
                    foreach ($areadata as $row4) {
                        $areaid = $row4['areaid'];
                        $srid = $row4['srid'];
                        $shopdetails = array();
                        $shopdata = $this->get_shop_api_count($srid, $areaid);
                        $shopcountadd[] = (is_array($shopdata)) ? count($shopdata) : $shopdata;
                    }
                }
                $shopcount = array_sum($shopcountadd);
            }
        }
        return $shopcount;
    }
    ...
    public function areacount($userid, $role, $customerno) {
        ...
        if (isset($distdata) && !empty($distdata)) {
            ...
            $SQL = "SELECT count(*) as areacount FROM area INNER JOIN " . SPEEDDB . ".user ON `user`.userid = area.distributorid  WHERE area.distributorid IN (" . $distid . ")  AND area.isdeleted=0";
            $Query = sprintf($SQL);
...
// checks for login
    function check_login($username, $password) {
        ...
        if ($outputResult['userkeyparam'] != 0) {
            ...
            if ($usertype == 0 && $userkeyparam != 0) {
                ...
                $retarray['erpusertoken'] = $arrResult['erpUserToken'];
                $retarray['isDistributor'] = isset($arrResult['isDistributor']) ? $arrResult['isDistributor'] : 0;
                // get customer settings //
                if (isset($arrResult['customerno']) && $arrResult['customerno'] != "" && $arrResult['customerno'] != 0) {
                    $obj = new stdClass();
                    $obj->customerNo = $arrResult['customerno'];
                    $secSettings = $this->getSecSettings($obj);
                    if (isset($secSettings[0]) && $secSettings[0] != "") {
                        $retarray['secSettings'] = $secSettings[0];
                    } else {
                        $retarray['secSettings'] = "";
                    }
                }
                ...
                $suplist = '';
                $supdata = array();
                if ($arrResult['role'] == 'ASM') {
                    // $suplist = getSuplists($arrResult['role'], $arrResult['userid'], $arrResult['customerno']);
                    $suplist = $this->getsupervisors_byasm($arrResult['userid'], $arrResult['customerno']);

                    // print("<pre>"); print_r($supervisors); die;
                    if (isset($suplist) && !empty($suplist)) {
                        $mob = new Sales($arrResult['customerno'], $arrResult['userid']);
                        //echo "<pre>"; print_r($suplist);
                        foreach ($suplist as $row) {
                            $supdata[] = array(
                                ...
                                'phone' => $row->phone,
                                'heirarchy_id' => $row->heirarchy_id,
                                /*'shopcount' => $mob->shopcountSUP($row->userid),
									                                'areacount' => $mob->areacountSUP($row->userid),
								*/
                                'shopcount' => $mob->shopcount($row->userid, $row->role, $arrResult['customerno']),
                                'areacount' => $mob->areacount($row->userid, $row->role, $arrResult['customerno']),
                                'distcount' => $mob->distcount($row->userid, $row->role, $arrResult['customerno'])
                ...
                $retarray['categorycount'] = $mob->categoryCount();
                $retarray['skucount'] = $mob->skuCount();
                $retarray['shoptypecount'] = $mob->ShopTypeCount();
    ...
    public function categoryCount() {
        $count = 0;
        $Query = "select categoryid from `category` where isdeleted=0 AND customerno= " . $this->customerno;
        $SQL = sprintf($Query);
        $this->executeQuery($SQL);
        $count = $this->get_rowCount();
        return $count;
    }

    public function skuCount() {
        $count = 0;
        $Query = "select styleid from `style` where isdeleted=0 AND customerno= " . $this->customerno;
        $SQL = sprintf($Query);
        $this->executeQuery($SQL);
        $count = $this->get_rowCount();
        return $count;
    ...
    public function ShopTypeCount() {
        $count = 0;
        $Query = "select shid from `shoptype` where isdeleted=0 AND customerno= " . $this->customerno;
        $SQL = sprintf($Query);
        $this->executeQuery($SQL);
        $count = $this->get_rowCount();
        return $count;
    ...
    public function getshopdata_byarea($areaid) {
        $shopdata = null;
        $Query = "select shopid,distributorid,areaid,salesid,sequence_no from shop where isdeleted=0 AND  areaid = " . $areaid;
        $SQL = sprintf($Query);
        $this->executeQuery($SQL);
        if ($this->get_rowCount() > 0) {
            while ($row = $this->get_nextRow()) {
    ...
    public function get_catdata_inv($distid) {
        $Query = sprintf("SELECT distinct(i.skuid),i.distributorid,i.quantity,st.styleno,c.categoryid,c.categoryname "
            . "FROM inventory as i "
    ...
    public function get_sku_invapi($distid, $categoryid) {
        ...
        $getskudata = array();
        $Query = "select st.carton as carton_qty,st.carton,st.styleid,st.categoryid,st.styleno,COALESCE(i.quantity, 0) as qty from style as st
                left join category as c on c.categoryid = st.categoryid
                left join inventory as i on i.skuid = st.styleid AND i.isdeleted=0 AND i.distributorid=" . $distid . "
                where st.isdeleted=0 AND st.customerno= $this->customerno AND st.categoryid IN (" . $catid . ")";
    ...
    public function get_sku_invapi_search($distid, $categoryid, $searchtxt = NULL) {
        $searchtxt = "%$searchtxt%";
        $catids = array();
        if ($categoryid == 1) {
            $Query = sprintf("SELECT categoryid from category where isdeleted=0 AND categoryid<>1");
        ...
        if (!empty($searchtxt) && $searchtxt != '') {
            $SQL = sprintf($Query, Sanitise::String($searchtxt));
        } else {
            $SQL = sprintf($Query);
        }
        $this->executeQuery($SQL);
        if ($this->get_rowCount() > 0) {
            while ($row = $this->get_nextRow()) {
                $getskudata[] = array(
                    ...
                    'styleno' => utf8_encode($row['styleno']),
                    'quantity' => $row['qty'] * $row['carton'],
                    'cartonperqty' => utf8_encode($row['carton_qty'])
                );
            }
            return $getskudata;
        }
    ...
    public function getshopids_bydist($distid) {
        $shopdata = array();
        $shopdata = null;
        $Query = "select shopid,distributorid,areaid,salesid,sequence_no from shop where distributorid = " . $distid;
        $SQL = sprintf($Query);
        $this->executeQuery($SQL);
        if ($this->get_rowCount() > 0) {
            while ($row = $this->get_nextRow()) {
    ...
    // get all data
    public function getAllDataApi($userdata, $is_Sendall = 1, $srid) {
        //pulldatasr
        $userid = $userdata['userid'];
        $role = $userdata['role'];
        $customerno = $userdata['customerno'];
//        $Query = "select userid,lastsync_datetime from userpull_sucess where userid = " . $userid . " order by lastsync_datetime desc limit 1";
        //        $SQL = sprintf($Query);
        //        $this->executeQuery($SQL);
        //        $rowcount = $this->get_rowCount();
        //        if ($rowcount > 0) {
        //            while ($row = $this->get_nextRow()) {
        //                $lastsyncdatetime = $row['lastsync_datetime'];
        //            }
        //        }
        $rowcount = 0;
        if ($rowcount == 0 || $is_Sendall == 1) {
            $distdata = $this->getDistributordata_bysr($srid);
            $distributordata = array();
            foreach ($distdata as $row1) {
                $areadata = array();
                $distid = $row1->userid;
                $areadata = $this->get_area_api($distid);
                $areadetails = array();
                if (!empty($areadata) && $areadata != "") {
                    foreach ($areadata as $row2) {
                        $areaid = $row2['areaid'];
                        $shopdata = $this->get_shop_api($srid, $areaid);
                        $shopdetails = array();
                        if (!empty($shopdata)) {
                            foreach ($shopdata as $row3) {
                                if (!empty($row3)) {
                                    $shopid = $row3->shopid;
                                    $shopdetails[] = array(
                                        'shopid' => $row3->shopid,
                                        ...
                                        'distributorid' => $row3->distributorid,
                                        'shoptypeid' => $row3->shoptypeid,
                                        'sequence_no' => $row3->sequence_no,
                                        'areaname' => $row3->areaname,
                                        'shop_type' => $row3->shop_type,
                                        'phone' => $row3->phone,
                                        'phone2' => $row3->phone2,
                                        'dob' => $row3->dob,
                                        'owner' => $row3->owner,
                                        'owner_shop' => $row3->owner_shop,
                                        'address' => $row3->address,
                                        'emailid' => $row3->emailid,
                                        'owner' => $row3->address,
                                        'is_entryexists' => $row3->is_entryexists,
                                        'operation_type' => 0,
                                        'srid' => $srid
                                    );
                                }
                        ...
                        $areadetails[] = array(
                            ...
                            'distributorid' => $row2['distributorid'],
                            'operation_type' => 0,
                            'srid' => $srid,
                ...
                $distributordata[] = array(
                    'distributorid' => $row1->userid,
                    'role' => $row1->role,
                    'roleid' => $row1->roleid,
                    'distemail' => $row1->email,
                    'distname' => $row1->realname,
                    'dphone' => $row1->phone,
                    'salesid' => $row1->heirarchy_id,
    ...
    // get all data
    public function getAllDataApi2($userdata, $is_Sendall = 1) {
        $userid = $userdata['userid'];
        $role = $userdata['role'];
        $customerno = $userdata['customerno'];
//        $Query = "select userid,lastsync_datetime from userpull_sucess where userid = " . $userid . " order by lastsync_datetime desc limit 1";
    ...
    // get all data
    public function getAllDataApi1($userdata, $is_Sendall = 0) {
        $userid = $userdata['userid'];
        $role = $userdata['role'];
        $customerno = $userdata['customerno'];
        $srdetails = array();
        ...
        $sridin = "  srid IN (" . $sridIn . ") ";

        $Query = "select userid,lastsync_datetime from userpull_sucess where   userid = " . $userid . " order by lastsync_datetime desc limit 1";
        $SQL = sprintf($Query);
        $this->executeQuery($SQL);
        $rowcount = $this->get_rowCount();
        if ($rowcount > 0 && $is_Sendall == 0) {
            while ($row1 = $this->get_nextRow()) {
                $timelastsync = $row1['lastsync_datetime'];
            }
            $syncQuery = "select syncid,operation_type,table_type,rowid,srid from syncdata where " . $sridin . " AND timestamp > '" . $timelastsync . "'";
            $SyncSQL = sprintf($syncQuery);
            $this->executeQuery($SyncSQL);
            $rowcount = $this->get_rowCount();
            if ($rowcount > 0) {
                $syncdata = array();
                while ($row2 = $this->get_nextRow()) {
                    $sync = new stdClass();
                    $sync->syncid = $row2['syncid'];
                    $sync->operation_type = $row2['operation_type'];
                    $sync->table_type = $row2['table_type'];
                    $sync->rowid = $row2['rowid'];
                    $sync->srid = $row2['srid'];
                    $syncdata[] = $sync;
                }
            }
            ...
            $srlist = "";
            $syncsrQuery = "select distinct srid from syncdata where " . $sridin . " AND timestamp > '" . $timelastsync . "'";
            $SyncsrSQL = sprintf($syncsrQuery);
            $this->executeQuery($SyncsrSQL);
            $rowcount1 = $this->get_rowCount();
            if ($rowcount1 > 0) {
                $syncdatasr = array();

                while ($row2 = $this->get_nextRow()) {
                    $syncdatasr[] = $row2['srid'];
                }
            }

            if (is_array($syncdatasr) && isset($syncdatasr) && !empty($syncdatasr)) {
                $srdetails = array();
                for ($i = 0; $i < count($syncdatasr); $i++) {
                    $srid = $syncdatasr[$i];
                    $distdata = $this->getDistributordata_bysr($srid);
                    ...
                    foreach ($distdata as $row1) {
                        $areadata = array();
                        $distid = $row1->userid;
                        $areadata = $this->get_area_api_sync($distid, $timelastsync, $srid);
                        $areadetails = array();
                        if (!empty($areadata) && $areadata != "") {
                            foreach ($areadata as $row2) {
                                $areaid = $row2['areaid'];
                                $shopdata = $this->get_shop_api_sync($srid, $areaid, $timelastsync);
                                $shopdetails = array();
                                if (!empty($shopdata)) {
                                    foreach ($shopdata as $row3) {
                                        if (!empty($row3)) {
                                            ...
                                            $shopdetails[] = array(
                                                ...
                                                'owner' => $row3->address,
                                                'is_entryexists' => $row3->is_entryexists,
                                                'operation_type' => $row3->operation_type,
                                                'srid' => $srid
                                            );
                                        }
                                    }
    ...
    public function getAllDataApimerge($userdata, $is_Sendall) {
        $userid = $userdata['userid'];
        $role = $userdata['role'];
        $customerno = $userdata['customerno'];
        $srdetails = array();
        ...
        if ($role == "ASM") {
            ...
            $sridIn = $userid;
            $srdetails = $this->getsrdetails($userid);
            $salesuser = new stdClass();
            foreach ($srdetails as $srdetail) {
                $salesuser->realname = $srdetail->realname;
            }
            $salesuser->userid = $userid;
            $srdata = array($salesuser);
        }
        $sridin = "  srid IN (" . $sridIn . ") ";
        $timelastsync = "";
        $Query1 = "select userid,lastsync_datetime from userpull_sucess where   userid = " . $userid . " order by lastsync_datetime desc limit 1";
        $SQL1 = sprintf($Query1);
        $this->executeQuery($SQL1);
        $rowcount1 = $this->get_rowCount();
        while ($row1 = $this->get_nextRow()) {
        ...
        $shopcounttest = $this->shopcount($userid, $role, $customerno);
        $areacounttest = $this->areacount($userid, $role, $customerno);
        $disttest = $this->distcount($userid, $role, $customerno);

        if ($rowcount1 == 0 || $is_Sendall == 1) {
            $srdetails = array();
            $shopcountadd = array();
            $distcountadd = array();
            $areacountadd = '';
            ...
            $result = array(
                'totalshopcount' => $shopcounttest,
                'totalareacount' => $areacounttest,
                'totaldistcount' => $disttest,
                'srdetails' => $srdetails
            );
        }

        if ($rowcount1 > 0 && $is_Sendall == 0) {
            // updated data send
            $shopcountadd = array();
            $distcountadd = array();
            $areacountadd = '';

            //total count code here
            $totalsr = explode(",", $sridIn);
            $disids = array();
            $shopcounts = array();
            $totalareacount = '';
            $totaldistcount1 = '';
            ...
            $srdetails1 = array();
            if (isset($syncdatasr) && !empty($syncdatasr)) {
                for ($i = 0; $i < count($syncdatasr); $i++) {
                    $srid = $syncdatasr[$i];
                    $distdata = $this->getDistributordata_bysr($srid);
                        ...
                        if (!empty($areadata) && $areadata != "") {
                            foreach ($areadata as $row2) {
                                ...
                                $areadetails[] = array(
                                    ...
                                    'areaname' => $row2['areaname'],
                                    'distributorid' => $row2['distributorid'],
                                    'operation_type' => $row2['operation_type'],
                                    'srid' => $srid
                                );
                            }
                        }

                        $shopdata = $this->get_shop_api_sync($srid, $distid, $timelastsync);
                        $shopdetails = array();
                        if (!empty($shopdata)) {
                            foreach ($shopdata as $row3) {
                                if (!empty($row3)) {
            ...
            $result = array(
                ...
                'totalareacount' => $areacounttest,
                'totaldistcount' => $disttest,
                'srdetails' => $srdetails1
            );
        }
        return $result;
    }

    public function pushstatus_updated_data($dpd) {
        $userid = $dpd['userid'];
        $syncQuery = "select userid from `userpull_sucess` where userid =" . $userid;
        $SyncSQL = sprintf($syncQuery);
        $this->executeQuery($SyncSQL);
        $rowcount = $this->get_rowCount();
        if ($rowcount > 0) {
            //update
            $Query = "update `userpull_sucess` set lastsync_datetime='" . $this->today . "' where userid=" . $userid;
            $SQL = sprintf($Query);
            $this->executeQuery($SQL);
        } else {
            //insert
            $Query1 = "Insert into `userpull_sucess`(userid,lastsync_datetime)VALUES('%s','$this->today')";
            $SQL1 = sprintf($Query1, Sanitise::String($userid));
            $this->executeQuery($SQL1);
        }
    }

    public function getsrlistbyusers($userid, $role) {
        if ($role == 'Administrator' || $role == 'elixir') {
            $srdata = $this->get_sr_by_supervisors($userid, $this->customerno, $all = 'ALL');
        } elseif ($role == "ASM") {
            $supid = array();
            $supervisors = $this->getsupervisors_byasm($userid, $this->customerno);
            foreach ($supervisors as $row) {
                $supid[] = $row->userid;
            }
            $srdata = $this->get_sr_by_supervisors($supid, $this->customerno);
        } elseif ($role == "Supervisor") {
            $srdata = $this->get_sr_by_supervisors($userid, $this->customerno);
        } elseif ($role == "sales_representative") {
            $salesuser = new stdClass();
            $salesuser->userid = $userid;
            $srdata = array($salesuser);
    ...
    public function getCallData($data) {
        //echo $srarr; die();
        $srdata = array();
        if ($data['srid'] != '-1') {
            $srdata[] = $this->calldata($data['srid'], $data);
        } elseif ($data['srid'] == '-1') {
            $srlistarr = explode(',', $data['srarr']);
            foreach ($srlistarr as $key => $val) {
                $srid = $val;
                $srdata[] = $this->calldata($srid, $data);
            }
        }
        return $srdata;
    }

    public function getCallDataSR($data, $userdate) {
        $srdata = array();
        if ($data['srid'] != '-1') {
            $srdata[] = $this->calldata_datewise($data['srid'], $data, $userdate);
        } elseif ($data['srid'] == '-1') {
            $srlistarr = explode(',', $data['srarr']);
            foreach ($srlistarr as $key => $val) {
                $srid = $val;
                $srdata[] = $this->calldata_datewise($srid, $data, $userdate);
            }
        }
        return $srdata;
    }

    public function calldata_datewise($srid, $data, $userdate) {
        $sdate = date('Y-m-d', strtotime($userdate));
        $edate = date('Y-m-d', strtotime($userdate));
        $startdate = $sdate . " 00:00:00";
        $enddate = $edate . " 23:59:00";

        $entryquery = "select sh.shopname,sh.areaid,ar.areaname,ut.role,ut.realname as mastername,u.realname,e.entryid, e.salesid,e.shopid,e.is_asm,e.addedby,e.entrydate,e.latitude,e.longtitude from entry as e
                        left join " . SPEEDDB . ".user as u ON u.userid = e.salesid
                        left join " . SPEEDDB . ".user as ut ON ut.userid = e.addedby
                        left join shop as sh ON sh.shopid = e.shopid
                        left join area as ar ON ar.areaid = sh.areaid
                        where
                        e.customerno=" . $this->customerno . "
                        AND e.salesid=" . $srid . "
                        AND e.entrydate between '" . $startdate . "' and '" . $enddate . "'
                        order by e.entrydate ASC";
        $sql = sprintf($entryquery);
        $this->executeQuery($sql);
        $rowcount = $this->get_rowCount();
        $datasr = array();
        $firstlastcall = array();
        ...
        if ($rowcount > 0) {
            while ($row = $this->get_nextRow()) {
                $datasr = array(
                    ...
                    'longtitude' => $row['longtitude'],
                    'areaname' => $row['areaname'],
                    'entrybyrole' => $row['is_asm'],
                    'masterid' => $row['addedby'],
                    'mastername' => $row['mastername'],
                    'masterrolename' => $row['role']
                );
            }
            $datafirstdata = reset($datasr);
            $datalastdata = end($datasr);
        }

        $firstlastcall[] = array(
            'srdetails' => $this->getsrdetails($srid),
            'firstcall' => $datafirstdata,
            'lastcall' => $datalastdata,
            'ordercounts' => $this->ordercounts($srid, $startdate, $enddate),
            'entrycounts' => $this->entrycounts_reports($srid, $startdate, $enddate)
        );
        return $firstlastcall;
    }

    public function calldata_datewise_summary($srid, $data, $userdate, $bysupervisor = NULL, $supid = NULL) {
        $sdate = date('Y-m-d', strtotime($userdate));
        $edate = date('Y-m-d', strtotime($userdate));
        $startdate = $sdate . " 00:00:00";
        $enddate = $edate . " 23:59:00";

        $ordercount = $this->ordercounts($srid, $startdate, $enddate, $bysupervisor, $supid);

        return $ordercount;
    }

    public function calldata($srid, $data) {
    ...
    public function getsrdetails($srid) {
        $salesdata = null;
        $Query = sprintf("SELECT userid,role,roleid,email,realname,username,phone,heirarchy_id FROM " . SPEEDDB . ".`user` where userid=" . $srid . " AND role!='elixir' AND isdeleted=0 AND customerno=" . $this->customerno);
        $this->executeQuery($Query);
        if ($this->get_rowCount() > 0) {
            while ($row = $this->get_nextRow()) {
                $salesuser = new stdClass();
    ...
    public function callstyledata($srid, $data) {
        $salesdata = array();
        $firstlastcall = array();
        $startdate = date('Y-m-d', strtotime($data['stdate']));
        $enddate = date('Y-m-d', strtotime($data['edate']));
        $startdate = $startdate . " 00:00:00";
        $enddate = $enddate . " 23:59:00";
        $secordersquery = "select st.categoryid, st.companysellingprice, s.shopname,st.styleno,st.mrp,st.distprice,st.retailprice,st.carton,so.soid,sop.skuid,sum(sop.quantity) as totalqty from secondary_order as so
                            left join secondary_order_productlist as sop ON  so.soid=sop.soid
                            left join style as st  ON st.styleid = sop.skuid
                            left join category as c ON c.categoryid = st.categoryid
                            INNER JOIN shop s on s.shopid = so.shopid
                            where
                            so.orderdate between '" . $startdate . "' and '" . $enddate . "'
                            AND so.srid =" . $srid . "
                            AND so.isdeleted=0
                            group by sop.skuid order by st.styleno,s.shopname ASC ";

        $this->executeQuery($secordersquery);
        if ($this->get_rowCount() > 0) {
            while ($row = $this->get_nextRow()) {
                $salesuser = new stdClass();
                $salesuser->categoryid = $row['categoryid'];
                $salesuser->categoryname = $row['shopname'];
                $salesuser->styleno = $row['styleno'];
                $salesuser->mrp = $row["mrp"];
                $salesuser->distprice = $row['distprice'];
                $salesuser->retailprice = $row["retailprice"];
                $salesuser->carton = $row['carton'];
                $salesuser->soid = $row["soid"];
                $salesuser->skuid = $row["skuid"];
                $salesuser->totalqty = $row["totalqty"];
                $salesdata[] = $salesuser;
                if (isset($row["companysellingprice"]) && $row["companysellingprice"] != '0.00') {
                    $salesuser->companysellingprice = $row["companysellingprice"];
                } else {
                    $salesuser->companysellingprice = "N/A";
                }
            }
        }
        $firstlastcall[] = array(
            'srdetails' => $this->getsrdetails($srid),
            'styledetails' => $salesdata
        );
        return $firstlastcall;
    }

    function ordercount($srid, $prevdate) {
        $ordercount = 0;
        $startdate = $prevdate . " 00:00:00";
        $enddate = $prevdate . " 23:59:00";

        $secordersquery = "select count(so.soid) as ordercount from secondary_order as so
                            where
                            so.orderdate between '" . $startdate . "' and '" . $enddate . "'
                            AND so.srid =" . $srid . "
                            AND so.customerno =" . $this->customerno . "
                            AND so.isdeleted=0";
        $this->executeQuery($secordersquery);
        if ($this->get_rowCount() > 0) {
            while ($row = $this->get_nextRow()) {
    ...
    function ordercounts($srid, $startdate, $enddate, $bysupervisor = NULL, $supid = NULL) {
        $ordercount = 0;
        if ($bysupervisor == 1 && !($supid == NULL)) {
            $secordersquery = "select count(so.soid) as ordercount from secondary_order as so
                            where
                            so.orderdate between '" . $startdate . "' and '" . $enddate . "'
                            AND so.addedby =" . $supid . "
                            AND so.srid =" . $srid . "
                            AND so.customerno =" . $this->customerno . "
                            AND so.isdeleted=0";
        } else {
    ...
    function entrycounts($srid, $prevdate) {
        $entrycount = 0;
        $startdate = $prevdate . " 00:00:00";
        $enddate = $prevdate . " 23:59:00";
        $entryquery = "select count(e.entryid) as entrycount from entry as e
                        left join " . SPEEDDB . ".user as u ON u.userid = e.salesid
                        left join shop as sh ON sh.shopid = e.shopid
                        where
                        e.customerno=" . $this->customerno . "
    ...
    function entrycounts_reports($srid, $startdate, $enddate) {
        $entrycount = 0;
        $entryquery = "select count(e.entryid) as entrycount from entry as e
                        left join " . SPEEDDB . ".user as u ON u.userid = e.salesid
                        left join shop as sh ON sh.shopid = e.shopid
    ...
    public function pushattendance($dpd) {
        //0-absent  //1 - attendance
        $date = date('Y-m-d');
        $startdate = $date . " 00:00:00";
        $enddate = $date . " 23:59:00";

        $userid = $this->userid; //$dpd['userid'];
        $attnquery = "select a.atid, a.updatedon from attendance as a
                        left join " . SPEEDDB . ".user as u ON u.userid = a.userid
                        where
                        a.customerno=" . $this->customerno . "
                        AND a.userid=" . $userid . "
                        AND a.createdon between '" . $startdate . "' and '" . $enddate . "'
                        AND a.isdeleted = 0
                        order by a.createdon desc";
        $this->executeQuery($attnquery);
        $atd = 0;
        $msg = '';
        if ($dpd->status == 0) {
            $updatestatus = 0; //off
            $msg = "Attendance updated successfully";
            // $msg = "Attendance updated successfully as absent ";
        }
        if ($dpd->status == 1) {
            $updatestatus = 1; //present
            $msg = "Attendance updated successfully ";
        }
        if ($this->get_rowCount() > 0) {
            while ($row = $this->get_nextRow()) {
                $atd = $row['atid'];
                $isUpdated = (isset($row['updatedon']) && $row['updatedon'] != "0000-00-00 00:00:00") ? 1 : 0; // 1 = Already Updated and 0 = access to update
                // echo($row['updatedon']); die;
                // echo $isUpdated; die;
            }
            if ($isUpdated == 0) {
                //update attendance
                $Query = "update `attendance` set lat = " . $dpd->lat . " , lng = " . $dpd->lng . ",updatedby='" . $this->userid . "', updatedon='" . $this->today . "' where atid=" . $atd;

                /*$Query = "update `attendance` set lat = ".$dpd->lat." , lng = ".$dpd->lng." , onoff='" . $updatestatus . "',updatedby='" . $this->userid . "', updatedon='" . $this->today . "' where atid=" . $atd;*/
                $SQL = sprintf($Query);
                $this->executeQuery($SQL);
            } else {
            ...
            //insert new record
            // - $today = date("Y-m-d H:i:s");
            $Query = "Insert into attendance (userid,customerno,onoff,lat,lng,createdby,createdon)
                    VALUES($this->userid,$this->customerno,%d,$dpd->lat,$dpd->lng,$this->userid,'$this->today')";

            $SQL = sprintf($Query, Sanitise::Long($updatestatus), Sanitise::Long($dpd->lat), Sanitise::Long($dpd->lng));

            // $SQL = sprintf($Query);
            $this->executeQuery($SQL);
        }
        $status = array('message' => $msg, 'attendancestatus' => $updatestatus);
        return $status;
    }

    public function pullattendance_status($dpd) {
        $status = array();
        $onoff = 0;
        $date = date('Y-m-d');
        $startdate = $date . " 00:00:00";
        ...
        $userid = $dpd['userid'];
        $attnquery = "select atid,onoff from attendance as a
                        left join " . SPEEDDB . ".user as u ON u.userid = a.userid
                        where
                        a.customerno=" . $this->customerno . "
                        AND a.userid=" . $userid . "
        ...
        $status = array('attendancestatus' => $onoff);
        //0-OFF ,1-ON
        return $status;
    }

    public function add_attendancedata($srcode, $entrydate, $entrytime, $status) {
        //0-ON ,1-OFF
        $date = date('Y-m-d', strtotime($entrydate));
        $startdate = $date . " 00:00:00";
        $enddate = $date . " 23:59:00";
        $attendancedatetime = $date . " " . $entrytime;
        ...
        if ($this->get_rowCount() > 0) {
            ...
            $Query = "update `attendance` set onoff='" . $updatestatus . "',updatedby='" . $this->userid . "', updatedon='" . $this->today . "' where atid=" . $atd;
            $SQL = sprintf($Query);
            $this->executeQuery($SQL);
        } else {
            //insert new record
            //$today = date("Y-m-d H:i:s");
            $Query = "Insert into attendance (userid,customerno,onoff,createdby,createdon)
                    VALUES($userid,$this->customerno,$updatestatus,$this->userid,'$attendancedatetime')";
            $SQL = sprintf($Query);
            $this->executeQuery($SQL);
        }
    }

    public function getattendanceeditdata($atid) {
        $data = array();
        $attnquery = "select u.realname,a.atid,a.userid,a.onoff,a.createdon from attendance as a
                left join " . SPEEDDB . ".user as u ON u.userid = a.userid
                where
                a.customerno=" . $this->customerno . "
                AND a.atid = $atid
    ...
    public function delete_attendancerecord($id) {
        $Query = "update `attendance` set isdeleted=1 where atid=" . $id;
        $SQL = sprintf($Query);
        $this->executeQuery($SQL);
    }

    public function getsupervisorslist($customerno) {
        $salesdata = array();
        $Query = sprintf("SELECT userid,role,roleid,email,realname,username,phone,heirarchy_id FROM " . SPEEDDB . ".`user` where isdeleted=0 AND customerno=" . $customerno . " AND roleid=40 order by realname asc");
        $this->executeQuery($Query);
        if ($this->get_rowCount() > 0) {
            while ($row = $this->get_nextRow()) {
                $salesuser = new stdClass();
    ...
    public function getCallSummaryData($data, $userdate) {
        $srdata = array();
        if ($data['srid'] != '-1') {
            $srdata[] = $this->callSummarydata($data['srid'], $data, $userdate);
        } elseif ($data['srid'] == '-1') {
            $srlistarr = explode(',', $data['srarr']);
            //print("<pre>"); print_r($srlistarr); die;
            foreach ($srlistarr as $key => $val) {
                $srid = $val;
                $srdata[] = $this->callSummarydata($srid, $data, $userdate);
                // $srdata[] = $this->calldata_datewise($srid, $data, $userdate);
            }
        }
        return $srdata;
    ...
    public function callSummarydata($srid, $data, $userdate) {
        $sdate = date('Y-m-d', strtotime($userdate));
        $edate = date('Y-m-d', strtotime($userdate));
        $startdate = $sdate . " 00:00:00";
        $enddate = $sdate . " 23:59:00";

        $entryquery = "

            select sh.shopname,sh.entrytime, sh.areaid,ar.areaname,ut.role,ut.realname as supervisor,
                u.realname,sh.salesid,sh.shopid,sh.addedby from shop as sh
                left join " . SPEEDDB . ".user as u ON u.userid = sh.salesid
                left join " . SPEEDDB . ".user as ut ON ut.userid = u.heirarchy_id
                left join area as ar ON ar.areaid = sh.areaid
                    where sh.customerno=" . $this->customerno . "
                        AND sh.salesid=" . $srid . "
                        AND sh.entrytime between '" . $startdate . "' and '" . $enddate . "'
                        order by sh.entrytime DESC
    ...
    public function get_ordertype_api() {
        $Query = "SELECT otId,orderTypeTitle FROM orderType WHERE customerno in (0,$this->customerno) AND isdeleted=0";
        $SQL = sprintf($Query);
        $this->executeQuery($SQL);
        if ($this->get_rowCount() > 0) {
            while ($row = $this->get_nextRow()) {
                $getotdata[] = array(
                    'otId' => $row['otId'],
                    'orderTypeTitle' => $row['orderTypeTitle']
                );
            }
            return $getotdata;
        }
    ...
    public function getattendancedata($data) {
        $mob = new Sales($data['customerno'], $data['userid']);
        $attendanceData = $this->callattendancedata($data);
        return $attendanceData;
    }

    public function callattendancedata($data) {
        ...
        $query = "select
                            ...
                            ,a.lng
                            from attendance a
                            left join speed.user u on a.userid = u.userid and u.isdeleted = 0

                            where
                            a.createdon between '" . $startdate . "' and '" . $enddate . "'
                            AND a.customerno =" . $data['customerno'] . "
                            AND a.isdeleted=0
                            order by a.createdon DESC";
        $this->executeQuery($query);
        if ($this->get_rowCount() > 0) {
            while ($row = $this->get_nextRow()) {
                ...
                $attendance->realname = $row['realname'];
                $attendance->role = $row['role'];
                $attendance->createdon = $row['createdon'];
                $attendance->updatedon = $row['updatedon'];
                $attendance->onoff = $row['onoff'];
                $attendance->lat = $row['lat'];
                $attendance->lng = $row['lng'];

                if (isset($row["onoff"]) && $row["onoff"] == '0') {
                    $attendance->status = "Absent";
                } elseif (isset($row["onoff"]) && $row["onoff"] == '1') {
    ...
    public function getCountOfProducts_SalesSummary($obj) {
        ...
        $queryCallSP = "CALL " . speedConstants::SP_GET_PRODUCT_COUNT . "(" . $sp_params . ")";
    ...
    public function getSecSettings($secObj) {
        $secSettingData = array();
        $Query = "SELECT * FROM secondarySettings WHERE customerNo in (0,$secObj->customerNo) AND isDeleted=0 LIMIT 1";
        $SQL = sprintf($Query);
        $this->executeQuery($SQL);
        if ($this->get_rowCount() > 0) {
            while ($row = $this->get_nextRow()) {
                $secSettingData[] = array(
                    ...
                    'isSecSales' => $row['isSecSales'],
                    'isPrimarySales' => $row['isPrimarySales'],
                    'isCatelog' => $row['isCatelog'],
                    'isSrList' => $row['isSrList'],
                    'isDeliveryDateTime' => $row['isDeliveryDateTime'],
                    'isDeadStock' => $row['isDeadStock'],
                    'isCustomerGroup' => $row['isCustomerGroup']

                );
            }
            return $secSettingData;
        }
        return $secSettingData;
    }
    ...
    public function getFranchiseList() {
        $franchiseList = array();
        $Query = "SELECT franchiseId,franchiseName FROM franchise WHERE customerNo in (0,$this->customerno) AND isDeleted=0 ";
        $SQL = sprintf($Query);
        $this->executeQuery($SQL);
        if ($this->get_rowCount() > 0) {
            while ($row = $this->get_nextRow()) {
    ...
    public function getProductList($obj) {
        $pdo = $this->CreatePDOConn();
        //print_r($pdo); die;
        $arrResult = array();
        $today = date("Y-m-d H:i:s");
        $obj->productId = isset($obj->productId) ? $obj->productId : 0;
        $obj->searchString = isset($obj->searchString) ? $obj->searchString : '';
        $obj->pageIndex = isset($obj->pageIndex) ? $obj->pageIndex : 1;
        $obj->pageSize = isset($obj->pageSize) ? $obj->pageSize : 10;
        $obj->shopId = isset($obj->shopId) ? $obj->shopId : 0;

        $sp_params = "'" . $obj->productId . "'"
        . ",'" . $obj->franchiseId . "'"
        . ",'" . $obj->searchString . "'"
        . ",'" . $obj->shopId . "'"
        . ",'" . $obj->pageIndex . "'"
        ...
        $queryCallSP = "CALL " . speedConstants::SP_GET_PRODUCT_LIST . "(" . $sp_params . ")";
        $arrResult = $pdo->query($queryCallSP)->fetchAll(PDO::FETCH_ASSOC);
    ...
    public function getAllShopsOrder($request) {
        $data = array();
        if (isset($request) && !empty($request)) {
            $todaysdate = isset($request->selected_date) ? date('Y-m-d', strtotime($request->selected_date)) : '';
            $request->dayId = isset($request->dayId) ? $request->userId : '-1';

            $userid = isset($request->userId) ? $request->userId : 0;
            // $shopids = $this->getshopids_bydist($distid);
            $shopList = $this->getShopsBySR($request);
// print("<pre>"); print_r($shopids); die;
            // $shopids1 .= implode(",", $shop);
...
// print_r($shopids1); die;
            ...
            if ($this->get_rowCount() > 0) {
                while ($row = $this->get_nextRow()) {
                    $edit_option = 0;
                    if ($row['addedby'] == $request->userkey_id) {
                        $edit_option = 1;
                    }
    ...
    public function getShopsBySR($request) {
        $pdo = $this->CreatePDOConn();
        //print_r($pdo); die;
        $arrResult = array();
        $today = date("Y-m-d H:i:s");
        $request->dayId = isset($request->dayId) ? $request->dayId : 0;
        $request->searchString = isset($request->searchString) ? $request->searchString : '';
        $sp_params = "'" . $request->userId . "'"
        . ",'" . $request->dayId . "'"
        . ",'" . $request->searchString . "'"
        . ",'" . $request->customerNo . "'";

        $queryCallSP = "CALL " . speedConstants::SP_GET_SHOPS_BY_SR . "(" . $sp_params . ")";
        //select shopid,distributorid,areaid,salesid,sequence_no from shop where distributorid = " . $distid;
        $arrResult = $pdo->query($queryCallSP)->fetchAll(PDO::FETCH_ASSOC);
        return $arrResult;
    }
    ...
    public function getOffers($obj) {
        $pdo = $this->CreatePDOConn();
        $obj->offerId = isset($obj->offerId) ? $obj->offerId : 0;
        $arrResult = array();
        $today = date("Y-m-d");
        $sp_params = "'" . $obj->offerId . "'"
        . ",'" . $obj->productId . "'"
        . ",'" . $today . "'"
        . ",'" . $obj->customerNo . "'";
        $queryCallSP = "CALL " . speedConstants::SP_GET_OFFERS . "(" . $sp_params . ")";
        $arrResult = $pdo->query($queryCallSP)->fetchAll(PDO::FETCH_ASSOC);
        return $arrResult;
    }
}
