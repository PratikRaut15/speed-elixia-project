...
function getfilteredfuelmaintenance($vehicleid,$dealerid,$sdate,$edate){
    $vehiclemgr = new VehicleManager($_SESSION['customerno']);
    $report = $vehiclemgr->getAllFilterdFuelRecords($vehicleid,$dealerid,$sdate,$edate);
    return $report;
}
...
function gettempreportpdf($customerno,$STdate,$EDdate,$deviceid,$vehicleno,$interval,$stime,$etime)
{
    ...
    if($days!=NULL && count($days)>0)
    {
        //$date_t = strtotime(date("Y-m-d H:i:s").' UTC');
        $today = date('Y-m-d H:i:s');
        $formatdate = date ('F j, Y H:i A',strtotime($STdate));
        $fromdate = date ('F j, Y H:i A',strtotime($STdate));
        $todate = date ('F j, Y H:i A',strtotime($EDdate));
        $finalreport = '<div style="width:auto; height:30px;">
                        <table style="width: auto; border:none;">
                        <tr>
                        <td style="width:430px; border:none;"><img style="width:50%; height: 50%;" src="../../images/elixiaspeed_logo.png" /></td>
...
function get_checkpointname($id) {
    $checkpointmanager = new CheckpointManager($_SESSION['customerno']);
    $checkpointname = $checkpointmanager->get_checkpointname($id);
    return $checkpointname;
}
function getdailyreport_check($STdate,$ETdate,$stime,$etime,$chkpt_start,$chkpt_end,$chkpt_via=null)
{
    $DATAS = Array();
    ...
    foreach($VEHICLES as $vehicle){
        $DATAS[] = GetCheckpoint_Report($location, $STdate,$ETdate,$vehicle->vehicleid,$chkpt_start, $chkpt_end, $vehicle->groupid, $chkpt_via);
    }
    return $DATAS;
}
...
function getdailyreport_check_pdf($customerno,$STdate,$ETdate,$stime,$etime,$chkpt_start,$chkpt_end,$chkpt_via)
{
    $DATAS = Array();
    $STdate = $STdate." ".date('H:i:s', strtotime($stime));
    $ETdate = $ETdate." ".date('H:i:s', strtotime($etime));
    ...
    foreach($VEHICLES as $vehicle){
         $DATAS[] = GetCheckpoint_Report_pdf($customerno,$location, $STdate,$ETdate,$vehicle->vehicleid,$chkpt_start, $chkpt_end,$chkpt_via);
    }
...
function getdailyreport_check_by_vehicle($STdate,$ETdate,$stime,$etime,$chkpt_start,$chkpt_end, $chkpt_via)
{
    $DATAS = Array();
    $STdate = $STdate." ".date('H:i:s', strtotime($stime));
    $ETdate = $ETdate." ".date('H:i:s', strtotime($etime));
    ...
    foreach($VEHICLES as $vehicle){
         $DATAS[] = GetCheckpoint_Report_by_vehicle($location, $STdate,$ETdate,$vehicle->vehicleid,$chkpt_start, $chkpt_end, $chkpt_via);
    }
...
function GetCheckpoint_Report($location,$STdate,$ETdate,$id,$chkpt_start,$chkpt_end, $groupid=null, $chkpt_via=null)
{
    $path = "sqlite:$location";
    $db = new PDO($path);
...
function GetCheckpoint_Report_pdf($customerno,$location,$STdate,$ETdate,$id,$chkpt_start,$chkpt_end,$chkpt_via=null)
{
    $path = "sqlite:$location";
    $db = new PDO($path);
...
function GetCheckpoint_Report_by_vehicle($location,$STdate,$ETdate,$id,$chkpt_start,$chkpt_end, $chkpt_via=null)
{
    $path = "sqlite:$location";
    $db = new PDO($path);
    $REPORT_Start = array();
...
function print_Trip_Report($customerno,$STDate,$EDdate,$Stime, $Etime, $chk_start, $chk_end,$chk_via, $ReportType)
{
    if($ReportType == 'trip')
              {
                  $reports = getdailyreport_check_pdf($customerno, $STDate, $EDdate, $Stime,$Etime,$chk_start,$chk_end,$chk_via);
                // print_r($reports);
                if(isset($reports))
                {
                    ?>
                    ...
                    <table>
                        <thead>
                            ...
                            <tr style="background-color: #CCCCCC; border: none; ">
                                <td colspan="100%" style="text-align: center; border: none; height: 30px;">Between Checkpoints <strong><?php echo get_checkpointname($chk_start);?> To <?php echo get_checkpointname($chk_end); ?> <?php if(!empty($chk_via)){echo "Via ".get_checkpointname($chk_via); } ?></strong> From <?php echo date("M j Y", strtotime($STDate)); ?> To <?php echo date("M j Y", strtotime($EDdate)); ?> </td>
                            </tr>
                            <tr>
                          ...
                          foreach($thisvehicle as $report)
                           ...
                           ?>
                                  <tr>
                                     ...
                                     <td style="text-align: center;"><?php if($report->distance !='0'){ echo $report->distance; }else { echo "N/A"; }?></td>
                                     <td style="text-align: center;"><?php echo $report->fuelconsume;?></td>
                                     <td style="text-align: center;"><?php if($report->distance !='0'){ echo m2h(getduration($report->enddate,$report->startdate)); }else { echo "N/A"; }?></td>
                                     <td style="text-align: center;"><?php if($report->idletime !=''){ echo m2h($report->idletime); }else { echo "N/A"; }?></td>
                                     <td style="text-align: center;"><?php echo date('d-m-Y g:i a', strtotime($report->startdate));?> </td>
                                     <td style="text-align: center;"><?php echo date('d-m-Y g:i a', strtotime($report->enddate));?>  </td>
                     ...
                     if(!empty($new_report))
                     {
                        ?>
                           <tr>
                               ...
                               <td style="text-align: center;"><?php if($new_report['distance'] !='0'){ echo round($new_report['distance'] /$new_report['trip'],2); }else { echo "N/A"; }?></td>
                                <td style="text-align: center;"><?php if($new_report['fuelconsume'] > 0)
                               {
                                   echo $new_report['fuelconsume'] / $new_report['trip'];
                               }else
                                   echo $new_report['fuelconsume'] ="N/A";
                                   ?></td>
                               <td style="text-align: center;"><?php if($new_report['distance'] !='0'){ echo m2h($new_report['timetaken'] / $new_report['trip'] ); }else { echo "N/A"; }?></td>
                               <td style="text-align: center;"><?php if($new_report['idletime'] !=''){ echo m2h($new_report['idletime'] / $new_report['trip']); }else { echo "N/A"; }?></td>
...
function print_Trip_report_csv($customerno,$STDate,$EDdate,$Stime, $Etime, $chk_start, $chk_end,$chk_via,$ReportType)
{
    if($ReportType == 'trip')
              {
                  $reports = getdailyreport_check_pdf($customerno, $STDate, $EDdate, $Stime,$Etime,$chk_start,$chk_end,$chk_via);
       ...
       <tr style="background-color: #CCCCCC; border: none; ">
                                <td colspan="7" style="text-align: center; border: none; height: 30px;">Between Checkpoints <strong><?php echo get_checkpointname($chk_start);?> To <?php echo get_checkpointname($chk_end); ?> <?php if(!empty($chk_via)){echo "Via ".get_checkpointname($chk_via); } ?> </strong> From <?php echo date("M j Y", strtotime($STDate)); ?> To <?php echo date("M j Y", strtotime($EDdate)); ?> </td>
       </tr>
                          ...
                          foreach($thisvehicle as $report)
                           ...
                           ?>
                              <tr>
                               ...
                               <td style="text-align: center;"><?php if($report->distance !='0'){ echo m2h(getduration($report->enddate,$report->startdate)); }else { echo "N/A"; }?></td>
                               <td style="text-align: center;"><?php if($report->idletime !=''){ echo m2h($report->idletime); }else { echo "N/A"; }?></td>
                               <td style="text-align: center;"><?php echo date('d-m-Y g:i a', strtotime($report->startdate));?>
                               <td style="text-align: center;"><?php echo date('d-m-Y g:i a', strtotime($report->enddate));?>
                               </td>
                            </tr>
                            <?php
                            }
       ...
       <tr style="background-color: #CCCCCC; border: none; ">
                                <td colspan="6" style="text-align: center; border: none; height: 30px;">Between Checkpoints <strong><?php echo get_checkpointname($chk_start);?> To <?php echo get_checkpointname($chk_end); ?> <?php if(!empty($chk_via)){echo "Via ".get_checkpointname($chk_via); } ?> </strong> From <?php echo date("M j Y", strtotime($STDate)); ?> To <?php echo date("M j Y", strtotime($EDdate)); ?> </td>
       </tr>
...
function getfuelcomsumptionpdf($customerno,$usemaintainance,$usehierarchy,$groupid,$STdate,$EDdate,$vehicleid)
{
...
function getfuelconsumptioncsv($customerno,$usemaintainance,$usehierarchy,$groupid,$STdate,$EDdate,$vehicleid)
{
    $totaldays = gendays($STdate, $EDdate);
    $location = "../../customer/$customerno/reports/dailyreport.sqlite";
    if(file_exists($location)){
...
function generate_maintenance_fuel($vid,$vno,$startdate,$enddate,$dealerid,$customerno){
    $vehicleid = (int)$vid;
    ...
    if($vehicleid==0){
        echo "Please select Vehicle Number";
    }
    elseif($sdate > $edate){
        ...
        $fuels = getfilteredfuelmaintenance($vehicleid,$dealerid,$sdate,$edate);
        ...
        if(!empty($fuels)){
            ...
            {
                echo "<tr>";
                echo "<td>".$i++."</td><td>{$fuel->vehicleno}</td><td>{$fuel->fuel}</td><td>{$fuel->amount}</td>";
                echo "<td>".round($fuel->rate,2)."</td>";
                echo "<td>{$fuel->refno}</td><td>{$fuel->openingkm}</td><td>{$fuel->endingkm}</td>";
                $average = $fuel->endingkm-$fuel->openingkm;
                echo "<td>$average</td>";
                echo "<td>{$fuel->average}</td><td>{$fuel->dealername}</td>";
                echo "<td>".date('d-m-Y', strtotime($fuel->addedon))."</td>";
                echo "</tr>";
            }
        }
        else{
            echo "<td colspan='12'>Data not available</td>";
        }
        echo "</table>";
    }
