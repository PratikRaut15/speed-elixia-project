    ...
    foreach ($users as $thisuser) {
        ...
        if (isset($Data)) {
            foreach ($Data as $thisdata) {
                ...
                $thisdata->overspeed = 15;
                $thisdata->topspeed = 100;
                if ($thisdata->overspeed > 0 && $thisdata->topspeed >= $thisdata->overspeed_limit && in_array($thisdata->vehicleid, $vehiclelistarray)) {
                    ...
                    $response = '';
                    echo $thisuser->phoneno = $thisuser->phone;
                    echo "<br/>";
                    ...
                    if (isset($thisuser->phoneno) && !empty($thisuser->phoneno)) {
                        ...
                        echo $thisdata->vehicleid;
                        echo "<br/>";

                        $vehDetails = vehicleDetails($thisdata->vehicleno);
                        //print_r($vehDetails);
                        echo "<br/>";
                        $vehPre = $vehDetails[3] . " " . $vehDetails[4] . " " . $vehDetails[5];
                        echo "<br/>";
                        $veStr = '';
                        $vehNo = str_split($vehDetails[6]);
                        if (isset($vehNo) && !empty($vehNo)) {
                            foreach ($vehNo as $v) {
                        ...
                        //print_r($vehNo);
                        echo "<br/>";
                        echo $v_details = $vehPre . " " . $veStr;
                        echo "<br/>";

                        $objTelAlertDetails = new stdClass();
                        //$objTelAlertDetails->flowId = '234988';
                        $objTelAlertDetails->phoneNo = "9421377403"; //"8828022111";
                        $objTelAlertDetails->customerno = $customerno;
                        $objTelAlertDetails->telAlertLogId = "1";
                        $objTelAlertDetails->cqId = "1";
                        $objTelAlertDetails->customMessage = "Dear " . $thisuser->realname . ", Vehicle Number " . $v_details . " has Over Speed for " . $thisdata->overspeed . " times on " . $reportPeriod . ". Please follow speed limits for company vehicles and stay safe. Regards,  Vehicle Management Team- I&S HO.";
                        echo $thisuser->userid;
                        echo "<br/>";
                        print_r($objTelAlertDetails);
                        echo "<br/>";
                    ...
                    $isMailSent = 1; //sendMailUtil(array($mailid), $CCEmail, $BCCEmail, $subject, $message, $attachmentFilePath, $attachmentFileName, $isTemplatedMessage = 1);
                    if (isset($isMailSent)) {
                        echo $message;
                    } else {
                        echo "Test";
