...
function ProcessData($arrLatestVehData) {
    $objVehicleManager = new VehicleManager(speedConstants::CUSTNO_JUNAID_TRANSPORT);
	...
	if(isset($arrLatestVehData) && !empty($arrLatestVehData) && is_array($arrLatestVehData)){
		foreach ($arrLatestVehData as $arrVehData) {
			...
			if (isset($vehData)) {
                ...
                //Enter in Sqlite
                $objVehDetail->customerNo = speedConstants::CUSTNO_JUNAID_TRANSPORT;
...
function InsertDataInSqlite($vehData, $objVehDetail) {
    if ($objVehDetail->unitNo != "") {
        ...
        $db = "sqlite:" . RELATIVE_PATH_DOTS . "customer/" . speedConstants::CUSTNO_JUNAID_TRANSPORT . "/unitno/" . $objVehDetail->unitNo . "/sqlite/" . $date . ".sqlite";
