...
/*$customerNo =  speedConstants::CUSTNO_GATI; // 644 Gati;
$postData = array(
...
$objJsonReq->apiUrl = "http://latlong.net.in/Client/LatLong_Mondelez.asmx?wsdl"; // remark, we have used "?wsdl" for xml api
$objJsonReq->customerNo = speedConstants::CUSTNO_GATI;
...
function ProcessData($arrLatestVehData) {
    $objVehicleManager = new VehicleManager(speedConstants::CUSTNO_GATI);
    foreach ($arrLatestVehData as $arrVehData) {
        ...
        if (isset($vehData)) {
            ...
            #########################################################################
                ...
                //Enter in Sqlite
                $objVehDetail->customerNo = speedConstants::CUSTNO_GATI;
...
function ParseData($vehData) {
    ...
    $objVehDetail->ignition = $vehData->currentignitionStatus;
...
function InsertDataInSqlite($vehData, $objVehDetail) {
    if ($objVehDetail->unitNo != "") {
        ...
        $db = "sqlite:" . RELATIVE_PATH_DOTS . "customer/" . speedConstants::CUSTNO_GATI . "/unitno/" . $objVehDetail->unitNo . "/sqlite/" . $date . ".sqlite";
