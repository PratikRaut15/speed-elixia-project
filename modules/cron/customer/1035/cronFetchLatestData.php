...
foreach ($geoTackUsers as $user) {
        ...
        $objVendorDetails->customerNo = speedConstants::CUSTNO_INDO_GULF_CARRIER;
...
function ProcessData($arrLatestVehData) {
    $objVehicleManager = new VehicleManager(speedConstants::CUSTNO_INDO_GULF_CARRIER);
    ...
    if(isset($arrLatestVehData) && !empty($arrLatestVehData) && is_array($arrLatestVehData)){
        foreach ($arrLatestVehData as $arrVehData) {
            ...
            if (isset($vehData)) {
                ...
                //Enter in Sqlite
                $objVehDetail->customerNo = speedConstants::CUSTNO_INDO_GULF_CARRIER;
