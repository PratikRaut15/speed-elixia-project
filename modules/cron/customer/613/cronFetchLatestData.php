...
$geoTrackUrls = array(
    array(
        "apiUrl" => "http://203.115.101.54/mobileapp/nestlenorth_api.php"

    ),
    array(
        "apiUrl" => "http://203.115.101.54/mobileapp/mondelezprimary_api.php"
    )

);

foreach ($geoTrackUrls as $apiUrl) {
...
//$url = "http://localhost/speed/modules/api/gpsprovider/gtrac/consumeApi.php";
$customerNo =  speedConstants::CUSTNO_COLDEX; // 613 Coldex;
...
$objJsonReq->apiUrl = $apiUrl['apiUrl'];
...
function ProcessData($arrLatestVehData) {
    $objVehicleManager = new VehicleManager(speedConstants::CUSTNO_COLDEX);
    ...
    foreach ($arrLatestVehData as $arrVehData) {
        ...
        if (isset($vehData)) {
            ...
            #########################################################################
                ...
                //Enter in Sqlite
                $objVehDetail->customerNo = speedConstants::CUSTNO_COLDEX;
            ...
            $objOutput = $objVehicleManager->gpsProviderUpdateDeviceData($objVehDetail);
            //Enter in Sqlite
            $objVehDetail->customerNo = speedConstants::CUSTNO_GTRACK;
            $objVehDetail->uid = isset($objOutput->uid) ? $objOutput->uid : 0;
            $objVehDetail->unitNo = isset($objOutput->unitNo) ? $objOutput->unitNo : "";
            $objVehDetail->vehicleId = isset($objOutput->vehicleId) ? $objOutput->vehicleId : 0;
            $objVehDetail->deviceId = isset($objOutput->deviceId) ? $objOutput->deviceId : 0;
...
function InsertDataInSqlite($vehData, $objVehDetail) {
    if ($objVehDetail->unitNo != "") {
        ...
        $db = "sqlite:" . RELATIVE_PATH_DOTS . "customer/" . speedConstants::CUSTNO_GTRACK . "/unitno/" . $objVehDetail->unitNo . "/sqlite/" . $date . ".sqlite";
