...
$geoTackUsers = array(
    array(
        "userName" => "sanjaytptco1",
        "pwd" => "SANjay@27"
    ),
    array(
        "userName" => "sanjaytptco2",
        "pwd" => "SANjay@27"
    ),
    array(
        "userName" => "sanjaytpt",
...
foreach ($geoTackUsers as $user) {
        ...
        $objVendorDetails->customerNo = speedConstants::CUSTNO_RAJIV_TRANSPORT_CO;
...
function ProcessData($arrLatestVehData) {
    $objVehicleManager = new VehicleManager(speedConstants::CUSTNO_RAJIV_TRANSPORT_CO);
    ...
    if(isset($arrLatestVehData) && !empty($arrLatestVehData) && is_array($arrLatestVehData)){
        foreach ($arrLatestVehData as $arrVehData) {
            ...
            if (isset($vehData)) {
                ...
                //Enter in Sqlite
                $objVehDetail->customerNo = speedConstants::CUSTNO_RAJIV_TRANSPORT_CO;
