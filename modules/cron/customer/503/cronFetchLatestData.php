...
define("GEOTRACKER_CUST_ID_MRKOOL", "RAVI");
...
$objVendorDetails->userName = "ruralvts";
$objVendorDetails->pwd = "kOOL@1977";
...
function ProcessData($arrLatestVehData) {
    $objVehicleManager = new VehicleManager(speedConstants::CUSTNO_MRKOOL);
    ...
    foreach ($arrLatestVehData as $arrVehData) {
        ...
        if (isset($vehData)) {
            ...
            //Enter in Sqlite
            $objVehDetail->customerNo = speedConstants::CUSTNO_MRKOOL;
...
function ParseData($vehData) {
    ...
    $objVehDetail->vehNo = str_replace(" ", "", str_replace(GEOTRACKER_CUST_ID_MRKOOL . "-", "", $vehData->regNo));
...
function InsertDataInSqlite($vehData, $objVehDetail) {
    if ($objVehDetail->unitNo != "") {
        ...
        $db = "sqlite:" . RELATIVE_PATH_DOTS . "customer/" . speedConstants::CUSTNO_MRKOOL . "/unitno/" . $objVehDetail->unitNo . "/sqlite/" . $date . ".sqlite";
