...
$url = "http://speed.elixiatech.com/modules/api/gpsprovider/ashokLeyland/consumeApi.php";
// $url = "http://localhost/elixia_speed/modules/api/gpsprovider/ashokLeyland/consumeApi.php";

$objJsonReq = new stdClass();
$objVendorDetails = new stdClass();
/*$objVendorDetails->userName = "ghgconline";
$objVendorDetails->pwd = "vts@ghgc";*/
$objVendorDetails->token = "eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJOakkwTWpnPVxyXG4iLCJhdWQiOiJEQUFTIn0.brgKtmze0rK_1uU3dIuhLLMz6KExj2B256GO_Hhl0LI";
...
function ProcessData($arrLatestVehData) {
    $objVehicleManager = new VehicleManager(speedConstants::CUSTNO_DELEX);
    foreach ($arrLatestVehData as $arrVehData) {
        ...
        if (isset($vehData)) {
            ...
            //Enter in Sqlite
            $objVehDetail->customerNo = speedConstants::CUSTNO_DELEX;
...
function ParseData($vehData) {
    ...
    $objVehDetail->vehNo = str_replace(" ", "", str_replace(" ", "", $vehData->vehicleregnumber));
...
function InsertDataInSqlite($vehData, $objVehDetail) {
    if ($objVehDetail->unitNo != "") {
        ...
        $db = "sqlite:" . RELATIVE_PATH_DOTS . "customer/" . speedConstants::CUSTNO_DELEX . "/unitno/" . $objVehDetail->unitNo . "/sqlite/" . $date . ".sqlite";
        // echo $db; die;
