...
* Requirment is specially done for Kutwal Foods (795)
* This cron will update the tripstatusid column from tripdetails table as follows with condition
* if vehicle moves out of the is respective checkpoint then tripstatusid = 4 (Enroute) 
* and 
* if vehicle moves inside the is respective checkpoint then tripstatusid = 9 (Unloading Start)
* 
* the purpose this cron is to update the tripstatusid and display distributor(consignee) vehicles which left the checkpoint (loading location)
*/
...
$arrCustomers = array("795");
$userId = 8604;
$customerList = implode(",", $arrCustomers);
/* $chkpts = $cm->getalldeviceswithchkpforcron($customerExceptionList, $customerList); */
$chkpts = $cm->getAllDevicesForTripDetails($customerExceptionList, $customerList);
$arrayDataToUpdateStatus4 = [];
$arrayDataToUpdateStatus9 = [];
    ...
    foreach ($chkpts as $thischkpt) {
        //if ($thischkpt->vehicleid == 14894) {
            ...
            if ($distance >= $crad && $thischkpt->conflictstatus == 0) {
                ...
                $objCustomer->updateTripStatus($thischkpt->customerno,$thischkpt->vehicleid,4);
                // Insert in Queue
               /*  $cqm = new ComQueueManager();
                ...
                $cvo = new VOComQueue(); */
                $objCustomer = new CustomerManager();
                $objCustomer->updateTripStatus($thischkpt->customerno,$thischkpt->vehicleid,9);
               /*  $cvo->customerno = $thischkpt->customerno;
                $cvo->lat = $thischkpt->devicelat;
                $cvo->long = $thischkpt->devicelong;
