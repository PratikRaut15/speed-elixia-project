...
function ProcessData($arrLatestVehData) {
    $objVehicleManager = new VehicleManager(speedConstants::CUSTNO_SARTAJ_ALI);
	...
	if(isset($arrLatestVehData) && !empty($arrLatestVehData) && is_array($arrLatestVehData)){
		foreach ($arrLatestVehData as $arrVehData) {
			...
			if (isset($vehData)) {
                ...
                //Enter in Sqlite
                $objVehDetail->customerNo = speedConstants::CUSTNO_SARTAJ_ALI;
...
function ParseData($vehData) {
    ...
    /*$pos = strpos($vehData->Vehicle_No, "-");
    $rest = substr($vehData->Vehicle_No, $pos+1);*/
    ...
    $objVehDetail->odometer = isset($vehData->distance) ? ROUND(($vehData->distance * 1000),0) : 0;
...
function InsertDataInSqlite($vehData, $objVehDetail) {
    if ($objVehDetail->unitNo != "") {
        ...
        $db = "sqlite:" . RELATIVE_PATH_DOTS . "customer/" . speedConstants::CUSTNO_SARTAJ_ALI . "/unitno/" . $objVehDetail->unitNo . "/sqlite/" . $date . ".sqlite";
