...
foreach ($geoTackUsers as $user) {
        ...
        $objVendorDetails->customerNo = speedConstants::CUSTNO_ARYAVEER;
...
function ProcessData($arrLatestVehData) {
    $objVehicleManager = new VehicleManager(speedConstants::CUSTNO_ARYAVEER);
    ...
    if(isset($arrLatestVehData) && !empty($arrLatestVehData) && is_array($arrLatestVehData)){
        foreach ($arrLatestVehData as $arrVehData) {
            ...
            if (isset($vehData)) {
                ...
                //Enter in Sqlite
                $objVehDetail->customerNo = speedConstants::CUSTNO_ARYAVEER;
