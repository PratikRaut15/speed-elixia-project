...
/*http://www.php-ease.com/classes/sqlite.html
...
class SQLite {
    ...
    public function __construct($database = '', $version = 3) {
        ...
        if (empty($database)) {
            ...
            $path = BASE . 'sqlite/' . $database . '.sqlite';
    ...
    public function create($table, $columns, $changes = array()) {
        ...
        // the table has already been created in it's requested state
        if ($executed) {
            // then this table is being altered in some way
    ...
    public function statement($query, $values, $type = '') {
        ...
        if ($type == 'select') {
            // returns true or false, and sets $this->result
            $this->result = $stmt;
            $rows = true;
        }
        unset($stmt);
        ...
        switch ($action) {
            case 'create':
                ...
                foreach ($fields as $field) {
                    $columns[$field] = 'TEXT NOT NULL DEFAULT ""';
                }
            ...
            case 'search':
                ...
                break;
            case 'register':
                // a private option, but help yourself
    ...
    private function fulltext($search, $string) {
        ...
        if ($search_count == 0 || $string_count == 0) {
            return $result;
        }
    ...
    private function alter($table, $columns, $changes = array()) {
        // used in $this->create()
        $this->query("SELECT * FROM {$table} LIMIT 1");
        $row = $this->fetch('assoc');
        $map = array();
        foreach ($changes as $old => $new) {
            if (isset($columns[$new]) && isset($row[$old])) {
                $map[$old] = $new;
            }
            // legitimate changes
        }
        foreach ($row as $key => $value) {
            if (isset($columns[$key]) && !isset($map[$key])) {
                $map[$key] = $key;
            }
            // old fields that match the new
        }
        $results = array();
