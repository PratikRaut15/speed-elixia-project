...
class api {
        ...
        if ($validation['status'] == "successful") {
            ...
            if ($initday > 0) {
                ...
                while ($row = $this->db->fetch_array($record)) {
                    ...
                    if($row['humidity']!=0) {
                        $temp_coversion->rawtemp = $row['analog' . $row['humidity']];
                        $temp2 = getTempUtil($temp_coversion);
    ...
    function summary_wh($userkey, $vehicleid, $date) {
        ...
        if ($validation['status'] == "successful") {
            ...
            $analog_humidity = isset($row['humidity']) ? $row['humidity'] : 0;
...
//                if ($useHumidity == 1) {
                ...
                //                } else {
                $Data = $this->DataForTemprature($path, $vehicle->deviceid, $stime, $etime, $interval, $temp_sensors, $useHumidity, $analog_humidity);
                ...
                if (isset($Data) && count($Data) > 0) {
                    ...
                    foreach ($Data as $datarow) {
                        ...
                        if ($useHumidity == '1') {
                            if ($datarow->humidity == 'Not Active') {
                                $reportarr['Humidity'] = $datarow->humidity;
                            } else {
                                $reportarr['Humidity'] = $datarow->humidity . " %";
                            }
                        }
                        if ($datarow->temp1 == 'Not Active') {
                            $reportarr['Temperature1'] = $datarow->temp1;
                            $temp1_arr[] = 1;
                        } else {
                            $reportarr['Temperature1'] = $datarow->temp1 . " &degC";
                        }
                        if ($datarow->temp2 == 'Not Active') {
                            $reportarr['Temperature2'] = $datarow->temp2;
                            $temp2_arr[] = 1;
                        } else {
                            $reportarr['Temperature2'] = $datarow->temp2 . " &degC";
                        }
                        if ($datarow->temp3 == 'Not Active') {
                            $reportarr['Temperature3'] = $datarow->temp3;
                            $temp3_arr[] = 1;
                        } else {
                            $reportarr['Temperature3'] = $datarow->temp3 . " &degC";
                        }
                        if ($datarow->temp4 == 'Not Active') {
                            $reportarr['Temperature4'] = $datarow->temp4;
                            $temp4_arr[] = 1;
                        } else {
                            $reportarr['Temperature4'] = $datarow->temp4 . " &degC";
                        }
                        $report[] = $reportarr;
                    }
    ...
    function get_otp_forgotpwd($username) {
        ...
        if ($isUserExists) {
            ...
            if ($otpparam == -1) {
                ...
                if (!empty($phone)) {
                    $smscount = $customermanager->getSMSStatus($customerno, $userid, 0);
                    if ($smscount == 0) {
                        $response = '';
...
//                echo $EndTime.'_'.$StartTime.'<br>';
    ...
    function DataForTemprature($path, $deviceid, $startdate, $enddate, $interval, $temp_sensors, $useHumidity = null, $analog_humidity = null) {
        $devices = array();
        $dbs = new PDO($path);
        $query = "SELECT devicehistory.ignition, devicehistory.lastupdated, devicehistory.devicelat, devicehistory.devicelong, unithistory.vehicleid,unithistory.digitalio,unithistory.analog1, unithistory.analog2, unithistory.analog3, unithistory.analog4,vehiclehistory.vehicleno
            from devicehistory
        ...
        if (isset($result) && !empty($result)) {
            foreach ($result as $row) {
                ...
                if (round(abs(strtotime($row['lastupdated']) - strtotime($lastupdated)) / 60, 2) >= $interval) {
                    ...
                    case 4:
                        if ($row['analog4'] != '0') {
                            $temp4 = $this->getDigitalTemp($row['analog4']);
                        }
                    case 3:
                        if ($row['analog3'] != '0') {
                            $temp3 = $this->getDigitalTemp($row['analog3']);
                        }
                    case 2:
                        if ($row['analog2'] != '0') {
                            $temp2 = $this->getDigitalTemp($row['analog2']);
                        }
                    case 1:
                        if ($row['analog1'] != '0') {
                            $temp1 = $this->getDigitalTemp($row['analog1']);
                        }
                    }
                    if (isset($useHumidity) && $useHumidity == '1') {
                        $analog = "analog" . $analog_humidity;
                        $humidity = $this->gethumidity($row[$analog]);
                    }
                    /*
                    if ($row['analog4'] != '0') {
                    $humidity = $this->getDigitalTemp($row['analog4']);
    ...
    function multiauthRequest($userid, $customerno) {
        ...
        if (!empty($getuser)) {
            $response = '';
            $smsstatus = $cm->getSMSStatus($customerno, $userid, 0);
            if ($smsstatus == 0) {
