...
class reports {
    ...
    function gettempreport($customerno, $STdate, $EDdate, $deviceid, $vehicleno, $interval, $stime, $etime, $switchto, $mail_type, $vgroupname = null) {
        ...
        if ($days != NULL && count($days) > 0) {
            ...
            $finalreport .= "<hr /><br/><br/><div style='align:center;'>
            ...
            //echo "<pre>";print_r($veh_temp_details);exit;
            $finalreport .= $this->create_temp_from_report1($days, $unit, $customerno, $veh_temp_details);
        }
    ...
    function create_temp_from_report1($datarows, $vehicle, $custID = NULL, $veh_temp_details = null) {
        //echo "<pre>"; print_r($veh_temp_details); die();
        ...
        $mutedetails = $this->getunitmutedetails($vehicle['vehicleid'], $vehicle['uid'], $customerno);
        if (isset($datarows)) {
            foreach ($datarows as $change) {
                $comparedate = date('d-m-Y', strtotime($change->endtime));
                $today = date('d-m-Y', strtotime('Now'));
...
//        $t4 = getName_ByType($vehicle['n4']);
                ...
                $objTemp->use_humidity = $cm_details->use_humidity;
                $objTemp->switch_to = 3;
                switch ($cm_details->temp_sensors) {
                    case 4:
                        if ($vehicle['tempsen4'] != 0) {
                            $isTemperatureMuted = $this->checkTemperatureMute($mutedetails, 4, $change->lastupdated);
                            if ($isTemperatureMuted) {
                                ...
                                if ($change->$s4 != 0) {
                                    ...
                                    if ($temp4 == 0 || $temp4 > NORMAL_MAX_TEMP || $temp4 < NORMAL_MIN_TEMP) {
                                        ...
                                        $temp4_min = $this->set_summary_min_temp4($temp4);
                                        $temp4_max = $this->set_summary_max_temp4($temp4);
                    ...
                    case 3:
                        if ($vehicle['tempsen3'] != 0) {
                            $isTemperatureMuted = $this->checkTemperatureMute($mutedetails, 3, $change->lastupdated);
                            if ($isTemperatureMuted) {
                                $temp3 = 'Muted';
                                $temp3_mute_reading++;
                            } else {
                                $temp3_nonmute_reading++;
                                $s3 = "analog" . $vehicle['tempsen4'];
                                if ($change->$s3 != 0) {
                                    ...
                                    if ($temp3 == 0 || $temp3 > NORMAL_MAX_TEMP || $temp3 < NORMAL_MIN_TEMP) {
                                        ...
                                        $temp3_min = $this->set_summary_min_temp3($temp3);
                                        $temp3_max = $this->set_summary_max_temp3($temp3);
                    ...
                    case 2:
                        if ($vehicle['tempsen2'] != 0) {
                            $isTemperatureMuted = $this->checkTemperatureMute($mutedetails, 2, $change->lastupdated);
                            if ($isTemperatureMuted) {
                                $temp2 = 'Muted';
                                $temp2_mute_reading++;
                            } else {
                                ...
                                if ($change->$s2 != 0) {
                                    ...
                                    if ($temp2 == 0 || $temp2 > NORMAL_MAX_TEMP || $temp2 < NORMAL_MIN_TEMP) {
                                        ...
                                        $temp2_min = $this->set_summary_min_temp2($temp2);
                                        $temp2_max = $this->set_summary_max_temp2($temp2);
                    ...
                    case 1:
                        if ($vehicle['tempsen1'] != 0) {
                            $isTemperatureMuted = $this->checkTemperatureMute($mutedetails, 1, $change->lastupdated);
                            if ($isTemperatureMuted) {
                                $temp1 = 'Muted';
                                $temp1_mute_reading++;
                            } else {
        ...
        $summary = "<table align='center' style='width: auto; font-size:13px; text-align:center;border-collapse:collapse; border:1px solid #000;'>
        <thead>
            <tr><td colspan='4' style='background-color:#CCCCCC;font-weight:bold;'>Statistics</td></tr>
        </thead>
        <tbody>
            <tr>$temphtml</tr>
        </tbody>
    ...
    function getunitdetailsfromdeviceid($deviceid, $customerno) {
        $Query = "SELECT unit.get_conversion,unit.uid, vehicle.vehicleid,vehicle.fuel_min_volt, vehicle.fuel_max_volt, vehicle.fuelcapacity,vehicle.max_voltage,unit.unitno, unit.tempsen1, unit.tempsen2,unit.tempsen3,unit.tempsen4,unit.n1,unit.n2, unit.n3,unit.n4,unit.humidity,unit.acsensor, unit.fuelsensor, unit.is_ac_opp,unit.get_conversion FROM unit
    ...
    function getunitdetailsfromvehid($vehicleid, $customerno) {
        $Query = "SELECT unit.get_conversion,vehicle.fuel_min_volt, vehicle.fuel_max_volt, vehicle.fuelcapacity,vehicle.max_voltage,unit.fuelsensor, vehicle.temp1_min, vehicle.temp1_max, vehicle.temp2_min, vehicle.temp2_max,vehicle.temp3_min, vehicle.temp3_max,vehicle.temp4_min, vehicle.temp4_max, unit.unitno, unit.tempsen1, unit.tempsen2, unit.tempsen3, unit.tempsen4 FROM unit INNER JOIN vehicle ON vehicle.vehicleid = unit.vehicleid
    ...
    function get_min_max_temp($tempselect, $return, $temp_sensors = null) {
        $sess_temp_sensors = ($temp_sensors != null) ? $temp_sensors : '';
...
<tbody>
    ...
    function save_xls($full_path, $content) {
        ...
        require_once $RELATIVE_PATH_DOTS . 'lib/bo/simple_html_dom.php';
    ...
    public function getunitmutedetails($vehicleid, $uid, $customerno) {
        $mutearr = array();
        $Query = 'SELECT
            muteid,
            vehicleid,
