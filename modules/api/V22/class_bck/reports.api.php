...
class reports {
    ...
    function create_temp_from_report($datarows, $vehicle, $custID = NULL, $veh_temp_details = null) {
        ...
        if (isset($vehicle->kind) && $vehicle->kind == 'Warehouse') {
            $temp_conversion->switch_to = 3;
        }
        if (isset($vehicle->use_humidity)) {
            $temp_conversion->use_humidity = $vehicle->use_humidity;
        }
        if (isset($datarows)) {
    ...
    function getunitdetailsfromdeviceid($deviceid, $customerno) {
        ...
        $Query = "SELECT vehicle.fuel_min_volt, vehicle.fuel_max_volt, vehicle.fuelcapacity,vehicle.max_voltage,vehicle.kind
            ,unit.unitno, unit.tempsen1, unit.tempsen2,unit.tempsen3,unit.tempsen4,unit.n1,unit.n2, unit.n3,unit.n4
            ,unit.humidity,unit.acsensor, unit.get_conversion,unit.fuelsensor, unit.is_ac_opp ,customer.use_humidity FROM unit
    ...
    function set_humidity_graph_data($updated_date, $unit_type = null, $unit, $analog1, $analog2, $analog3, $analog4, $only_date = false) {
...
<tbody>
    ...
    public function GetReport($objReqDetails) {
        $arrRowData = array(); //$days = array();
        $arrFormattedData = array();
        $daysForReport = $this->gendays($objReqDetails->startDate, $objReqDetails->endDate);
        $finalreport = '';
        $unitDetails = $this->GetUnitDetailsByVehicleId($objReqDetails->vehicleId, $objReqDetails->customerNo);
        $unitDetails = (array) $unitDetails;

        $customerDetails = $this->getcustomerdetail_byid($objReqDetails->customerNo);

        $t1 = $this->getNameForTemp($unitDetails['n1'], $objReqDetails->customerNo);
        if ($t1 == '') {
            $t1 = "Temperature 1";
            $temperatureColumns['Temperature1'] = $t1;
        }
        $t2 = $this->getNameForTemp($unitDetails['n2'], $objReqDetails->customerNo);
        if ($t2 == '') {
            $t2 = "Temperature 2";
            $temperatureColumns['Temperature2'] = $t2;
        }
        $t3 = $this->getNameForTemp($unitDetails['n3'], $objReqDetails->customerNo);
        if ($t3 == '') {
            $t3 = "Temperature 3";
            $temperatureColumns['Temperature3'] = $t3;
        }
        $t4 = $this->getNameForTemp($unitDetails['n4'], $objReqDetails->customerNo);
        if ($t4 == '') {
            $t4 = "Temperature 4";
            $temperatureColumns['Temperature4'] = $t4;
        }
        $totalTemperatureSensors = $customerDetails->temp_sensors;
        //print_r($daysForReport);
        if (isset($daysForReport)) {
            foreach ($daysForReport as $reportDate) {
                $arrData = null;
                $sqlitePath = "../../../customer/" . $objReqDetails->customerNo . "/unitno/" . $unitDetails['unitno'] . "/sqlite/" . $reportDate . ".sqlite";
                if (file_exists($sqlitePath)) {
                    $sqlitePath = "sqlite:" . $sqlitePath;
                    $objReqDetails->sqlitePath = $sqlitePath;
                    $arrData = $this->getReportDataFromSqlite($objReqDetails);
                } else {
                    $arrFormattedData['ReportStatus'] = "Data Not Available";
                }
                //print_r($arrData);die();
                if (count($arrData) > 0) {
                    $arrFormattedData[$reportDate] = $this->GetFormattedData($objReqDetails, $arrData, $unitDetails, $customerDetails);
                }
            }
        }
        $arrFormattedData = array("TemperatureSensors" => $totalTemperatureSensors, "TemperatureColumns" => $temperatureColumns, "ReportData" => $arrFormattedData);
        return $arrFormattedData;
    }

    public function getReportDataFromSqlite($objReqDetails) {

        //$location, $deviceid, $interval, $startdate, $enddate, $graph = false, $unit = null, $customerno = null;
        $reportData = array();
        ...
        $Query = "SELECT $columnsForSelection
            FROM vehiclehistory as v
            INNER JOIN devicehistory as d ON d.lastupdated = v.lastupdated
            INNER JOIN unithistory as u ON u.lastupdated = v.lastupdated
            WHERE v.lastupdated BETWEEN '$objReqDetails->reportStartDate' AND '$objReqDetails->reportEndDate'
            AND v.vehicleid= $objReqDetails->vehicleId
            AND d.status!='F'
            GROUP BY v.lastupdated ORDER BY v.lastupdated ASC";
        try {
            $database = new PDO($objReqDetails->sqlitePath);
            //$query1 = $query . " ORDER BY devicehistory.lastupdated ASC";

            $arrSqliteData = $database->query($Query);

            if (isset($arrSqliteData) && $arrSqliteData != "") {
                foreach ($arrSqliteData as $row) {
                    if (!isset($lastupdated)) {
                        $lastupdated = $row['lastupdated'];
                    }
                    if (round(abs(strtotime($row['lastupdated']) - strtotime($lastupdated)) / 60, 2) >= $objReqDetails->interval) {
                        $objVehicle = new stdClass();
                        $objVehicle->odometer = $row['odometer'];
                        $objVehicle->speed = $row['curspeed'];
                        $objVehicle->lastUpdated = $row['lastupdated'];
                        $objVehicle->deviceLat = $row['devicelat'];
                        $objVehicle->devicelng = $row['devicelong'];
                        $objVehicle->ignition = $row['ignition'];
                        $objVehicle->digitalIO = $row['digitalio'];
                        $objVehicle->analog1 = $row['analog1'];
                        $objVehicle->analog2 = $row['analog2'];
                        $objVehicle->analog3 = $row['analog3'];
                        $objVehicle->analog4 = $row['analog4'];
                        $reportData[] = $objVehicle;
                        $lastupdated = $row['lastupdated'];
                    }
                }
            }
    ...
    public function GetUnitDetailsByVehicleId($vehicleId, $customerNo) {
        $Query = "SELECT
            unit.unitno,
            unit.fuelsensor,
            unit.tempsen1,
            ...
            INNER JOIN vehicle ON vehicle.vehicleid = unit.vehicleid
            INNER JOIN devices ON devices.uid = unit.uid
            WHERE unit.customerno = '$customerNo'
            AND unit.vehicleid = '$vehicleId'";
        $record = $this->db->query($Query, __FILE__, __LINE__);
        while ($row = $this->db->fetch_array($record)) {
            $unit = new stdClass();
            $unit->unitno = $row['unitno'];
    ...
    public function GetFormattedData($objReqDetails, $arrRowData, $unitDetails, $customerDetails) {
        $i = 1;
        $tableRow = 0;
        $reportData = array();
        $formattedData = array();
        $temperatureColumns = array();
        $dateWiseDate = array();
        $tempAboveMax1 = $tempAboveMax2 = $tempAboveMax3 = $tempAboveMax4 = 0;
        $lastdate = null;
        $display = '';
        $unitDetails = (array) $unitDetails;
        $tempconversion = new TempConversion();
        ...
        if (isset($arrRowData)) {
            ...
            foreach ($arrRowData as $data) {
                //prettyPrint($data);die();
                $objformattedData = new stdClass();
                $temperature1 = $temperature2 = $temperature3 = $temperature4 = "Not Active";
                $objformattedData->speed = $data->speed;
                $objformattedData->lastUpdated = $data->lastUpdated;
                $objformattedData->location = $this->get_location_bylatlong($data->deviceLat, $data->devicelng, $objReqDetails->customerNo);
                switch ($customerDetails->temp_sensors) {
                    case 4:
                        $sensor = "analog" . $unitDetails['tempsen4'];
                        if ($unitDetails['tempsen4'] != 0 && $data->$sensor != 0) {
                            $tempconversion->rawtemp = $data->$sensor;
                            $temperature4 = getTempUtil($tempconversion) . " &deg; C";
                        } else {
                            $temperature4 = "-";
                        }
                        $objformattedData->temperature4 = $temperature4;
                    case 3:
                        $sensor = "analog" . $unitDetails['tempsen3'];
                        if ($unitDetails['tempsen3'] != 0 && $data->$sensor != 0) {
                            $tempconversion->rawtemp = $data->$sensor;
                            $temperature3 = getTempUtil($tempconversion) . " &deg; C";
                        } else {
                            $temperature3 = "-";
                        }
                        $objformattedData->temperature3 = $temperature3;
                    case 2:
                        $sensor = "analog" . $unitDetails['tempsen2'];
                        if ($unitDetails['tempsen2'] != 0 && $data->$sensor != 0) {
                            $tempconversion->rawtemp = $data->$sensor;
                            $temperature2 = getTempUtil($tempconversion) . " &deg; C";
                        } else {
                            $temperature2 = "-";
                        }
                        $objformattedData->temperature2 = $temperature2;
                    case 1:
                        $sensor = "analog" . $unitDetails['tempsen1'];
                        if ($unitDetails['tempsen1'] != 0 && $data->$sensor != 0) {
                            $tempconversion->rawtemp = $data->$sensor;
                            $temperature1 = getTempUtil($tempconversion) . " &deg; C";
                        } else {
                            $temperature1 = "-";
                        }
                        $objformattedData->temperature1 = $temperature1;
                }
                $formattedData[] = $objformattedData;
            }
