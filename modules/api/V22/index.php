...
if ($action == "pulldetails") {
    if (isset($userkey)) {
        ...
        $groupid = (isset($groupid) && !empty($groupid)) ? $groupid : 0;
        $isRequiredThirdParty = isset($isRequiredThirdParty) ? $isRequiredThirdParty : 0;
        $isClientCode = isset($isClientCode) ? $isClientCode : 0;

        $test = $apiobj->device_list($userkey, $pageIndex, $pageSize, $searchstring, $groupid, $isRequiredThirdParty, $isClientCode);
...
if ($action == "pullroutehistory") {
    if (isset($userkey) && isset($vehicleid) && isset($sdate) && isset($stime) && isset($edate) && isset($etime) && isset($reporttype) && isset($overspeed) && isset($holdtime)) {
        ...
        $isClientCode = isset($isClientCode) ? $isClientCode : 0;
        if ($isClientCode) {
            $validation = $apiobj->checkClientCode($userkey);
...
if ($action == "pullCheckPoints") {
    ...
    if (isset($inputData['userkey'])) {
        ...
        $getObjectData->isClientCode = isset($inputData['isClientCode']) ? $inputData['isClientCode'] : 0;
