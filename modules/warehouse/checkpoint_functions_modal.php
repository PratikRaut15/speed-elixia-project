...
function addchk($form)
{
    ...
    $eta = GetSafeValueString($form["STime"], "string");
    $checkpoint->eta = date('H:i:s', strtotime($eta));
    if(isset($_POST['crad']))
...
function addchktovehicle($form)
{
    ...
    foreach($form as $single_post_name=>$single_post_value)
    {
        ...
        else if (substr($single_post_name, 0, 15) == "to_added_chkpt_")
            $checkptss[] = substr($single_post_name, 15, 16);
    }
    $checkpts = $checkptss;
    //print_r($checkpts);
