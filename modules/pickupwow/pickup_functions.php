...
require_once $Mpath . '../../lib/system/DatabasePickupwowManager.php';
...
function editorder($pickupboyid, $oid, $status, $timeslotid, $awbno) {
...
//function addpickup($vendor, $maping) {   //pinn mapping remove
//    $pickup = new Pickup($_SESSION['customerno'], $_SESSION['userid']);
//    $responce = $pickup->add_pickup($vendor, $maping);
//    return $responce;
//}
function addpickup($vendor) {
    $pickup = new Pickup($_SESSION['customerno'], $_SESSION['userid']);
    $responce = $pickup->add_pickup($vendor);
    return $responce;
}

function editpickup($vendor) {
    $pickup = new Pickup($_SESSION['customerno'], $_SESSION['userid']);
    $responce = $pickup->edit_pickup($vendor);
    return $responce;
}

function getpickupallstatus() {
    $pkpmanager = new Pickup($_SESSION['customerno'], $_SESSION['userid']);
    $pkp = $pkpmanager->getallpickupstatus();
    return $pkp;
}

function getpickupslots() {
    $pkpmanager = new Pickup($_SESSION['customerno'], $_SESSION['userid']);
    $pkp = $pkpmanager->getallpickupslotstime();
    return $pkp;
}
...
function getshipping_details($oid, $userid) {
    $pkpmanager = new Pickup($_SESSION['customerno'], $_SESSION['userid']);
    $shdetails = $pkpmanager->getshipping_details($oid, $userid);
    return $shdetails;
}
...
function upload_checkpoint($all_form) {
    ...
    foreach ($all_form as $form) {
        ...
        $checkpoint->fulfillmentid = GetSafeValueString($form["fulfillmentid"], "String");
        $checkpoint->awbno = GetSafeValueString($form["awbno"], "String");
        $checkpoint->shipperid = GetSafeValueString($form["shipperid"], "String");
        $checkpoint->pickupdate = date_maker($form["pickupdate"]);
    ...
    if (isset($datarows)) {
        ...
        <table  id='search_table_2' style="width: 1120px; font-size:12px; text-align:center;border-collapse:collapse; border:1px solid #000;">
            <tr style="background-color:#CCCCCC;font-weight:bold;">
                ...
                <td style='width:50px;height:auto; text-align: center;'>Order ID</td>
                <td style='width:100px;height:auto; text-align: center;'>AWB No</td>
                <td style='width:50px;height:auto; text-align: center;'>Time Slot</td>
                <td style='width:50px;height:auto; text-align: center;'>Pickup Boy</td>
                <td style='width:300px;height:auto; text-align: center;'>Pickup Date</td>
...
if (isset($change->awbno)) {
            ...
            ?> </td>
                    <td style='width:50px;height:auto; text-align: center;'> <?php
if (isset($change->timeslot)) {
                echo $change->timeslot;
            }
            ?> </td>
                    <td style='width:50px;height:auto; text-align: center;'> <?php
if (isset($change->realname)) {
            ...
            ?> </td>
                    <td style='width:50px;height:auto; text-align: center;'> <?php
if (isset($change->pickupdate)) {
                echo date('d-m-Y', strtotime($change->pickupdate));
            }
            ?> </td>
                    <td style='width:50px;height:auto; text-align: center;'> <?php
if (isset($change->trackingstatusname)) {
                echo $change->trackingstatusname;
            }
            ?> </td>
                </tr>
                <?php
...
function OrderPadding($orderid) {
    ...
    $orderpadding = "WN" . $value;
    return $orderpadding;
}

function OrderUnPadding($awbno) {
    $input = $awbno;
    $value = str_replace("WN", "", $input);
    $orderid = ltrim($value, '0');
    return $orderid;
}
