...
if ($report_name == 'stoppageMail') {
    ...
    if (!isset($_REQUEST['vehicleid']) && isset($_REQUEST['groupid'])) {
        ...
        if (!empty($devices)) {
            ...
            if ($mail_type === 'pdf') {
                foreach ($devices AS $device) {
                    $cat .= gettempreportpdf($cust_no, $_REQUEST['sdate'], $_REQUEST['edate'], $device->deviceid, $device->vehicleno, $_REQUEST['interval'], $_REQUEST['stime'], $_REQUEST['etime'], $_REQUEST['switchto'], null, 'pdf');
                }
                if ($cat != '') {
                    echo $cat;
                } else {
                ...
                foreach ($devices AS $device) {
                    $cat .= gettempreportpdf($cust_no, $_REQUEST['sdate'], $_REQUEST['edate'], $device->deviceid, $device->vehicleno, $_REQUEST['interval'], $_REQUEST['stime'], $_REQUEST['etime'], $_REQUEST['switchto'], null, 'xls');
                }
                if ($cat != '') {
                    echo $cat;
                } else {
        ...
        if ($mail_type === 'pdf') {
            ...
            echo gettempreportpdf($cust_no, $_REQUEST['sdate'], $_REQUEST['edate'], $_REQUEST['vehicleid'], $_REQUEST['vehicleno'], $_REQUEST['interval'], $_REQUEST['stime'], $_REQUEST['etime'], $_REQUEST['switchto'], null, 'xls');
        }
    }
} elseif ($report_name == 'overspeedMail') {
    require_once 'reports_overspeed_functions.php';
...
function get_save_send($file_end, $subject, $mail_content) {
    ...
    if (!isset($_REQUEST['vehicleid']) && isset($_REQUEST['groupname'])) {
        $veh_no = isset($_REQUEST['groupname']) ? str_replace(' ', '', $_REQUEST['groupname']) : '';
    } else {
