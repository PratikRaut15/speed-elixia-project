...
if(isset($_GET["devicekey"]))
{
    ...
    if(isset($device->customerno) && $device->customerno != "0")
    {
        ...
        $jsonstatus->freqdata = $rm->getfreqdata($device->customerno);        
        $jsonstatus->devicekey = $device->devicekey; 
        $jsonstatus->usecreditcard = $device->usecreditcard;         
        $jsonstatus->useforms = $device->useforms;                 
        $jsonstatus->usecheque = $device->usecheque;                 
        $jsonstatus->usepartialpayment = $device->usepartialpayment;                         
        $jsonstatus->customercompany = $device->customercompany;         
        ...
        else
        {
            ...
            $jsonstatus->pushform = $controls->pushform;               
