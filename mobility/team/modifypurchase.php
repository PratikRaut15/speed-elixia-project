...
if(isset($_POST["save"])  )
{
    ...
    $sql = sprintf("UPDATE `purchase`
                    ...
                    `details` ='%s',
                    `approval` ='%d'                    
                    WHERE purchaseid=%d LIMIT 1",$imei,$model,$cost,$dealerid, $color, $details,$approve,$purchaseid);
    $db->executeQuery($sql);            
...
where p.purchaseid='%d' AND approval = 0",$purchaseid);
$db->executeQuery($sql);
...
<form method="post" id="form1" action="modifypurchase.php">
<input type="hidden" name = "pid" value="<?php echo($purchaseid) ?>"/>
<table width="100%">
    <tr>
    <td>IMEI</td><td><input name = "imei" id="imei" type="text" onkeyup="showcreatepurchase();" value="<?php echo($imei); ?>"></td>
    </tr>
    <tr>
    <td>Model</td>
    <td>
    ...
    <tr>
    <td>Actual Cost (in Rs)</td><td><input name = "cost" id="cost" type="text" size="7" maxlength="7" onkeypress='validate(event)' value="<?php echo($cost); ?>"></td>
    </tr>        
    <tr>
    ...
    ?>
    <tr>
    <td>Approve?</td><td><input name = "approve" id="approve" type="checkbox"></td>
    </tr>                
    <?php
    }
    ?>
