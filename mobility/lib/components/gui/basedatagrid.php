        ...
        foreach( $this->_fields as $thisfield )
        {
            ...
            else
            {
                if(!isset($this->_controls[ $thisfield]->id) )
                {
                    ...
                    {
                        echo("<td><font size='2'>");
                        if($row->status == "Unread" && $row->sentmessage == "messages_notsent")
                        {
                            echo("<b>");
                        }                        
                        echo($fieldval); 
        ...
        if(isset($this->_rightactions) && count($this->_rightactions)>0)
        {
            ...
            foreach( $this->_rightactions as $thisaction )
            {
                ...
                if($allowcontrol)
                {
                    ...
                    echo("<a target='_blank' class=\"imagebutton\" href=\"". $toolurl . "\" " . $onclick . "><img class=\"clickimage\" src=\"" . $toolicon . "\" title=\"". $thisaction ."\"  ></a>");
                }
                $i++;
            }
            echo("</td>");
