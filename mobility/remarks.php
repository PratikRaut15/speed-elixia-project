...
require_once("class/class.remarks.php");
...
<?php 
	...
	switch($index)
	{
		case 'add' :
						if(@$submit=="Submit")
						$productinfo->addremark('server');
						else
						$productinfo->addremark('local');	
			break;
		
		case 'search' :
						
						$productinfo->SearchRecord($remarkname);
			break;	
		
		case 'edit' :
						if(@$submit=="Submit")
						$productinfo->editclient('server',$remarkid);
						else
						$productinfo->editclient('local',$remarkid);	
			break;	
		
		case 'delete' :
					$productinfo->deleteStudent($remarkid);	
			break;
			
		
		
		case 'View' :
					$productinfo->RemarksView($remarkid);	
			break;	
							
		case 'viewAll' :
					$productinfo->showAllRemarksfInfo();	
			break;
		default :			
					$productinfo->showAllRemarksfInfo();	
			break;		
