...
<?php 
	...
	switch($index)
	{
		...
		case 'search' :
						...
						$clientinfo->SearchRecord($clientname,$phoneno,$branch);
			break;	
		
		case 'edit' :
						if(@$submit=="Submit")
						$clientinfo->editclient('server',$id);
						else
						$clientinfo->editclient('local',$id);	
			break;	
		
		case 'delete' :
					$clientinfo->deleteStudent($id);	
			break;
			
		
		
		case 'View' :
					$clientinfo->ClientView($id);	
			break;	
