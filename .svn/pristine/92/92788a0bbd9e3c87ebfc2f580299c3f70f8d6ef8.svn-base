...
// Handler for .ready() called.
    ...
    if (transidparam == 2) {
        ...
        $("#btnApprove").click(function () {
            ...
            var htmldata = "You are About to close " + count_id + " Transactions";
...
function push_closed_transaction(action) {
    ...
    if (notes == '') {
        ...
        var dataString = "work=multiple_approval&action=" + action + "&approvaldata=" + approval_array + "&note=" + notes+"&ofasnumber="+ofasnumber;
        jQuery.ajax({
            url: 'route_ajax.php',
            type: 'POST',
            data: dataString,
...
function addPart() {
    ...
    jQuery.ajax({
        url: 'route_ajax.php',
        type: 'POST',
        data: "action=addpart&partname=" + pname,
...
function addTask() {
    ...
    jQuery.ajax({
        url: 'route_ajax.php',
        type: 'POST',
        data: "action=addtask&taskname=" + tname,
        success: function (res) {
            var data = jQuery.parseJSON(res);
            if (data[0] == 1) {
...
function activetextbox()
{
    if (jQuery("#rf").attr("checked"))
    {
        ...
        jQuery("#nright_front").attr("readonly", false);
    }

    if (jQuery("#rb_out").attr("checked"))
    {

        jQuery("#nright_back_out").attr("readonly", false);
    }
    if (jQuery("#rb_in").attr("checked"))
    {

        jQuery("#nright_back_in").attr("readonly", false);
    }
    if (jQuery("#lb_out").attr("checked"))
    {

        jQuery("#nleft_back_out").attr("readonly", false);
    }
    if (jQuery("#lf").attr("checked"))
    {

        jQuery("#nleft_front").attr("readonly", false);
    }
    if (jQuery("#lb_in").attr("checked"))
    {
        jQuery("#nleft_back_in").attr("readonly", false);
    }
    if (jQuery("#st").attr("checked"))
    {
        jQuery("#nstepney").attr("readonly", false);
    }
    if (jQuery("#newbatt_chkbox").attr("checked")) {
        jQuery("#new_battsrno").attr("readonly", false);
    }
}
function push_transaction_by_category() {
    ...
    /**
     else if(category_id == 5 && jQuery("#accessory_select_1").val() != "-1" && (parseInt(jQuery("#amount1").val()) > parseInt(jQuery("#max_amount_hid_1").val())))
     {
     jQuery("#max_perm_amount_error").show();
     jQuery("#max_perm_amount_error").fadeOut(6000);        
     }
     else if(category_id == 5 && jQuery("#accessory_select_2").val() != "-1" && (parseInt(jQuery("#amount2").val()) > parseInt(jQuery("#max_amount_hid_2").val())))
     {
     jQuery("#max_perm_amount_error").show();
     jQuery("#max_perm_amount_error").fadeOut(6000);        
     }
     else if(category_id == 5 && jQuery("#accessory_select_3").val() != "-1" && (parseInt(jQuery("#amount3").val()) > parseInt(jQuery("#max_amount_hid_3").val())))
     {
     jQuery("#max_perm_amount_error").show();
     jQuery("#max_perm_amount_error").fadeOut(6000);        
     }
     else if(category_id == 5 && jQuery("#accessory_select_4").val() != "-1" && (parseInt(jQuery("#amount4").val()) > parseInt(jQuery("#max_amount_hid_4").val())))
     {
     jQuery("#max_perm_amount_error").show();
     jQuery("#max_perm_amount_error").fadeOut(6000);        
     }
     else if(category_id == 5 && jQuery("#accessory_select_5").val() != "-1" && (parseInt(jQuery("#amount5").val()) > parseInt(jQuery("#max_amount_hid_5").val())))
     {
     jQuery("#max_perm_amount_error").show();
     jQuery("#max_perm_amount_error").fadeOut(6000);        
     }
     else if(category_id == 5 && jQuery("#accessory_select_6").val() != "-1" && (parseInt(jQuery("#amount6").val()) > parseInt(jQuery("#max_amount_hid_6").val())))
     {
     jQuery("#max_perm_amount_error").show();
     jQuery("#max_perm_amount_error").fadeOut(6000);        
     }
     else if(category_id == 5 && jQuery("#accessory_select_7").val() != "-1" && (parseInt(jQuery("#amount7").val()) > parseInt(jQuery("#max_amount_hid_7").val())))
     {
     jQuery("#max_perm_amount_error").show();
     jQuery("#max_perm_amount_error").fadeOut(6000);        
     }**/
    ...
    if (typeof iserror !== 'undefined' && iserror != '1')
    {
        ...
        switch (category_id.toString()) {
            ...
            case '2':
                ...
                var tot = parseFloat(parts_total) + parseInt(tasks_total);
...
function editbattery(maintenanceid, vehicleid, statusid, categoryid, quote_amount)
{
    ...
    if (getin == true)
    {
        var data = jQuery('#getbattery_edit').serialize() + '&edit_batt_vehicleid=' + vehicleid + '&categoryid=' + categoryid + '&maintenanceid=' + maintenanceid + '&status_maintenance=' + statusid + '&new_battsrno=' + new_batt + '&file_invoice_name=' + files_invoice_name + get_string;
        jQuery.ajax({
            url: "route_ajax.php",
            type: 'POST',
            cache: false,
...
function showInvoiceDetails() {
    if (jQuery('#sent_pay').is(":checked"))
    {
        ...
        if ((category_id == 2 || category_id == 3)) {
            ...
            var tasks_total = 0;
            var inv_amt = 0;
            var tax = parseInt(jQuery("#p_tax").val());

            for (var i = 1; i < 16; i++) {
                if (jQuery("#parts_select_" + i + "").val() != '-1' && (jQuery("#parts_qty" + i + "").val() == '' || jQuery("#parts_amount" + i + "").val() == '' || jQuery("#parts_qty" + i + "").val() != '0' || jQuery("#parts_amount" + i + "").val() != '0')) {
                    parts_list.push(jQuery("#parts_select_" + i + "").val() + "-" + jQuery("#parts_amount" + i + "").val() + "-" + jQuery("#parts_qty" + i + "").val());
                    parts_total += jQuery("#parts_amount" + i + "").val() * jQuery("#parts_qty" + i + "").val();
                }
                if (jQuery("#tasks_select_" + i + "").val() != '-1' && (jQuery("#tasks_qty" + i + "").val() == '' || jQuery("#tasks_amount" + i + "").val() == '' || jQuery("#tasks_qty" + i + "").val() != '0' || jQuery("#tasks_amount" + i + "").val() != '0')) {
                    tasks_list.push(jQuery("#tasks_select_" + i + "").val() + "-" + jQuery("#tasks_amount" + i + "").val() + "-" + jQuery("#tasks_qty" + i + "").val());
                    tasks_total += jQuery("#tasks_amount" + i + "").val() * jQuery("#tasks_qty" + i + "").val();
                }
            }
            var tot = parseInt(parts_total) + parseInt(tasks_total);
            inv_amt = tot + tax;
