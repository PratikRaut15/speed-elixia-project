...
require_once "class/model/clsMuteTemperatureRequest.php";
    ...
    try {
        ...
        if (isset($objGetVehicleList->userkey) && $objGetVehicleList->userkey != "") {
            ...
            if (empty($arrVTSResult)) {
                ...
                $customerno = Sanitise::Long($arrVTSResult['customerno']);
                $userid = Sanitise::Long($arrVTSResult['userid']);
...
if ($action == "getvehicledata") {
    try {
        ...
        if (isset($objGetLatestVehicleRequest->userkey) && $objGetLatestVehicleRequest->userkey != "") {
            $objGetLatestVehicleRequest->pageindex = isset($objGetLatestVehicleRequest->pageindex) ? Sanitise::Long($objGetLatestVehicleRequest->pageindex) : 1;
            /* If pagesize is -1, it means we need to give all the records in a single page */
            $objGetLatestVehicleRequest->pagesize = isset($objGetLatestVehicleRequest->pagesize) ? Sanitise::Long($objGetLatestVehicleRequest->pagesize) : -1;
            /* If search string is set, it means we need to return all the records matching this string */
            $objGetLatestVehicleRequest->searchstring = isset($objGetLatestVehicleRequest->searchstring) ? Sanitise::String($objGetLatestVehicleRequest->searchstring) : '';
            $objGetLatestVehicleRequest->groupid = isset($objGetLatestVehicleRequest->groupid) ? Sanitise::Long($objGetLatestVehicleRequest->groupid) : 0;
            $objGetLatestVehicleRequest->iswarehouse = isset($objGetLatestVehicleRequest->iswarehouse) ? Sanitise::Long($objGetLatestVehicleRequest->iswarehouse) : 0;
            $objGetLatestVehicleRequest->isRequiredThirdParty = isset($objGetLatestVehicleRequest->isRequiredThirdParty) ? Sanitise::Long($objGetLatestVehicleRequest->isRequiredThirdParty) : 0;
            $objGetLatestVehicleRequest->isLocationEnabled = isset($objGetLatestVehicleRequest->isLocationEnabled) ? Sanitise::Long($objGetLatestVehicleRequest->isLocationEnabled) : 0;
...
if ($action == "muteVehicleTemperature") {
    try {
        $reqMuteVehTempCondition = json_decode($jsonreq, true);
        $reqMuteVehTempCondition = $objJsonMapper->map($reqMuteVehTempCondition, new clsMuteTemperatureRequest());
        if (isset($reqMuteVehTempCondition->userkey) && $reqMuteVehTempCondition->userkey != "") {
            $arrVTSResult = $apiobj->get_userdetails_by_key($reqMuteVehTempCondition->userkey);
            $status = 0;
            if (empty($arrVTSResult)) {
                $output = $apiobj->failure('No Userdata');
                //Log error in DB
                ...
                $validDays = $apiobj->checkValidity($customerno);
                if ($validDays > 0) {
                    $reqMuteVehTempCondition->customerNo = $customerno;
                    $reqMuteVehTempCondition->userId = $userid;

                    $status = $apiobj->setVehicleTemperatureMute($reqMuteVehTempCondition);
                    if ($status == 1) {
                        $message = "Vehicle temperature mute status changed";
                    } else {
                        $message = "Vehicle temperature mute status not changed";
                    }
                    $output = $apiobj->success($message, array($status));
                } else {
                    $output = $apiobj->failure("Device Validity expired.");
                }
            }
...
if ($action == "getLocation") {
    try {
        ...
        if (isset($request->userkey) && $request->userkey != "") {
            $arrVTSResult = $apiobj->get_userdetails_by_key($request->userkey);
            ...
            if (empty($arrVTSResult)) {
                ...
                if ($validDays > 0) {
                    ...
                    $arrResponse = $apiobj->getLocation($request);
                    if (isset($arrResponse) && !empty($arrResponse)) {
                        $message = "Available Records fetched successfully";
                        $output = $apiobj->success($message, $arrResponse);
                    } else {
                        $output = $apiobj->failure("Location not found.");
                    }
...
if ($action == "getTotalTravelDistance") {
    set_time_limit(90);
    include_once $RELATIVE_PATH_DOTS . 'modules/reports/reports_common_functions.php';
    $reqGetHistoricVehSummary = json_decode($jsonreq, true);
    $objVehSummaryReq = $objJsonMapper->map($reqGetHistoricVehSummary, new clsVehSummaryRequest());
    if (isset($objVehSummaryReq->userkey) && $objVehSummaryReq->userkey != "") {
        ...
        if (empty($arrVTSResult)) {
            ...
            if ($validDays > 0) {
                $objVehSummaryReq->customerNo = $customerno;
                $getdata = $apiobj->getTotalTravelDistance($objVehSummaryReq);
...
if ($action == "getVehicleRouteSummary") {
    try {
        $reqGetLatestVehDetails = json_decode($jsonreq, true);
        //print_r($jsonreq);
        //Log input request in DB
        ...
        //print_r($objGetLatestVehicleRequest);
        if (isset($objGetLatestVehicleRequest->userkey) 
                && $objGetLatestVehicleRequest->userkey != "" 
                && isset($objGetLatestVehicleRequest->vehicleid)
                && $objGetLatestVehicleRequest->vehicleid != '') {
            $arrVTSResult = $apiobj->get_userdetails_by_key($objGetLatestVehicleRequest->userkey);
            ...
            if (empty($arrVTSResult)) {
                ...
                if ($validDays > 0) {
                    $objGetLatestVehicleRequest->customerno = $customerno;
                    $getdata = $apiobj->getVehicleRouteSummary($objGetLatestVehicleRequest);
                    if (isset($getdata) && !empty($getdata)) {
                        $message = "Available Records fetched successfully";
                        $output = $apiobj->success($message, $getdata);
                    } else {
 ...
 if ($action == 'getRouteDistanceAndTime') {
     include_once $RELATIVE_PATH_DOTS . 'modules/reports/reports_common_functions.php';
    $objVehSummaryReq = json_decode($jsonreq);
    if (isset($objVehSummaryReq->userkey) && $objVehSummaryReq->userkey != "") {
        $arrVTSResult = $apiobj->get_userdetails_by_key($objVehSummaryReq->userkey);
        if (empty($arrVTSResult)) {
            $output = $apiobj->failure('No Userdata');
            ...
            if ($validDays > 0) {
                ...
                $objVehSummaryReq->customerNo = $customerno;
                $getdata = $apiobj->getRouteDistanceAndTime($objVehSummaryReq);
                if (!empty($getdata)) {
                    $message = "Available Records fetched successfully";
                    $output = $apiobj->success($message, $getdata);
                } else {
...
if($action == 'getCheckPointId' ){
	...
	if(!empty($userKey) && !empty($lattitude) && !empty($longitude) && !empty($query) && !empty($checkpointname) && !empty($radius)){
		$userKey = trim(sha1($userKey));
		$lattitude = trim($lattitude);
		$longitude = trim($longitude);
		$checkpointname = trim($checkpointname);
		$radius = trim($radius);
		$query = trim($query);
		$isUserValid = loginwithuserkey($userKey);
	...
	if(!empty($vehicleJson)){
		try{
			...
			if(!empty($data['userkey'])){
				$isUserValid = loginwithuserkey(sha1($data['userkey']));
				if($isUserValid){
					if(isset($_SESSION['customerno'])){
						$customerNo = $_SESSION['customerno'];
						$userid = $_SESSION['userid'];
						$vehicleId = isset($data['vehicleId']) ? $vehicleId = $data['vehicleId'] : $output = $apiobj->failure("Seems like you are playing around without any actions !!");
