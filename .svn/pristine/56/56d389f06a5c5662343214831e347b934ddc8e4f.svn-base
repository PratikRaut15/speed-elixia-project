...
<?php
$def_date = date('Y-m-d');
$allVehicles = getVehiclesForMapping();
$preset_slots = getMappedZoneSlot();
$allFence = getZones();
$allSlots = getSlots();
$fenceids = array();
?>
...
<div class="container-fluid">
    ...
    <table class='table mapT' id='vehTable' >
        ...
        foreach($allVehicles as $s_vehid=>$singleVeh){
            echo '<tr><td><span class="draggable" id="vi'.$s_vehid.'">'.$singleVeh['vehno'].' ('.$singleVeh['seatcapacity'].') </span></td></tr>';
        }
        ?>
        </tbody>
    </table>
