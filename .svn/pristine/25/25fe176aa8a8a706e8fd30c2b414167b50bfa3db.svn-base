...
<form action="reports.php?id=13" method="POST" onsubmit="getTempReport();
      ...
      $today = date('d-m-Y');
     // $test = $_SESSION["temp_sensors"] ;2;
      $temp_sensors = $_SESSION["temp_sensors"] ;
      include 'panels/tempExceptabular.php';
...
<?php
if($temp_sensors == 1){
    echo '<td><select id="tempsel" name="tempsel"><option value="1">Temperature 1</option>/select></td>';
}
  ...
  echo '<td><select id="tempsel" name="tempsel">
            <option value="1">Temperature 1</option>';
    echo '<option value="2">Temperature 2</option><option value=3>Temperature 3</option><option value=4>Temperature 4</option></select></td>';
}
else if($temp_sensors == 4){
  echo '<td><select id="tempsel" name="tempsel">
            <option value="1">Temperature 1</option>';
    echo '<option value="2">Temperature 2</option><option value=3>Temperature 3</option><option value=4>Temperature 4</option></select></td>';
}
?>
        ...
        <td>
            ...
            <a href='javascript:void(0)' onclick="get_pdfreportTempExcep(<?php echo $_SESSION['customerno']; ?>, <?php echo $_SESSION['switch_to']; ?>);"><img src="../../images/pdf_icon.png" alt="Export to PDF" class='exportIcons' title="Export to PDF" /></a>
            <a href='javascript:void(0)' onclick="html2xlsTempExcep(<?php echo $_SESSION['customerno']; ?>, <?php echo $_SESSION['switch_to']; ?>);
