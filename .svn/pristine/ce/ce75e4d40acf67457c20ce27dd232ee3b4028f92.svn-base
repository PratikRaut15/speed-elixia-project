...
if (isset($_REQUEST['order'])) {
    ...
    $street_landmark_area = urlencode($orders->street.", ".$orders->landmark.", ".$orders->area.", ".$orders->city);
    $landmark_area = urlencode($orders->landmark.", ".$orders->area.", ".$orders->city);
    //$landmark = urlencode($orders->landmark.", ".$orders->city);
    ...
    */
    ...
    $google_api2 = "https://maps.googleapis.com/maps/api/geocode/json?address=$street_landmark_area&region=in&key=$key";
    $google_api3 = "https://maps.googleapis.com/maps/api/geocode/json?address=$landmark_area&region=in&key=$key";
    $google_api4 = "https://maps.googleapis.com/maps/api/geocode/json?address=$area_city&region=in&key=$key";
    $google_api5 = "https://maps.googleapis.com/maps/api/geocode/json?address=$area_city_pin&region=in&key=$key";
    $google_api6 = "https://maps.googleapis.com/maps/api/geocode/json?address=$pincode&region=in&key=$key";
        ...
        if($array_data2->status === 'OK') {
         ...
         $ch1 = curl_init();
            ...
            if ($array_data5->status === 'OK' && $cn == 'India' ) {
                ...
                $ch1 = curl_init();
            curl_setopt($ch1, CURLOPT_URL, $google_api6);
            curl_setopt($ch1, CURLOPT_RETURNTRANSFER, 1); // return into a variable
            $results6 = curl_exec($ch1);
            $array_data6 = json_decode($results6);
            if(isset($array_data6->results[0]->address_components)){
            foreach($array_data6->results[0]->address_components as $add){
                if($add->types[0] == 'country'){
                    $cn = $add->long_name;
                }
            }
            }
            if ($array_data6->status === 'OK' && $cn == 'India' ) {
                $location = $array_data6->results[0]->geometry->location;
                $lat = $location->lat;
                $long = $location->lng;
                $accuracy = 6;
            }else{
    ...
    */
    ...
    $test = $apiobj->addEditOrder($orders);
    $testjson = json_encode($test);
    echo $testjson;
