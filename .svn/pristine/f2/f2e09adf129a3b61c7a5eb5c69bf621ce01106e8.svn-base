...
// $url = "http://localhost/speed/modules/api/gpsprovider/latLong/consumeApi.php";

$objJsonReq = new stdClass();
// $objJsonReq->apiUrl = "http://latlong.net.in/Client/LatLong_WebService-Reli.asmx?wsdl";//remark, we have used "?wsdl" for xml api
$objJsonReq->apiUrl = "http://latlong.net.in/Client/LatLong_Relicorp_SE.asmx"; 

$objJsonReq->isGetDataByDateTime = "1"; // (1) for get data from fromDateTime to toDtaeTime

$objJsonReq->customerNo = speedConstants::CUSTNO_RELICORP_LOGISTICS;
...
if (isset($jsonResponse) && $jsonResponse != "") {
    ...
    if ($outputData->Status == "1") {
        ...
        ProcessData($outputData->Result, $objJsonReq->isGetDataByDateTime);
    }
}

function ProcessData($arrLatestVehData, $isGetDataByDateTime=0) {
    $objVehicleManager = new VehicleManager(speedConstants::CUSTNO_RELICORP_LOGISTICS);
    foreach ($arrLatestVehData as $arrVehData) {
        ...
        // print("<pre>"); print_r($arrVehData); die;
        if (isset($vehData)) {
            ...
            #########################################################################
                ...
                if(isset($arrHistData) && !empty($arrHistData)){
                    ...
                    if(isset($isGetDataByDateTime) && $isGetDataByDateTime == 1){
                ...
                //Enter in Sqlite
                $objVehDetail->customerNo = speedConstants::CUSTNO_RELICORP_LOGISTICS;
