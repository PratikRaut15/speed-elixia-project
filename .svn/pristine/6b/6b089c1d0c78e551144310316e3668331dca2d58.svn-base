...
if (isset($_POST['nomensdeviceid']) && $_POST['nomensdeviceid'] != '') {
                ...
                if (isset($_SESSION['ecodeid'])) {
                    ...
                    if ($datecheck == 1) {
                        if ($datediffcheck <= 30) {
                            ...
                            if (isset($final_graph_data)) {
                                echo '<div id="temp_container'.$key.'" style="width: 950px; min-height: 600px; margin: 0 auto;"></div>';
                                echo "<br>";
                            }
                            ...
                            include 'pages/panels/groupwisetemprephist.php';
                            echo $return[0];
                        } else {
                            echo $error2;
                        }
                    ...
                    if ($datecheck == 1) {
                        if ($datediffcheck <= 30) {
                            ...
                            if (isset($final_graph_data[0]) && !empty($final_graph_data[0])) {
                                echo '<div id="temp_container'.$key.'" style="width: 950px; min-height: 600px; margin: 0 auto;"></div>';
                                echo "<br>";
                            } else {
                                echo '<div id="temp_container'.$key.'" style="width: 950px; min-height: 20px; margin: 0 auto;"></div>';
                                echo "<br>";
                            }
                            extract(get_min_max_temp($tempsel, $return[2]));
                            $static_min_temp = get_min_temperature(null, true);
    ...
    function showGraph(vehicleno,key) {
...
if (isset($final_graph_data[0]) && !empty($final_graph_data[0])) {
    ?>
        ...
        $('#temp_container'+key).highcharts('StockChart', {
        rangeSelector: {
        buttons: [{
        type: 'hour',
                count: 10,
    ...
    echo "jQuery('#temp_container".$key."').html('No data for Chart generation');";
}
?>
    }</script>
            <?php
            echo '<script> showGraph("' . $vehicleno . '",'.$key.'); </script>';
