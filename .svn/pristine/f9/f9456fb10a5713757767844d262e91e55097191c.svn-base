<?php
    ...
    function create_temphtml_from_report($datarows, $vehicle, $veh_temp_details = null, $interval = 120, $tempselect = null, $objReport=null) {
        ...
        case 2:
            ...
            if (isset($totalKelvinTemperature2)) {
                ...
                $finalKelvinTemperature = ($kelvinNumeratirComplete2 / $kelvinDenominatorComplete2);  */
            ...
            foreach ($datarows as $change) {
                ...
                if (strtotime($lastdate) != strtotime($comparedate)) {
                    ...
                    if (isset($tempselectArr) && $tempselectArr[0] != "null") {
                        ...
                        if ($cm_details->temp_sensors == 2 && $tempselectArr[0] == 2 && (isset($tempselectArr[1]) && $tempselectArr[1] == "all")) {
                            $display .= "<td style='width:150px;height:auto;'>Temperature 1</td>
                        <td style='width:150px;height:auto;'>Temperature 2</td>";
                        }
                    } else {
    ...
    function getgensetdata_fromsqlite($location, $deviceid, $startTime = null, $endtime = null, $gensetSensor = null) {
        ...
        unithistory.digitalio, unithistory.analog1 as extradigitalio " . $fuelsensor . "
        from devicehistory
            ...
            if (isset($result) && $result != "") {
                ...
                foreach ($result as $row) {
                    ...
                    if (isset($gensetSensor) && $gensetSensor == 2) {
                        if (@$laststatus['extradigitalio'] != $row['extradigitalio']) {
                            $fuel_ltr = "";
                            if ($unit->fuelsensor != 0) {
    ...
    function set_temp_graph_data($updated_date, $unit_type, $unit, $analog1, $analog2, $analog3, $analog4, $only_date = false, $tempSelected = 1) {
    ...
    function gettempdata_fromsqlite($location, $deviceid, $interval, $startdate, $enddate, $graph = false, $unit = null, $customerno = null, $isApi = null) {
        $devices = array();
        $graph_devices = array();
            ...
            if (isset($result) && $result != "") {
                ...
                foreach ($result as $row) {
                    ...
                    if ((!isset($lastupdated)) || (round(abs(strtotime($row['lastupdated']) - strtotime($lastupdated)) / 60, 2) >= $interval)) {
                        ...
                        $devices[] = $device;
                        if ($graph) {
                            $temp_val1 = set_temp_graph_data($row['lastupdated'], $unit_type, $unit, $row['analog1'], $row['analog2'], $row['analog3'], $row['analog4'], false, 1);
                            $temp_val2 = set_temp_graph_data($row['lastupdated'], $unit_type, $unit, $row['analog1'], $row['analog2'], $row['analog3'], $row['analog4'], false, 2);
                            $temp_val3 = set_temp_graph_data($row['lastupdated'], $unit_type, $unit, $row['analog1'], $row['analog2'], $row['analog3'], $row['analog4'], false, 3);
                            $temp_val4 = set_temp_graph_data($row['lastupdated'], $unit_type, $unit, $row['analog1'], $row['analog2'], $row['analog3'], $row['analog4'], false, 4);
    ...
    function getdailyreport_All($STdate, $EDdate, $vehicleid = NULL) {
        ...
        if (file_exists($location)) {
            ...
            $DATA = GetDailyReport_Data_All($location, $totaldays, $options = array('islocation' => 1, 'customerno' => array(), 'vehicleid' => $vehicleid, 'groupid' => array()));
        }
        return $DATA;
    }
                                                    ...
                                                    if (isset($result) && !empty($result)) {
                                                        foreach ($result as $resKey => $resVal) {
                                                            ...
                                                            if ($data['vehicleNo'] != '') {
                                                                ...
                                                                $data['duration']   = findTimeDifference($start_date,$end_date,'hour')." Hrs";*/
