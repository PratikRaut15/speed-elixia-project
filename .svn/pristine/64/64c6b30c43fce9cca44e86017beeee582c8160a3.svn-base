...
CREATE  PROCEDURE `get_transporter_proposed_indent_detail`(
	...
	, IN startdate DATE
	, IN enddate DATE
    , IN isAcceptedparam varchar(5)
    , IN isAutoRejectedparam varchar(5)
    , IN pageIndex INT
	, IN pageSize INT
...
BEGIN
    ...
    IF(enddate = '' OR enddate = 0) THEN
		SET enddate = NULL;
    END IF;
    IF(isAcceptedparam = '') THEN
		SET isAcceptedparam = NULL;
    END IF;
    IF(isAutoRejectedparam = '') THEN
		SET isAutoRejectedparam = NULL;
    END IF;
    ...
    SET recordCount = (SELECT count(pit.proposedindentid)	        
        ...
        AND	(pit.isAccepted = isAcceptedparam OR isAcceptedparam IS NULL)
        AND	(pit.isAutoRejected = isAutoRejectedparam OR isAutoRejectedparam IS NULL)
