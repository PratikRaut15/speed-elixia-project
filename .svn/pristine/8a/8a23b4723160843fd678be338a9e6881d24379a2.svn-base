        ...
        jQuery.each(allAllSlot, function (indexS, valueS) {
            ...
            if (keyval != undefined) {
                jQuery(keyval).each(function (indexLast, valueLast) {
                    jQuery('#slot' + valueS).parent().find('td').eq(zoneIndex).append("<div style='background-color:#46B111;margin-bottom:5px;'>" + allVehicles[valueLast[0]]['delboyname'] + close_img + "</div>");
                });
            }
        });
    });
...
function map_all() {
    ...
    if (jQuery.isEmptyObject(vehArr)) {
        alert('No vehicles assigned.');
        return false;
    }
    /*check if red color exists*/
    if (!checkRed()) {
        alert('Please clear the issue. For eg: 1 vehicle cannot be in 2 zones of same time-slot or Same vehicle in single Zone/Slot.');
        return false;
    }
    var data = vehArr;
