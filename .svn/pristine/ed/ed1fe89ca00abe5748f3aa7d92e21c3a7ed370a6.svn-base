       ...
       rename($target_path, $target_path_old."/information.sqlite" );
        ...
        try
        {
            ...
            if(isset($result))
            {
                ...
                $sm = new SCVF1Manager($customerno);                                    
            ...
            // Update Payment
            $resultp = $database->query('SELECT * FROM payment WHERE ispushed = 0');
            if(isset($resultp))
            {
                $jsonstatus->status = "successful";                                                                                                
                include_once("../../../lib/wbo/SCVF1Manager.php");                           
                ...
                {
                    $paymentmanage = new paymentmanagedb();
                    $paymentmanage->type = $row["type"];
                    $paymentmanage->partial_amt = $row["partial_amt"];
                    $paymentmanage->is_partial = $row["is_partial"];
                    $paymentmanage->chequeno = $row["chequeno"];
                    $paymentmanage->accountno = $row["accountno"];
                    $paymentmanage->branch = $row["branch"];
                    $paymentmanage->bank = $row["bank"];                    
                    $paymentmanage->reason = $row["reason"];
                    $paymentmanage->paymentid = $row["paymentid"];
                    
                    $serviceid = $row["serviceid"];
                    $sm->pushpartialpayment($serviceid, $paymentmanage);
                }
            }
            
            // Update Forms
            $resultf = $database->query('SELECT * FROM formresult WHERE ispushed = 0');
            if(isset($resultf))
            {
                $jsonstatus->status = "successful";                                                                                                
                include_once("../../../lib/wbo/SCVF1Manager.php");                           
                ...
                {
                    $formmanage = new formmanagedb();
                    $formmanage->serviceid = $row["serviceid"];
                    $formmanage->formid = $row["formid"];
                    $formmanage->dtid = $row["dtid"];
                    $formmanage->result = $row["result"];
                    $formmanage->datepushed = $row["datepushed"];
                    $sm->pushformresult($formmanage);
                }
            }
