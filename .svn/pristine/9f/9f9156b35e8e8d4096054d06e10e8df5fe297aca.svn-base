...
//$url = "http://speed.elixiatech.com/modules/api/gpsprovider/tracknovate/consumeApi.php";
$url = "http://localhost/speed/modules/api/gpsprovider/tracknovate/consumeApi.php";
$customerNo =  speedConstants::CUSTNO_LAXMIPARIWAHAN;
/* Request Parameters */
$objJsonReq = new stdClass();
$objJsonReq->userName = "lpplac";
$objJsonReq->password = "1223456";
...
function ProcessData($arrLatestVehData) {
    $objVehicleManager = new VehicleManager(speedConstants::CUSTNO_LAXMIPARIWAHAN);
    ...
    foreach ($arrLatestVehData as $arrVehData) {
        ...
        if (isset($vehData)) {
            ...
            $objVehDetail->customerNo = speedConstants::CUSTNO_LAXMIPARIWAHAN;
...
function InsertDataInSqlite($vehData, $objVehDetail) {
    if ($objVehDetail->unitNo != "") {
        ...
        $db = "sqlite:" . RELATIVE_PATH_DOTS . "customer/" . speedConstants::CUSTNO_LAXMIPARIWAHAN . "/unitno/" . $objVehDetail->unitNo . "/sqlite/" . $date . ".sqlite";
