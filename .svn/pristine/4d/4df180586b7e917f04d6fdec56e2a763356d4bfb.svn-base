            ...
            if ($initday > 0) {
                ...
                if ($validation['role'] != 'Viewer') {
                    ...
                    $sp_params1 = "'" . $pageIndex . "'"
                        ...
                        . ",'" . $groupidparam . "'"
                ;
                    $queryCallSP = "CALL " . SP_GET_VEHICLEWAREHOUSE_DETAILS_VIEWER . "($sp_params1)"; // For Grp Based Vehicles - 30 Nov
                }
        ...
        if ($validation['status'] == "successful") {
            ...
            if (!empty($vids)) {
                ...
                if (isset($record)) {
                    ...
                    if ($code->email != '') {
                        $subject = "Client Code";
                        $content = 'Your Client Code is ' . $ecoderandom . '. <br/>Generated by ' . $validation['realname'] . '. <br/>Please visit www.speed.elixiatech.com and trace your vehicle. <br/> Valid Until: ' . date(speedConstants::DEFAULT_DATETIME, strtotime($code->enddate));
                    ...
                    if ($code->sms != '') {
                        $smsmessage = 'Client Code: ' . $ecoderandom . '. Generated by ' . $validation['realname'] . '. Validity: ' . date(speedConstants::DEFAULT_DATETIME, strtotime($code->enddate)) . '. Link: www.speed.elixiatech.com';
            ...
            if (is_a($records, 'mysqli_result') && $recordCount > 0) {
                ...
                while ($row = $this->db->fetch_array($records)) {
                    ...
                    if ($username != '' && $userphone != '' && $vehicleno != '' && $drivername != '') {
                        ...
                        $smscount = $customermanager->getSMSStatus($customerno,$userid,$vehicleid);
                        if ($smscount == 0) {
                            $response = '';
