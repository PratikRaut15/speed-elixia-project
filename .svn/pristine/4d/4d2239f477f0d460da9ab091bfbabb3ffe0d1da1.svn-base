...
class Trips extends DatabaseManager {
    ...
    public function add_tripdetails($data) {
        ...
        $orderno = isset($data['chitthiNo']) && $data['chitthiNo'] != '' ? $data['chitthiNo'] : "";
        $orderdatetime = isset($data['statusdatetime']) && $data['statusdatetime'] != '' ? $data['statusdatetime'] : "";
        if ($orderdatetime != '') {
            $orderdatetime = date('Y-m-d H:i:s', strtotime($data['statusdatetime']));
        }
        if ($vehicleid > 0) {
            if ($unitno == "") {
        ...
        '%s','%s','%s','%s','%s','$this->today',$this->userid,'%s','%s')";
            ...
            /* For Expeditors */
            if ($this->customerno == 307) {
                $expSql = "Insert into expeditorsTripDetails (tripid, referenceNumber, customerNo, createdby, createdon)VALUES('%s','%s',%d,%d,'%s')";
                $expQuery = sprintf($expSql, Sanitise::String($triplastid), Sanitise::Long($data['triplogno']), $this->customerno, $this->userid, $this->today);
                $this->executeQuery($expQuery);
            }
        }
        return $tripId;
    }
    ...
    public function edittripdetails($data, $isApi = 0) {
        ...
        $statusodometer = $data['statusodometer'];
        $vehicleid = $data['vehicleid'];
        $data['returnYard'] = isset($data['returnYard']) ? $data['returnYard'] : '0';
        $tripstatusid = 0;
...
/////// end if status is 10 /////////////////////
        ...
        if (!isset($data['unitno'])) {
            $sql = "SELECT unitno FROM unit
                      LEFT JOIN vehicle ON vehicle.uid = unit.uid
        ...
        if (!empty($statusdatetimeedit)) {
            ...
            if (file_exists($location)) {
                $path = "sqlite:$location";
                $db = new PDO($path);
                $statusdatetime2 = $statusdatetimeedit;
    ...
    public function gettripdetailsedit($tripid) {
        ...
        $Query = "SELECT u.unitno,a.remark,a.odometer, a.perdaykm, a.vehicleid"
            ...
            . " ,a.etarrivaldate, a.materialtype ,SUM(od.bags) as totalbags, dc.chlNo, c.cname,od.truckNo ,od.stockCode, c1.eta as yardETA"
            ...
            . " left join orderDetails as od on od.tripid = a.tripid "
            . " left join deliveryChallan as dc on dc.orderid = od.orderid "
            . " left join tripYardLog as tyl on tyl.tripid = a.tripid "
            . " left join checkpoint as c on c.checkpointid = tyl.checkpointid
                LEFT JOIN checkpoint c1 ON c1.checkpointid = a.returnYard "
            . " WHERE a.customerno=$this->customerno AND a.tripid = $tripid AND a.isdeleted=0 GROUP BY od.tripid";
        $SQL = sprintf($Query);
        //echo $SQL;exit;
        $this->executeQuery($SQL);
        if ($this->get_rowCount() > 0) {
            while ($row = $this->get_nextRow()) {
                ...
                $tripedit->bags = $row['totalbags'];
                $tripedit->quantity = round(($row['totalbags'] / 20), 2);
                $tripedit->chlNo = $row['chlNo'];
                $tripedit->cname = $row['cname'];
                $tripedit->truckNo = $row['truckNo'];
                $tripedit->stockCode = $row['stockCode'];
                $tripedit->yardETA = $row['yardETA'];
                $tripedit->yardToCheckpointDistance = $this->getTripDetailsFromyardtocheckpoint($tripid, $this->customerno);
                $tripedit->YardToYardDistance = $this->getYardToYardDistance($tripid, $tripedit->entrytime, $tripedit->unitno, $tripedit->vehicleid, $this->customerno);
                $tripdata[] = $tripedit;
            }
            return $tripdata;
        }
    ...
    public function get_closed_triprecord($sdate = null, $edate = null) {
        ...
        $Query .= "DISTINCT(th.tripid),COALESCE(th.statusdate, '') as starttime, a.tripid,ts.tripstatus,a.routename,a.drivername,a.drivermobile1,a.drivermobile2,a.remark,a.budgetedkms,a.mintemp,
                ...
                from  tripdetails as a
                INNER JOIN tripdetail_history as th on a.tripid = th.tripid  AND th.isdeleted=0";
        /* remove condition to get all closed trip report AND th.tripstatusid=3 */
        ...
        if ($this->get_rowCount() > 0) {
            ...
            foreach ($data as $row) {
                ...
                $obj = new stdClass();
                $obj->tripid = $row['tripid'];
                $obj->customerno = $_SESSION['customerno'];
                $isTripEnd = 1;
                $tripLrDetails = $this->getTripLrDetails($obj, $isTripEnd);
                if (isset($tripLRDetails[0]['varChitthiCreation']) && isset($tripLRDetails[0]['varLrCreation'])) {
                    $varLrDelayTime = getTimeDiff($tripLRDetails[0]['varLrCreation'], $tripLRDetails[0]['varChitthiCreation']);
                } else {
                    $varLrDelayTime = '';
                }
...
////////////////////////////////////////////////////////////////////
                    ...
                    if ($sdate != null && $edate != null) {
                        $getdata[] = array(
                            ...
                            'statusdate' => date(speedConstants::DEFAULT_DATETIME, strtotime($row['statusdate'])),
                            'lrCreationTime' => isset($tripLrDetails[0]['varLrCreation']) ? date('d-m-Y h:i', strtotime($tripLrDetails[0]['varLrCreation'])) : '',
                            'yardCheckout' => isset($tripLrDetails[0]['varYardCheckout']) ? date('d-m-Y h:i', strtotime($tripLrDetails[0]['varYardCheckout'])) : '',
                            'lrDelayTime' => $varLrDelayTime,
                            'yardCheckin' => isset($tripLrDetails[0]['varYardCheckin']) ? date('d-m-Y h:i', strtotime($tripLrDetails[0]['varYardCheckin'])) : '',
                            'yardDetention' => isset($tripLrDetails[0]['varYardDetention']) ? date('d-m-Y h:i', strtotime($tripLrDetails[0]['varYardDetention'])) : '',
                            'emptyReturnDeviation' => isset($tripLrDetails[0]['varEmptyReturnDeviation']) ? date('d-m-Y h:i', strtotime($tripLrDetails[0]['varEmptyReturnDeviation'])) : ''
                        );
...
td.maxtemp,td.drivername,td.drivermobile1,td.drivermobile2,td.tripid,td.statusdate,
    ...
    public function gettripidbytriplogno($triplogno) {
        ...
        $Query = 'SELECT tripid FROM tripdetails WHERE customerno="' . $this->customerno . '" AND trim(triplogno) = "' . trim($triplogno) . '"';
        $SQL = sprintf($Query);
        $this->executeQuery($SQL);
        while ($row = $this->get_nextRow()) {
            $tripid = $row['tripid'];
    ...
    public function deMapDataForRTD($data) {
        // print_r($data); die;
        $tripid = $data['tripid'];
        $vehicleid = $data['vehicleid'];
    ...
    public function checkTripVehicleNo($vehicleno) {
        $is_triplogno_exists = 0;
        $Query = "  SELECT  tripid
                    FROM    tripdetails
                    WHERE   trim(vehicleno) = '" . trim($vehicleno) . "'
                    AND     is_tripend = 0";
        $SQL = sprintf($Query);
        $this->executeQuery($SQL);
        if ($this->get_rowCount() > 0) {
    ...
    public function insertTripOrderMapping($objTrip) {
        $insertedOrderId = 0;
        try {
            $pdo = $this->CreatePDOConn();
            ...
            . ",'" . $objTrip->isdeleted . "'"
                . ",@insertedOrderId";
            $queryCallSP = "CALL " . speedConstants::SP_MERGE_ORDER_DETAILS . "($sp_params)";
            $outputResult = $pdo->query($queryCallSP)->fetch(PDO::FETCH_ASSOC);
            $outputParamsQuery = "SELECT @insertedOrderId AS insertedOrderId";
            $outputResult = $pdo->query($outputParamsQuery)->fetch(PDO::FETCH_ASSOC);
            $insertedOrderId = $outputResult['insertedOrderId'];
        } catch (Exception $ex) {
            $log = new Log();
            $log->createlog($this->customerno, $ex, 0, "Trip", __FUNCTION__);
        }
    ...
    public function insertTripLrMapping($objTrip) {
        ...
        try {
            ...
            . ",'" . $objTrip->placeCode . "'"
            . ",'" . $objTrip->loading . "'"
            . ",'" . $objTrip->tpvPlacecode . "'"
            . ",'" . $objTrip->rem1 . "'"
            . ",'" . $objTrip->transpBillNo . "'"
            . ",'" . $objTrip->customerno . "'"
            . ",'" . $objTrip->userid . "'"
            . ",'" . $objTrip->todaysDate . "'"
            . ",'" . $objTrip->isdeleted . "'"
                . ",@insertChallanId";
            $queryCallSP = "CALL " . speedConstants::SP_MERGE_DELIVERY_CHALLAN . "($sp_params)";
            $outputResult = $pdo->query($queryCallSP)->fetch(PDO::FETCH_ASSOC);
            $outputParamsQuery = "SELECT @insertChallanId AS insertChallanId";
            $outputResult = $pdo->query($outputParamsQuery)->fetch(PDO::FETCH_ASSOC);
            $insertedOrderId = $outputResult['insertChallanId'];
        } catch (Exception $ex) {
            $log = new Log();
            $log->createlog($this->customerno, $ex, 0, "Trip", __FUNCTION__);
        }
    ...
    public function getDashboardTripDetails($objTrip) {
        $outputResult = null;
        $sql_trip = "";
        try {
            $pdo = $this->CreatePDOConn();
            ...
            "'" . $objTrip->customerno . "'"
            . ",'" . $objTrip->currentDate . "'";
            $queryCallSP = "CALL " . speedConstants::SP_GET_DASHBOARD_TRIPDETAILS . "($sp_params)";
            $outputResult = $pdo->query($queryCallSP)->fetch(PDO::FETCH_ASSOC);
    ...
    public function getInTransitVehicles($request) {
        $vehiclesQuery = "SELECT DISTINCT(T1.vehicleId) AS vehicleId
                            FROM tripdetails T1
                            ...
                            WHERE T1.customerno= $request->customerno
                            AND T1.is_tripend =0
                            AND T1.isdeleted =0
                            AND vehicle.chkpoint_status=1 ";
        $this->executeQuery($vehiclesQuery);
        $inTransitVehicles = array();
        if ($this->get_rowCount() > 0) {
            while ($row = $this->get_nextRow()) {
                $inTransitVehicles[] = $row['vehicleId'];
            }
        }
        return $inTransitVehicles;
    }

    public function get_vehicle_details($request) {
        $Query = "SELECT u.unitno,d.deviceid
            FROM vehicle as v
            LEFT join unit as u on u.uid = v.uid
            LEFT join devices as d on d.uid = v.uid
            WHERE v.customerno =$request->customerno AND v.vehicleid =$request->vehicleid AND v.isdeleted=0";
        $this->executeQuery($Query);
        if ($this->get_rowCount() > 0) {
            while ($row = $this->get_nextRow()) {
                $vehicleDetails['unitno'] = $row['unitno'];
                $vehicleDetails['deviceid'] = $row['deviceid'];
            }
    ...
    public function getTripDetailsReportData($objTrip) {
        $result = null;
        $sql_trip = "";
        try {
            $pdo = $this->CreatePDOConn();
            $sp_params = "'" . $objTrip->customerno . "'"
            . ",'" . $objTrip->reportType . "'"
            . ",'" . $objTrip->todaydate . "'";
            $queryCallSP = "CALL " . speedConstants::SP_GET_TRIPDETAILS_DATA . "($sp_params)";
            $result = $pdo->query($queryCallSP)->fetchAll(PDO::FETCH_ASSOC);
        } catch (Exception $ex) {
            $log = new Log();
            $log->createlog($this->customerno, $ex, 0, "Trip", __FUNCTION__);
    ...
    public function getInTransitVehicleDetails($request) {
        $arrVehicles = array();
        $vehiclesQuery = "SELECT DISTINCT(T1.vehicleId) AS vehicleId,T1.triplogno,unit.unitno, devices.deviceid,vehicle.vehicleno
        FROM tripdetails T1
        inner JOIN orderDetails O ON O.tripId = T1.tripId
        INNER JOIN deliveryChallan D ON D.orderId = O.orderId
        INNER JOIN vehicle ON vehicle.vehicleId = T1.vehicleId
    ...
    public function getTripLrDetails($objTrip, $isTripEnd) {
        $result = null;
        $sql_trip = "";
        try {
            $pdo = $this->CreatePDOConn();
            ...
            . "," . $objTrip->customerno . ""
                . "," . $isTripEnd . ""
            ;
            $queryCallSP = "CALL " . speedConstants::SP_GET_TRIP_LR_DETAILS . "($sp_params)";
            $result = $pdo->query($queryCallSP)->fetchAll(PDO::FETCH_ASSOC);
        } catch (Exception $ex) {
            $log = new Log();
            $log->createlog($this->customerno, $ex, 0, "Trip", __FUNCTION__);
    ...
    public function getTripDetailsFromyardtocheckpoint($tripid, $customerno) {
        //$tripid= '20455';
        $latLongDistanceDet = "";
        $query = "SELECT td.lat,td.lng,c.cgeolat,c.cgeolong
                   FROM tripdroppoints as td
                   LEFT JOIN checkpoint c ON c.customerno =td.customerno
                   WHERE tripId = '" . $tripid . "'
                   AND c.checkpointid =  (select checkpointid from tripYardLog tyl where tyl.tripId = '" . $tripid . "'
                   ORDER BY tyl.checkpointid DESC LIMIT 1)
                   AND td.customerno = '" . $customerno . "'
                   ORDER BY td.tripdroppointid ASC LIMIT 1 ";
        $this->executeQuery($query);
        if ($this->get_rowCount() > 0) {
            while ($row = $this->get_nextRow()) {
                $latLongDistanceDet = $this->getDistanceByLatLong($row['lat'], $row['lng'], $row['cgeolat'], $row['cgeolong']);
            }
        }
        return $latLongDistanceDet;
    }

    public function getDistanceByLatLong($lat, $lang, $cgeolat, $cgeolong, $earthRadius = 6371000) {
        // convert from degrees to radians
        $latFrom = deg2rad($lat);
        $lonFrom = deg2rad($lang);
        $latTo = deg2rad($cgeolat);
        $lonTo = deg2rad($cgeolong);
    ...
    public function getYardToYardDistance($tripid, $entrytime, $unitno, $vehicleid, $customerno) {
        $query1 = "SELECT odometer FROM tripdetails
                  WHERE tripid = '" . $tripid . "'
                   AND customerno = '" . $customerno . "' ORDER BY tripid ASC LIMIT 1";
        $this->executeQuery($query1);
        if ($this->get_rowCount() > 0) {
            while ($row1 = $this->get_nextRow()) {
                $firstodometer = $row1['odometer'];
            }
        }
        $date = substr($entrytime, 0, 10);
        $time = substr($entrytime, 10, 6);

        $path = RELATIVE_PATH_DOTS . "customer/$customerno/unitno/$unitno/sqlite/$date.sqlite";
        if (file_exists($path)) {
            $path = "sqlite:" . RELATIVE_PATH_DOTS . "customer/$customerno/unitno/$unitno/sqlite/$date.sqlite";
            $Query = "SELECT max(vehiclehistory.odometer) as odometer FROM vehiclehistory
                WHERE vehiclehistory.vehicleid=$vehicleid AND vehiclehistory.lastupdated >= '$entrytime'
        ...
        if ($lastodometer != $firstodometer) {
            $distance = ($lastodometer - $firstodometer) / 1000;
            return $distance;
        } else {
            return null;
        }
    ...
    public function getYardList($objTrip) {
        $arrYardList = null;
        try {
            $pdo = $this->CreatePDOConn();
            $sp_params = " " . $objTrip->customerno . ", "
            . " " . $objTrip->tripid . ", "
                . " @checkpointparam "
            ;
            $queryCallSP = "CALL " . speedConstants::SP_GET_YARD_LIST . "($sp_params)";
            $outputData['arrYardList'] = $pdo->query($queryCallSP)->fetchAll(PDO::FETCH_ASSOC);
            $outputParamsQuery = "SELECT @checkpointparam AS checkpointparam";
            $outputResult = $pdo->query($outputParamsQuery)->fetch(PDO::FETCH_ASSOC);
            $outputData['checkpointparam'] = $outputResult['checkpointparam'];
        } catch (Exception $ex) {
            $log = new Log();
            $log->createlog($this->customerno, $ex, 0, "Trip", __FUNCTION__);
        }
    ...
    public function addTripDetails($data) {
        $tripId = array();
        // print_r($data);
        // die();
        $customerNo = $this->customerno;
        if ($customerNo == 795) {
            $tripLogNo = isset($data['invoiceNo']) ? $data['invoiceNo'] : 0;
            $consignorId = 566; //Replace value with Live consignorId
            $consignor = 'Factory';
        }
        $Query = "INSERT INTO tripdetails (customerno,vehicleno,vehicleid,
        triplogno,tripstatusid,statusdate,consignor,consignee,consignorid,consigneeid,entrytime,addedby) ";
        $Query .= "VALUES ('%d','%s',%d, '%s',%d,'%s','%s','%s',%d,%d,'%s', %d)";
        $SQL = sprintf($Query, $customerNo, Sanitise::String($data['vehicleno']), Sanitise::Long($data['vehicleId']), Sanitise::String($tripLogNo), Sanitise::String($data['tripstatus']), Sanitise::String($data['todaysDate']), Sanitise::String($consignor), Sanitise::String($data['consignee']), Sanitise::String($consignorId), Sanitise::String($data['consigneeid']), Sanitise::String($data['todaysDate']), Sanitise::String($data['userid']));
        $this->executeQuery($SQL);
        $triplastid = $this->get_insertedId();
        if ($triplastid != '' || $triplastid != 0) {
            $tripId['tripId'] = $triplastid;
            $tripId['invoiceNo'] = $tripLogNo;
            $tripId['vehicleNo'] = $data['vehicleno'];
        }
