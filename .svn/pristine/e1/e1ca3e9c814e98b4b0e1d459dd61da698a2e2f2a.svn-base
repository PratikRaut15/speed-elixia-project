...
class api {
    ...
    function pushlocation($validation, $data) {
        $result = array();
        if (!empty($data['latitude']) && !empty($data['longitude']) && !empty($data['ignition']) && !empty($data['speed']) && !empty($data['lastupdated']) && !empty($data['vehicleno']) && !empty($data['odometer']) && !empty($data['unitno']) && !empty($data['online_offline'])) {
            $devicedata = $this->getDeviceid($data['unitno'], $validation['customerno']);
            $lat = $data['latitude'];
            $long = $data['longitude'];
            $ignition = isset($data['ignition']) ? $data['ignition'] : 0;
            $lastupdated = $data['lastupdated'];
            $online_offline = $data['online_offline'];
            $gsmstrength = isset($data['gsmstrength']) ? $data['gsmstrength'] : '';
            $direction = isset($data['direction']) ? $data['direction'] : '';
            $speed = $data['speed'];
            $odometer = $data['odometer'];
            $extbatt = $data['vehicle_battery'];
            $devsql = "update devices set devicelat='" . $lat . "',devicelong='" . $long . "',ignition='" . $ignition . "',lastupdated='" . $lastupdated . "',online/offline='" . $online_offline . "',gsmstrength='" . $gsmstrength . "',directionchange='" . $direction . "' where deviceid =" . $deviceid;
            $record = $this->db->query($devsql, __FILE__, __LINE__);

            $vehsql = "update vehicle set curspeed='" . $speed . "',odometer='" . $odometer . "',extbatt='" . $extbatt . "' where deviceid =" . $deviceid;
            $record = $this->db->query($vehsql, __FILE__, __LINE__);

            $result['status'] = "successful";
            $result['message'] = "Location added sucessfully.";
        } else {
            $result['status'] = "successful";
            $result['message'] = speedConstants::API_MANDATORY_PARAM_MISSING;
        }
    ...
    function getvehiclelist($validation, $vehicleno = null) {
        return $this->getlocation_vehicle($validation, $vehicleno);
    }

    function getlocation_vehicle($validation, $searchstring = null) {
        $pageIndex = 1;
        $pageSize = -1;
        $isWareHouse = 0;
        $groupidparam = 0;
        $isRequiredThirdParty = 0;
        $arr_p = array();
...
//                    }
    ...
    function update_user_notification_status($userid, $notification_status) {
        $sql = "update user set notification_status=" . $notification_status . " where userid =" . $userid;
        $record = $this->db->query($sql, __FILE__, __LINE__);
        $arr_p = array();
        $arr_p['status'] = "successful";
        echo json_encode($arr_p);
    ...
    function pushbuzzer($userkey, $vehicleno, $status) {
        //buzzer status =1 on // buzzer status==0 off  //buzzerstatus ==-1
        $validation = $this->check_userkey($userkey);
        if(!empty($vehicleno)) {
            $vehicleid = $this->getvehicleid($validation, $vehicleno);
        }
        $arr_p = array();
        $arr_p['status'] = "unsuccessful";
        if ($validation['status'] == "successful") {
    ...
    function pushmobiliser($userkey, $vehicleno, $status) {
    ...
    function freezevehicleapi($userkey, $vehicleno, $fstatus) {
        $arr_p['status'] = "unsuccessful";
        $validation = $this->check_userkey($userkey);
        if (!empty($vehicleno)) {
            $vehicleid = $this->getvehicleid($validation, $vehicleno);
    ...
    function generate_clientcode($validation,$data){
        $vehiclelist = $data['vehicleno'];
        ...
        if ($validation['status'] == "successful") {
            ...
            foreach ($vehicles as $vehicle) {
                $SQL = "select vehicleid from vehicle where customerno=$customerno and REPLACE(vehicleno, ' ', '') = '$vehicle'";
    ...
    function getsummary_details($validation, $vehicleno = null) {
        $userkey = $validation['userkey'];
        $vehicleid = '';
        if (!empty($vehicleno)) {
            $vehicleid = $this->getvehicleid($validation, $vehicleno);
    ...
    function getvehicleid($validation, $vehicleno) {
        $sql = "select vehicleid from vehicle where vehicleno LIKE'%" . $vehicleno . "%'";
        $record = $this->db->query($sql, __FILE__, __LINE__);
        while ($row = $this->db->fetch_array($record)) {
            $vehicleid = $row['vehicleid'];
        }
    ...
    function InsertChkptDetails($objChkPtReqDetails) {
        $arrResult = array();
        $arrResult['status'] = 0;
        $arrResult['message'] = constants::API_INVALID_USERKEY;
        $arrResponse = array();
    ...
    function PullAlertHistory($objAlertHistReqDetails) {
        ...
        if (isset($objAlertHistReqDetails)) {
            $arrResult['status'] = 1;
    ...
    function getalerthistory($validation, $vehicleno) {
        $vehicleid="";
        if (!empty($vehicleno)) {
            $vehicleid = $this->getvehicleid($validation, $vehicleno);
        }
        ...
        echo $queueQuery .= " ORDER BY  comqueue.timeadded ASC ";
        ...
        while ($row = $this->db->fetch_array($record)) {
            $data = array(
                'type' => $row['type'],
                'timeadded' => date("g:i a", strtotime($row["timeadded"])),
                'message' => $row['message'],
    ...
    function check_userkey($userkey) {
        $sql = "select * from user where sha1(userkey)='" . $userkey . "' AND isdeleted=0";
        $record = $this->db->query($sql, __FILE__, __LINE__);
        $row = $this->db->fetch_array($record);
        $retarray = array();
        if ($row['userkey'] != "") {
    ...
    function GetOdometer_Max($day, $unitno, $customerno) {
    ...
    function updateSMSCount($existingSMSCount, $smsmessage, $customerno) {
        ...
        $divide = floor($smslength / speedConstants::PER_SMS_CHARACTERS);
        $mod = $smslength % speedConstants::PER_SMS_CHARACTERS;
        if ($mod > 0) {
