...
include_once $RELATIVE_PATH_DOTS . "lib/autoload.php";
    ...
    $url = "http://speed.elixiatech.com/modules/api/gpsprovider/rytMap/consumeApi.php";
    // $url = "http://localhost/elixia_speed/modules/api/gpsprovider/rytMap/consumeApi.php";
    
    $userHashKey = '$2y$10$vGr08CFNYTtlmyKaPrrZPOShBlnVKRkU2HsIiQ37z3UhZPpmTHuEy';
    
    $objJsonReq = new stdClass();
    $objJsonReq->userHashKey = $userHashKey;
    $objJsonReq->apiUrl = "http://track.rytmap.com/api/get_devices?";
    $objJsonReq->customerNo = speedConstants::CUSTNO_SHREE_KRISHNA_CONTAINER_MOVERS;

    $jsonreq = json_encode($objJsonReq);
    $postData = array(
        'action' => "getAllVehicleData",
        'jsonreq' => $jsonreq,
        'postDataForConsumeApi' => array('user_api_hash'=> $userHashKey)
    );
...
function ProcessData($arrLatestVehData) { 
    $objVehicleManager = new VehicleManager(speedConstants::CUSTNO_SHREE_KRISHNA_CONTAINER_MOVERS);
    ...
    if(isset($arrLatestVehData) && !empty($arrLatestVehData) && is_array($arrLatestVehData)){
        foreach ($arrLatestVehData as $arrVehData) {
            ...
            /*print("<pre>"); print_r($objVehDetail); die;*/
            if (isset($vehData)) {
...
print_r($arrHistData[0]);*/
                ...
                //Enter in Sqlite
                $objVehDetail->customerNo = speedConstants::CUSTNO_SHREE_KRISHNA_CONTAINER_MOVERS;
