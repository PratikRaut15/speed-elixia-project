...
//    $data = '';
	foreach ($freeze as $thisfreeze) {
		if ($thisfreeze->ignition == 1) {
			...
			$cradfreeze = (isset($thisfreeze->freeze_radius) AND ($thisfreeze->freeze_radius > 0)) ? $thisfreeze->freeze_radius : FREEZE_RADIUS; // 40 Meter define in utility page
			...
			if ((float) $distance_freeze >= (float) $cradfreeze) {
				...
				$vehiclemanager->unfreezedvehicle($thisfreeze->unitno, $thisfreeze->customerno, $getfreezedata['userid'], $getfreezedata['isAlertSent']);
				// Insert in Queue
				...
				if ($getfreezedata['isAlertSent'] == 0) {
					//to check if alert already sent.
					$cqm->InsertQChk($cvo);
				}
			}
		}
...
$customerExceptionList = "161,59,100,328,520,682,63";
$chkpts = $cm->getalldeviceswithchkpforcron($customerExceptionList);


//echo"<br>cron devices data is: <pre>"; print_r($chkpts); exit();
if (isset($chkpts)) {
	echo count($chkpts);
	foreach ($chkpts as $thischkpt) {
		//echo 'CHK: '.$thischkpt->checkpointid.'and customer no: '.$thischkpt->customerno.'<br>';
		...
		if ($distance >= $crad && $thischkpt->conflictstatus == 0) {
			// Invoked function for updating column 'isALertSent' = 0 in checkpointwise_stoppage_alert when vehicle moves out of checkpoint
			updateCHeckPointWiseStoppageAlert($thischkpt->checkpointid, $thischkpt->vehicleid, $thischkpt->customerno);
			// Insert in Queue
			$cqm = new ComQueueManager();
			$cvo = new VOComQueue();
			...
			if (!$isExistsRecord) {
				...
				if(isset($thischkpt->isErp) && $thischkpt->isErp == 1){
					$arrPost = array();
					$url = "http://uat-erp.elixiatech.com/api/updateTripStatusOnCheckPoints";
					$ch = curl_init($url);
					$arrPost['vehicleid'] = !empty($thischkpt->vehicleid) ? $thischkpt->vehicleid : "";
					$arrPost['vehicleno'] = !empty($thischkpt->vehicleno) ? $thischkpt->vehicleno : "";
					$arrPost['userkey'] = !empty($thischkpt->userkey) ? sha1($thischkpt->vehicleno) : ""; 
					$arrPost['checkPointStatus']['checkPointId'] = !empty($thischkpt->checkpointid) ? $thischkpt->checkpointid : ""; 
					$arrPost['checkPointStatus']['status'] = 'Out';
				...
				if (in_array($thischkpt->customerno, $route_dashboard_customer)) {
					...
					$interval = 5;
					/* Shop Alert Timing Changed For Kamaths - 682*/
					if ($thischkpt->customerno == 682) {
				...
				if ($thischkpt->customerno == 447) {
					$objTripLog = new stdClass();

					$objTripLog->vehicleid = $thischkpt->vehicleid;
					$objTripLog->checkpointid = $thischkpt->checkpointid;
					$objTripLog->eta = '';
					$objTripLog->ata = $thischkpt->lastupdated;
					$objTripLog->customerno = $thischkpt->customerno;
					$objTripLog->userid = 0;
					$objTripLog->todaydate = date('Y-m-d h:i:s');
					$vehiclemanager = new VehicleManager($thischkpt->customerno);
					$vehiclemanager->updateTripYardLog($objTripLog);
				}
			}
			...
			$cvo->lastupdated = $thischkpt->lastupdated;
			$isExistsRecord = $cqm->checkComQueExistance($cvo);
			/* function invoked to update vehicle route direction starts here */
			$cm->updateRealTimeRouteDirection($thischkpt->checkpointid, $thischkpt->vehicleid, 0, $thischkpt->customerno);
			/* function invoked to update vehicle route direction ends here */
			if (!$isExistsRecord) {
				...
				if(isset($thischkpt->isErp) && $thischkpt->isErp == 1){
					...
					$arrPost['userkey'] = !empty($thischkpt->userkey) ? sha1($thischkpt->vehicleno) : ""; 
					$arrPost['checkPointStatus']['checkPointId'] = !empty($thischkpt->checkpointid) ? $thischkpt->checkpointid : ""; 
					$arrPost['checkPointStatus']['status'] = 'In';
					curl_setopt($ch, CURLOPT_HEADER, false);
					curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
					curl_setopt($ch, CURLOPT_POST, true);
					curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query($arrPost));
...
function updateCHeckPointWiseStoppageAlert($checkpointid, $vehicleid, $customerno) {
	$cronManagerObject = new CronManager();
	$cronManagerObject->updateIsALertSentForCheckPointWiseStoppageAlerts($checkpointid, $vehicleid, $customerno);
}
