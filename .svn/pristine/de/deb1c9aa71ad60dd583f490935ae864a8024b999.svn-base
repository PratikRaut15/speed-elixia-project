...
include_once 'genset_functions.php';
...
if (isset($_POST['vehicleno']) && !isset($_POST['vehicleid'])) {
  ...
  echo $status;
} else if ($action == 'genset') {
  $q = ri($_REQUEST['term']);
  $vm = new VehicleManager($customerno, $userid);
  $result = $vm->getGenset($q);
  echo json_encode($result);
  exit;
} else if ($action == 'transmitter') {
  $q = ri($_REQUEST['term']);
  $vm = new VehicleManager($customerno, $userid);
  $result = $vm->getTransmitter($q);
  echo json_encode($result);
  exit;
}
?>
