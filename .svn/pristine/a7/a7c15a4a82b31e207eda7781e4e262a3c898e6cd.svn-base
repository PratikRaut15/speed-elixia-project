...
if ($work == 'pull_acc_history') {
    get_acc_approval_form_by_vehicle_id($acc_id);
}
...
if ($work == 'push_acc') {
    pushacc($main_id, $status, $note);
}

if ($work == 'approved') {
    $status = approved($vehicleid, $notes, $user_id, $_POST['veh_notes']);
    echo $status;
}

if ($work == 'reject') {
    $status = reject($vehicleid, $notes, $user_id, $_POST['veh_notes']);
    echo $status;
}
...
if (isset($_POST['vehicleno']) && !isset($_POST['vehicleid'])) {
    ...
    echo $vehicleid;
} else if (isset($_POST['edit_vehicle_id'])) {
    $vehicleid = edit_vehicle($_POST);
    echo $vehicleid;
    ...
    echo $status;
} else if (isset($_REQUEST['work']) && $_REQUEST['work'] == 'view_approvetransaction') {
    ...
    doMultipleApproval($_POST['action'], $_POST['approvaldata'], $_POST['note']);
