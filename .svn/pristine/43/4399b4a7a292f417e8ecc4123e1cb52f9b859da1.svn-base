...
class api {
    ...
    function dashboard($userkey, $pageIndex, $pageSize, $searchstring, $groupidparam, $isRequiredThirdParty, $isWareHouse) {
        ...
        if ($validation['status'] == "successful") {
            ...
            if ($initday > 0) {
                ...
                if ((is_array($grouplist) && isset($grouplist)) && count($grouplist) > 0) {
        ...
        if ($validation['status'] == "successful") {
            ...
            while ($row = $this->db->fetch_array($records)) {
                ...
                /* hardcoaded temp_sensors = 1 for Icelings Vehicle temp dashboard*/
                    if((isset($row['customer_no']) && $row['customer_no'] == 193) && $isWareHouse == 0){
                        $row['temp_sensors'] = 1;
                        $json_p[$x]['temp_sensors'] = $row['temp_sensors'];
                    }
                ...
                if ($row['use_humidity'] == 1 && $row['humidity'] != 0) {
                    ...
                    if ($customerno == 673) {
                        ...
                        $json_p[$x]['temp2'] = isset($temp4) ? $temp4 : "";
                    }
                }
                ...
                $veh_status = $this->getstatus($row['stoppage_flag'], $row['stoppage_transit_time'], $row['lastupdated'], $row['gpsfixed']);
                $json_p[$x]['veh_status'] = $veh_status;
    ...
    function getstatus($stoppage_flag, $stoppage_transit_time, $lastupdated_store, $gpsfixed = null) {
        ...
        if ($lastupdated < $ServerIST_less1) {
            ...
            $diff = $this->getduration_status($stoppage_transit_time);
            if ($stoppage_flag == '0') {
