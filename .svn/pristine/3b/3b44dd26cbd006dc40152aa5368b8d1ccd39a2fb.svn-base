jQuery(function(){
    ...
    jQuery('#changeStatus').change(function(){
        statChangeAjax(jQuery(this).val(), jQuery('#clientID').val());
    });
...
function dragdrop(){
    ...
    //hoverClass: "activeTd", 
        drop: function( event, ui ) {
            ...
            var rDiv = "<div class='allDataCell draggable' style='"+2+"' >"+ui.draggable.text()+"</div>"
...
function filterTimeline(tkname){
    ...
    function statusChange(sdate,services,mobile,cdetails,clientID,address,cost,time,discount){
        
    jQuery("#statusChngeBuble").css({"visibility":"visible"});
    jQuery('#clid').val(clientID);
