...
function getchks() {
    ...
    $checkpoints = $checkpointmanager->getcheckpointsforcustomercz();
    return $checkpoints;
}
...
function getchk($chkid) {
    ...
    $checkpointmanager = new CheckpointManager($_SESSION['customerno']);
    $checkpoint = $checkpointmanager->get_checkpointcz($chkid);
    return $checkpoint;
}
...
function deletechk($chkid) {
    $chkid = GetSafeValueString($chkid, 'long');
    $checkpointmanager = new CheckpointManager($_SESSION['customerno']);
    $checkpointmanager->DeleteCheckpointcz($chkid, $_SESSION['userid']);
}
...
function addchk($form) {
    ...
    $checkpoint->groupid = GetSafeValueString($form["grouplist"], "string");
    ...
    $checkpointmanager = new CheckpointManager($_SESSION['customerno']);
    $checkpointmanager->SaveCheckpointcz($checkpoint, $_SESSION['userid']);
}
