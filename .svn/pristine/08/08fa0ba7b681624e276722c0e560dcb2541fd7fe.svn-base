...
$SQL = sprintf ("SELECT c.renewal, c.totalsms, c.customerno, c.customername, c.smsleft, c.customercompany,count(unit.unitno) AS cunit FROM ".DB_PARENT.".customer c
                 LEFT OUTER JOIN unit ON c.customerno=unit.customerno AND unit.trans_statusid not in (10) WHERE c.renewal = -2
...
if(isset($customers))
{
    ...
    $SQL3 = sprintf("SELECT devices.deviceid,devices.customerno,devices.uid,count(simcardid) AS sim FROM devices 
                     ...
                     INNER JOIN simcard ON devices.simcardid = simcard.id
...
<br/>
<div class="panel">
<div class="paneltitle" align="center">Closed Customer List <span style="float: right;">Total Customers: <?php echo($srno); ?>, Total Units: <?php echo($totalunits); ?>, Total Simcards : <?php echo($totalsimcount); ?>, Pending Collection : Rs <?php echo($totalpayment); ?> /- </span></div>
