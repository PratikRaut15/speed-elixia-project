...
define("SAHARAROADLINES_CUST_ID_SR", "SR");
...
$geoTackUsers = array(
    ...
    array(
        "userName" => "coolways",
        "pwd" => "cways@123", //cw9101
    ),
    array(
        "userName" => "sahararoad",
        "pwd" => "NITIN1995", // trackit
    ),
    array(
        "userName" => "shahvts",
        "pwd" => "trackit",
...
foreach ($geoTackUsers as $user) {
    // echo $user['userName']. "and ". $user['pwd']."<br>";
		$url = "http://speed.elixiatech.com/modules/api/gpsprovider/geotrackers/consumeApi.php";
		//$url = "http://localhost/speed/modules/api/gpsprovider/geotrackers/consumeApi.php";
		$objJsonReq = new stdClass();
		$objVendorDetails = new stdClass();
		// $objVendorDetails->userName = "sahararoad";
		// $objVendorDetails->userName = "shahvts";
		// $objVendorDetails->pwd = "trackit";

		$objVendorDetails->userName = $user['userName'];
		$objVendorDetails->pwd = $user['pwd'];
        $objVendorDetails->customerNo = speedConstants::CUSTNO_SAHARA_ROADLINES;
...
function ProcessData($arrLatestVehData) {
    $objVehicleManager = new VehicleManager(speedConstants::CUSTNO_SAHARA_ROADLINES);
    ...
    if(isset($arrLatestVehData) && !empty($arrLatestVehData) && is_array($arrLatestVehData)){
        foreach ($arrLatestVehData as $arrVehData) {
            ...
            if (isset($vehData)) {
                ...
                //Enter in Sqlite
                $objVehDetail->customerNo = speedConstants::CUSTNO_SAHARA_ROADLINES;
