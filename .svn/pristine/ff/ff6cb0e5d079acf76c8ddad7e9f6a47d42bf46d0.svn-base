...
CREATE TABLE `speed.bucket_purpose` (
...
ALTER TABLE `elixiatech`.`sp_ticket`
ADD COLUMN `docketid` INT NULL DEFAULT 0 AFTER `prodId`;

ALTER TABLE `elixiatech`.`sp_ticket_details`
 ADD COLUMN `prodId` INT ;

ALTER TABLE `elixiatech`.`sp_ticket_details`
ADD COLUMN `docketid` INT NULL DEFAULT 0 AFTER `prodId`;

ALTER TABLE `speed.bucket` add column (docketid INT(11))
...
BEGIN
    ...
    START TRANSACTION;
        ...
        IF (customerNoParam IS NOT NULL ) THEN
            ...
            ELSE
                INSERT INTO elixiatech.`sp_ticket_details`(`ticketid` ,`description`,`allot_from`,`allot_to`,`status`,`create_by`,`create_on_time`,`eclosedate`,`created_type`,`docketid`,`prodId`)
...
BEGIN
  SELECT DISTINCT (customerno),rel_manager,customercompany,
    ...
    ELSE
      team.teamid
    END) as allot_to
  FROM speed.customer, elixiatech.team
