...
class ServiceCallV3Manager extends VersionedManager
{
    public function ServiceCallV3Manager($customerno)
    {
        // Constructor.
		...
		if(strtotime($row['expiry'])>=strtotime($expiry." 00:00:00"))
                {
                    ...
                    {
                        if($this->check_servicecall_with_code($client,$row['dis_id'])==0 && ($this->check_discount_for_client($client)!=0 || $this->check_discount_for_client($client)!=""))
                        {
                            $json_discobj->error=false;
                            $json_discobj->dis_amt=$row['dis_amt'];
                            $json_discobj->dis_id=$row['dis_id'];
