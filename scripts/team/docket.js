    ...
    var str = "\
       ...
       <table>\
          ...
          <input type='hidden' id='ticketId" + ticketCount + "' name='ticketId" + ticketCount + "' value=''>\
            <td>Ticket title:</td>\
            <td >\<input class='formControl' id='title" + ticketCount + "' name='title" + ticketCount + "' type='text'></td>\
            <td>Description: </td>\
            <td >\
               <textarea class='formControl' id='description" + ticketCount + "' name='description" + ticketCount + "'></textarea>\
            </td>\
        </tr>\
        <tr>\
        <td> Email:</td>\
            <td >\
               <input type='text' class='formControl' id='email" + ticketCount + "' name='email" + ticketCount + "' onkeyup='getmailids(" + ticketCount + ")'><input type='hidden' id='emailList" + ticketCount + "' name='emailList" + ticketCount + "'>\
            </td>\
            <td> <input type='button' class='g-button g-button-submit' style='background-color:#3b5998;color:#ffffff;' onclick='insertMailId("+ticketCount+")' value='Add Mail Id' name='addMail'></td>\
            </tr>\
            <tr>\
            <td>CC:</td>\
            <td >\
            <input type='text' class='formControl' id='CC"+ticketCount+"' name='CC"+ticketCount+"' onkeyup='getmailids_CC("+ticketCount+")'>\
        ...
        <tr>\
            ...
            <td >\
                <select class='formControl' id='priority" + ticketCount + "' name='priority" + ticketCount + "'></select>\
            </td>\
            <td>Ticket type: </td>\
            <td >\
                <select class='formControl' id='type" + ticketCount + "' name='type" + ticketCount + "' onchange='filterTeamList("+ticketCount+")'></select>\
            </td>\
        </tr>\
        <tr>\
            <td>Product: </td>\
            <td >\
                <select class='formControl' id='product" + ticketCount + "' name='product" + ticketCount + "'></select>\
            </td>\
            <td>Allot to: </td>\
            <td >\
                <select align='right ' class='formControl' id='allot" + ticketCount + "' name='allot" + ticketCount + "'></select>\
            </td>\
        </tr>\
        </table>\
         <input type='button' id='saveTicket' onclick='refreshGrid(" + ticketCount + ");' value='Save ticket'>\
        </form>\
        <table>\
        <tr>\
        <td>\
        <form id='fileForm"+ticketCount+"'><input type='file' name='file"+ticketCount+"' id='file"+ticketCount+"' onclick='fileUpload("+ticketCount+");' accept='application/zip,application/rar'/></form>\
        </td>\
        <td>\
    ...
    jQuery.ajax({
        ...
        success: function(data) {
            ...
            $.each(result[1], function(i, text) {
                console.log("texte");
                console.log(text);
                if(details.allot_to==text.teamid){
                    teamOptions = teamOptions + "<option value='"+text.teamid+"' data-did="+text.department_id+" selected>"+text.name+"</option>";
                }else{
    ...
    <div class='modal-content' id='modalContent" + ticketCount + "'>\
       ...
       <table>\
          <tr>\
            ...
            <td >\ <input class='formControl' id='title" + ticketCount + "' name='title" + ticketCount + "' type='text' value='" + details.title + "' readOnly></td>\
            <td>Description: </td>\
            <td >\
            <textarea class='formControl' id='description" + ticketCount + "' name='description" + ticketCount + "'>" + details.description + "</textarea>\
            </td>\
        </tr>\
        <tr>\
            <td>Email:</td>\
            <td >\ <input type='text' class='formControl' id='email" + ticketCount + "' name='email" + ticketCount + "' onkeyup='getmailids(" + ticketCount + ")'><input type='hidden' id='emailList" + ticketCount + "' name='emailList" + ticketCount + "'>\
            </td>\
            <td> <input type='button' onclick='insertMailId("+ticketCount+")' style='background-color:#3b5998;color:#ffffff;' value='Add Mail Id' name='addMail'></td>\
        </tr>\
        <tr>\
            <td>CC:</td>\
            <td >\
            ...
            <td>Ticket Status: </td>\
            <td><select id='status"+ticketCount+"' name='status"+ticketCount+"'></td>\
        </tr>\
        <tr>\
            <td>Ticket Priority: </td>\
            <td >\
            ...
            <td>Ticket type: </td>\
             <td >\
                <select class='formControl' id='type" + ticketCount + "' name='type" + ticketCount + "' readOnly onchange='filterTeamList("+ticketCount+")'></select>\
            </td>\
        </tr>\
        <tr>\
            <td>Product: </td>\
            <td>\
                <select class='formControl' id='product" + ticketCount + "' name='product" + ticketCount + "' readOnly></select>\
            </td>\
            <td>Allot to: </td>\
           <td >\
                <select align='right ' class='formControl' id='allot" + ticketCount + "' name='allot" + ticketCount + "'></select>\
