    ...
    jQuery.each(allAllSlot, function(indexS, valueS){
        ...
        if(keyval!=undefined){
            jQuery(keyval).each(function(indexLast, valueLast){
                jQuery('#slot'+valueS).parent().find('td').eq(zoneIndex).append("<div style='background-color:#46B111;margin-bottom:5px;'>"+allVehicles[valueLast[0]]['username']+close_img+"</div>");
            });
        }
    });
});
