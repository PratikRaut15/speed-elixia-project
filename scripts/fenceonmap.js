...
function mapfence() {
	var fence = "fenceid=" + encodeURIComponent(fenceid);
        ...
        var datastring = fence + "&get=fence";
                        jQuery.ajax({
                                    type: "POST",
                                    url: "route_ajax.php",
                                    data: datastring,
...
function plotfence(fdata) {
	for (var key in fdata) {
		/* if (fdata.hasOwnProperty(key)) { */
			try {
				initmap(fdata[key].cgeolat, fdata[key].cgeolong);
				var fencelatlng = new google.maps.LatLng(fdata[key].cgeolat, fdata[key].cgeolong);
				
				route.push(fencelatlng);
			} catch (ex) {
				alert(ex);
			}
		   console.log(fdata[key].cgeolat);
		   console.log(fdata[key].cgeolong);
	/* 	} */
	 }
...
function datapush() {
	...
	var vehiclearray = new Array();
        ...
        var datastring = "fenceid=" + fenceid + "&fencename=" + fencename + "&lat=" + latitudes + "&long=" + longitudes + "&vehiclearray=" + vehiclearray;
        jQuery.ajax({
		type: "POST",
		data: datastring,
                cache: false,
