...
class DailyReportManager extends VersionedManager {
   ...
   WHERE vehicle.isdeleted = 0';
    ...
    public function get_daily_report_mysql($date, $customerno, $vehicleid, $fuelbalance, $devicelat, $devicelong) {
        $i = 0;
        $query = "SELECT uid
                ,harsh_break
                ,sudden_acc
...
//                $update[$i]['end_lat'] = 0;
    ...
    public function getDailySMSDetails($customerno) {
        $dailyData = array();
    ...
    public function getMessages($customerno) {
        $dailyData = array();
    ...
    public function getDailyReportDetails($objDailyReport) {
        $i = 0;
        $dailyData = array();
        $query = "SELECT d.uid
                ,d.vehicleid
    ...
    public function get_daily_report_mysql_night($date, $customerno, $vehicleid) {
        $i = 0;
    ...
    public function get_daily_report_mysql_night_firstodo($date, $customerno, $vehicleid) {
        $i = 0;
    ...
    public function set_daily_report_mysql_night_firstodo($data, $date) {
    ...
    public function reset_dailyreport($data, $date) {
        /*
        foreach ($data as $customerno => $this_veh_details) {
        foreach ($this_veh_details as $veh_id => $veh_details) {
        $update_query = "update dailyreport set ";
    ...
    public function incre_fenceconflict($vehicleid, $customerno) {
        $query = "update dailyreport set fenceconflict=fenceconflict+1 where vehicleid=$vehicleid and customerno=$customerno";
        $this->_databaseManager->executeQuery($query);
    }

    public function incre_running_time($vehicleid, $customerno, $val, $isfreeze = null) {
        $freezeQuery = '';
    ...
    public function incre_idleignition_time($vehicleid, $customerno, $val, $isfreeze = null) {
        $freezeQuery = '';
        $todaysDate = date('Y-m-d');
        if (isset($isfreeze) && $isfreeze == 1) {
            $freezeQuery .= ", freezeIgnitionOnTime = freezeIgnitionOnTime+$val";
    ...
    public function update_previous_odometer($vehicleid, $customerno, $odometer, $lastupdated) {
    ...
    public function get_daily_report_mysql_night_per_userTime($date, $customerno, $vehicleid) {
        $i = 0;
    ...
    public function set_daily_report_mysql_night_travel($data, $date) {
        $count = 0;
        foreach ($data as $customerno => $this_veh_details) {
            foreach ($this_veh_details as $veh_id => $veh_details) {
                $update_query = "update dailyreport set ";
    ...
    public function GetDailyReport_Listener($objRequest) {
        ...
        #################################################################
         */
        $daily_date = "'" . $objRequest->daily_date . "'";
        ...
        $Query = "SELECT flag_harsh_break
                        ...
                        , daily_date
                        FROM dailyreport
                        WHERE vehicleid = %d AND daily_date = %s
                        ORDER BY daily_date
                        DESC LIMIT 1";
        $dailyReportQuery = sprintf($Query, Sanitise::String($objRequest->vehicleid), $daily_date);
