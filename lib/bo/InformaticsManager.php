...
class InformaticsManager {
    ...
    public function getInformatics($start_date, $end_date){
        $groupwise_vehicles = groupBased_vehicles_cron($this->customerno, $this->userid);
        $all_vehicles = array();
        ...
        $location = "../../customer/$this->customerno/reports/dailyreport.sqlite";
        if (!file_exists($location)) {
            return array('total' => 0);
        }
        $report_data = $this->get_daily_informatics($location, $totaldays);
        //print_r($report_data);
        ...
        if (!empty($report_data)) {
            ...
            $topspeedData = array();
            usort($overspeedsumVehicleWise, array('InformaticsManager','compare_values'));
            ...
            usort($harshbreakVehicleWise, array('InformaticsManager','compare_harshbreak'));
            ...
            */
            ...
            usort($accelerationVehicleWise, array('InformaticsManager','compare_accelaration'));
            ...
            */
            ...
            usort($nightDriveVehicleWise, array('InformaticsManager','compare_nightdrive'));
            ...
            */
            ...
            usort($weekendDriveVehicleWise, array('InformaticsManager','compare_weekenddrive'));
            ...
            */
            ...
            foreach ($overspeedsumVehicleWise as $data) {
                ...
                foreach ($totaldays as $day) {
                    ...
                    $location = "../../customer/$this->customerno/unitno/" . $data['unitno'] . "/sqlite/$date.sqlite";
                    if (file_exists($location)) {
                        $trackCount +=1;
                    }
                }
    ...
    public function get_daily_informatics($location, $days) {
        $objUnitManager = new UnitManager($this->customerno);
                ...
                if (isset($result) && $result != "") {
                    foreach ($result as $row) {
                        if (in_array($row['uid'], $arrUids)) {
                            ...
                            $objVehicleManager = new VehicleManager($this->customerno);
                            $Datacap->vehicleno = $objVehicleManager->getVehicleName($row['vehicleid']);
                            /* GET unit no from vehicleid */
                            $objUnitManager = new UnitManager($this->customerno);
                            $Datacap->unitno = $objUnitManager->getunitno($row['vehicleid']);
