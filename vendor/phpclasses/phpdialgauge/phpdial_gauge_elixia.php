...
define('BLANK_DIAL', "../../vendor/phpclasses/phpdialgauge/gauge_blank_elixia.png"); // A blank gauge Image
define('BLANK_IMG', "../../vendor/phpclasses/phpdialgauge/blank.png");   // A blank PNG Image
define('NEEDLE', "../../vendor/phpclasses/phpdialgauge/gauge_needle.png");  // Our Needle Image
...
class dial {
    ...
    function dial_gauge($value, $display_value = true, $range_bottom = 0, $range_top = 100) {
        ...
        if ($display_value) {
            ...
            imagestring($this->gaugeIMG, 3, $x_pos, Y_VAL_POS, floor($value), imagecolorallocate($this->gaugeIMG, 0, 0, 0));
        }
		...
		/*
        $correction = 0;
        //Correction for new gauge image
        if ($value <= 20) {
            $correction = (14 - $value);
        }
		...
		*/
        ...
        $needleIMG = imagerotate($needleIMG, -$angle, 0);  // max range from 0 -> -240
        ...
        imagepng($this->gaugeIMG); //This will normally output the image, but because of ob_start(), it won't.
        $imageContent = ob_get_contents(); //Instead, output above is saved to $contents
        ob_end_clean();
        return $imageContent;
    }
}
